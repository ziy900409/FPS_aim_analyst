# Graph Report - FPS_aim_analyst  (2026-07-14)

## Corpus Check
- 163 files · ~253,322 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1193 nodes · 2812 edges · 77 communities (60 shown, 17 thin omitted)
- Extraction: 92% EXTRACTED · 8% INFERRED · 0% AMBIGUOUS · INFERRED: 228 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `0c419d8d`
...
1. `createSharedState()` - 57 edges
2. `createSimLoop()` - 36 edges
3. `createDataRecorder()` - 35 edges
4. `loadDrill()` - 31 edges
5. `createTargetManager()` - 28 edges
6. `collectMeta()` - 21 edges
7. `規格書 v1.1 + WBS` - 21 edges
8. `createDrillRunner()` - 20 edges
9. `pushEvent()` - 20 edges
10. `generateRecoilTable()` - 18 edges
...
Cohesion: 0.07
Nodes (51): assertFinitePayload(), buildExportPayload(), downloadCSV(), downloadJSON(), downloadTextFile(), exportBasename(), formatBoolean(), formatNumber() (+43 more)
...
Cohesion: 0.11
Nodes (23): defaultLoader(), disposeScene(), loadScene(), createSceneManager(), createSceneManagerWithStatus(), SceneManager, err(), requireColorNumber() (+15 more)
...
Cohesion: 0.1
Nodes (19): assertReadyState(), createProtocolRunner(), requireCondition(), requireNonEmptyString(), requirePositiveInteger(), requireRecord(), validateProtocol(), applyResolutionMode() (+11 more)
...
Cohesion: 0.12
Nodes (9): consume(), createInputSampler(), isDrivenMotion(), motionOffset(), offsetAt(), triangleWave(), drainAll(), drainToArray() (+1 more)
...
Cohesion: 0.19
Nodes (11): createDataRecorder(), capacityForDrill(), createDrillRunner(), canonicalRun(), runFrames(), snap(), syntheticInputs(), canonicalLongrangeFrames() (+3 more)
...
Cohesion: 0.16
Nodes (8): setup(), createTargetManager(), collectImmediateSpawns(), killSequence(), motionConfig(), runDrill(), runPositions(), freshLoop()
...
Cohesion: 0.2
Nodes (11): createPointerLock(), beginNextProtocolCondition(), completeActiveProtocolCondition(), exportBasename(), onExportCSV(), onExportJSON(), protocolRunningText(), setProtocolStatus() (+3 more)

### Community 23 - "Community 23"
Cohesion: 0.29
Nodes (12): createMovementController(), run(), runBatches(), createBulletArena(), createImpactRing(), createInputRing(), createSharedState(), createShotRayRing() (+4 more)
...
Cohesion: 0.23
Nodes (11): createSimLoop(), applyMode(), canonicalFrames(), counterstrafeFrames(), FakeRenderer, framesAt(), runCounterstrafeTrace(), runDetectionSpawnSequence() (+3 more)
...
Cohesion: 0.36
Nodes (7): cameraLookingDownZ(), fireAt(), makeTarget(), meanSpreadRadius(), run(), runtime(), createRan1()
...
- **Why does `createDataRecorder()` connect `Community 15` to `Community 0`, `Community 37`, `Community 42`, `Community 46`, `Community 18`, `Community 22`, `Community 27`?**
  _High betweenness centrality (0.017) - this node is a cross-community bridge._
- **Why does `createSharedState()` connect `Community 23` to `Community 1`, `Community 2`, `Community 37`, `Community 42`, `Community 44`, `Community 13`, `Community 46`, `Community 15`, `Community 47`, `Community 18`, `Community 19`, `Community 27`?**
  _High betweenness centrality (0.017) - this node is a cross-community bridge._
- **Why does `generateRecoilTable()` connect `Community 1` to `Community 2`, `Community 27`, `Community 37`?**
  _High betweenness centrality (0.015) - this node is a cross-community bridge._
- **Are the 27 inferred relationships involving `createSharedState()` (e.g. with `setup()` and `fireAt()`) actually correct?**
  _`createSharedState()` has 27 INFERRED edges - model-reasoned connections that need verification._
- **Are the 20 inferred relationships involving `createSimLoop()` (e.g. with `buildSimLoop()` and `resetBulletArena()`) actually correct?**
  _`createSimLoop()` has 20 INFERRED edges - model-reasoned connections that need verification._
- **Are the 20 inferred relationships involving `createDataRecorder()` (e.g. with `capacityForDrill()` and `fireAt()`) actually correct?**
  _`createDataRecorder()` has 20 INFERRED edges - model-reasoned connections that need verification._
- **Are the 12 inferred relationships involving `loadDrill()` (e.g. with `loadDrillById()` and `loadSceneById()`) actually correct?**
  _`loadDrill()` has 12 INFERRED edges - model-reasoned connections that need verification._