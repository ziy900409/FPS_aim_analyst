...
## Corpus Check
- 95 files · ~145,217 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 745 nodes · 1590 edges · 50 communities (35 shown, 15 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 114 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `ae8f0477`
...
1. `createSharedState()` - 36 edges
2. `規格書 v1.1 + WBS` - 21 edges
3. `createSimLoop()` - 20 edges
4. `WP-3 輸入採集層 InputSampler (F1)` - 17 edges
5. `generateRecoilTable()` - 16 edges
6. `exec-plan/README.md 執行計畫索引` - 16 edges
7. `simStep()` - 14 edges
8. `PLAN.md 執行計畫 (階段 A)` - 14 edges
9. `createDataRecorder()` - 13 edges
10. `createRan1()` - 13 edges
...
- `runFrames()` --calls--> `createDrillRunner()`  [INFERRED]
  tests/regression/determinism.test.ts → src/drill/DrillRunner.ts
- `runFrames()` --calls--> `createSimLoop()`  [INFERRED]
  tests/regression/determinism.test.ts → src/loop/SimLoop.ts
...
Cohesion: 0.06
Nodes (48): assertFinitePayload(), buildExportPayload(), downloadCSV(), downloadJSON(), downloadTextFile(), exportBasename(), formatBoolean(), formatNumber() (+40 more)

### Community 1 - "Community 1"
Cohesion: 0.09
Nodes (41): buttonCss(), createNumberInput(), drawGrid(), drawPattern(), maxSpreadRadiusDeg(), mountPatternViewer(), niceStep(), patternBounds() (+33 more)
...
Cohesion: 0.09
Nodes (24): capacityForDrill(), keyMaskFromKeys(), keyMaskFromState(), keysFromMask(), TickArena, buildPeekWindows(), coefficientOfVariation(), computeMetrics() (+16 more)
...
Cohesion: 0.11
Nodes (30): loadDrill(), err(), requireFiniteNumber(), requireNonNegativeNumber(), requireObject(), requirePositiveInt(), requirePositiveNumber(), validateDrill() (+22 more)
...
Cohesion: 0.19
Nodes (7): createDrillRunner(), setup(), createTargetManager(), killSequence(), runDrill(), createFpsTestHarness(), makeHarness()
...
Cohesion: 0.28
Nodes (8): canonicalRun(), runFrames(), snap(), syntheticInputs(), createMovementController(), run(), runBatches(), createSharedState()
...
Cohesion: 0.33
Nodes (9): createDataRecorder(), recordVisibleEvents(), simStep(), cameraLookingDownZ(), fireAt(), makeTarget(), meanSpreadRadius(), run() (+1 more)
...
- **Why does `createSharedState()` connect `Community 19` to `Community 1`, `Community 5`, `Community 9`, `Community 10`, `Community 11`, `Community 15`, `Community 16`, `Community 21`, `Community 23`, `Community 25`?**
  _High betweenness centrality (0.014) - this node is a cross-community bridge._
- **Why does `createSimLoop()` connect `Community 15` to `Community 0`, `Community 1`, `Community 9`, `Community 10`, `Community 19`, `Community 21`?**
...
- **Are the 6 inferred relationships involving `generateRecoilTable()` (e.g. with `runtime()` and `createSimLoop()`) actually correct?**
  _`generateRecoilTable()` has 6 INFERRED edges - model-reasoned connections that need verification._
