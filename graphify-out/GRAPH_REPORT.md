...
## Corpus Check
- 140 files · ~207,497 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1021 nodes · 2313 edges · 70 communities (53 shown, 17 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 170 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `03cda050`
...
1. `createSharedState()` - 46 edges
2. `createSimLoop()` - 26 edges
3. `createDataRecorder()` - 22 edges
4. `loadDrill()` - 22 edges
5. `createTargetManager()` - 21 edges
6. `規格書 v1.1 + WBS` - 21 edges
7. `collectMeta()` - 18 edges
8. `generateRecoilTable()` - 18 edges
9. `WP-3 輸入採集層 InputSampler (F1)` - 17 edges
10. `simStep()` - 16 edges
...
Cohesion: 0.14
Nodes (10): collect64HzSamples(), recordVisibleEvents(), simStep(), cameraLookingDownZ(), fireAt(), makeTarget(), meanSpreadRadius(), run() (+2 more)
...
Cohesion: 0.21
Nodes (16): createPointerLock(), activeWeaponConfig(), buildCurrentExportPayload(), buildSimLoop(), exportBasename(), findSceneOption(), installSceneLoad(), loadDrillById() (+8 more)
...
Cohesion: 0.22
Nodes (9): createDataRecorder(), capacityForDrill(), canonicalRun(), runFrames(), snap(), syntheticInputs(), pushEvent(), canonicalFire() (+1 more)
...
Cohesion: 0.23
Nodes (10): createMovementController(), run(), runBatches(), createTargetManager(), collectImmediateSpawns(), killSequence(), motionConfig(), runDrill() (+2 more)

### Community 23 - "Community 23"
Cohesion: 0.21
Nodes (10): createSimLoop(), canonicalTrajectory(), freshState(), runFrames(), snap(), syntheticInputs(), baseClock(), holdFire() (+2 more)
...
Cohesion: 0.21
Nodes (6): createDrillRunner(), setup(), applyAimFixture(), canonicalMovingTargetFrames(), createCamera(), runMovingTarget()
...
- **Why does `createSharedState()` connect `Community 22` to `Community 35`, `Community 36`, `Community 37`, `Community 6`, `Community 10`, `Community 12`, `Community 14`, `Community 20`, `Community 23`, `Community 26`?**
  _High betweenness centrality (0.022) - this node is a cross-community bridge._
- **Why does `TickArena` connect `Community 31` to `Community 20`?**
  _High betweenness centrality (0.018) - this node is a cross-community bridge._
- **Why does `CameraController` connect `Community 32` to `Community 16`?**
  _High betweenness centrality (0.018) - this node is a cross-community bridge._
- **Are the 21 inferred relationships involving `createSharedState()` (e.g. with `setup()` and `fireAt()`) actually correct?**
  _`createSharedState()` has 21 INFERRED edges - model-reasoned connections that need verification._
- **Are the 13 inferred relationships involving `createSimLoop()` (e.g. with `buildSimLoop()` and `createMovementController()`) actually correct?**
  _`createSimLoop()` has 13 INFERRED edges - model-reasoned connections that need verification._
- **Are the 11 inferred relationships involving `createDataRecorder()` (e.g. with `capacityForDrill()` and `fireAt()`) actually correct?**
  _`createDataRecorder()` has 11 INFERRED edges - model-reasoned connections that need verification._
- **Are the 8 inferred relationships involving `loadDrill()` (e.g. with `loadDrillById()` and `loadSceneById()`) actually correct?**
  _`loadDrill()` has 8 INFERRED edges - model-reasoned connections that need verification._