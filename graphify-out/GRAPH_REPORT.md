...
## Corpus Check
- 70 files · ~89,614 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 600 nodes · 1172 edges · 43 communities (26 shown, 17 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 59 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `58fb1e7a`
...
Cohesion: 0.05
Nodes (47): createDataRecorder(), loadDrill(), createDrillRunner(), setup(), consume(), createInputSampler(), createPointerLock(), createRenderLoop() (+39 more)
...
Cohesion: 0.16
Nodes (14): 正規單位 (canonical unit, source unit u/s), CS2 physics 常數 (階段 B 校準起點), 雙迴圈 (dual-loop), RenderLoop 元件, RenderSnapshot 窄介面, 速度歸零誤差 (residual speed), SharedState 元件, 階段 B (Stage B) (+6 more)
...
Cohesion: 0.27
Nodes (10): 不可違反的硬約束 (技術), cross-origin isolation (COOP/COEP), 決定性 (determinism), MetricsDashboard 元件, D1 — 2D UI = 純 TS + DOM overlay, D3 — COOP/COEP 部署 (Vite plugin + 靜態主機後定), ADR-2 邏輯與渲染解耦 (雙迴圈 + fixed-timestep), ADR-4 performance.now() + cross-origin isolation (+2 more)
...
- **Why does `規格書 v1.1 + WBS` connect `Community 8` to `Community 9`, `Community 10`, `Community 11`, `Community 12`, `Community 13`, `Community 16`?**
