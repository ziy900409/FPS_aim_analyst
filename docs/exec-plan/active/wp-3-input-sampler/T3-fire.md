...
| **Touches** | MODIFY `src/input/InputSampler.ts`（開火部分） |
| **Status** | ⬜ TODO |
...
## Steps
- [ ] `mousedown` handler（僅 locked + button 0）寫 fire 事件。
- [ ] Vitest：合成 mousedown → 緩衝含 fire 事件 + 時間戳。
- [ ] 手動驗：鎖定中點左鍵入緩衝；未鎖定不入。
- [ ] `vitest run` + `tsc` 綠燈。

## Definition of Done
- [ ] 開火事件帶 `event.timeStamp` 入緩衝；僅鎖定中採計。
