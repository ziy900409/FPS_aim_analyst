...
## Steps
- [ ] 建 `DrillRunner.ts` 狀態機。
- [ ] sim tick 呼叫 `DrillRunner.tick`。
- [ ] Vitest：idle→countdown→running→ended phase 轉換時序正確；endCondition（目標數/時限）觸發 ended；restart 後 state/target/首發全空。
- [ ] 手動驗：開始倒數 → 遊玩 → 達標結束 → 重來乾淨。
- [ ] `vitest run` + `tsc` 綠燈。

## Definition of Done
- [ ] 生命週期完整；endCondition 正確；restart 後無殘留狀態（測試斷言）。
