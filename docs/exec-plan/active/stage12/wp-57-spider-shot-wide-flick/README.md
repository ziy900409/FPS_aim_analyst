...
| **OQ-57.5** | 抬滑鼠疑慮旗標的門檻（movement 窗內角速度停滯的 ms 與角速度閾值）？ | 🟡 **已校準於單一硬體，跨硬體待驗（2026-09-08 稍晚，D-57.T5-7）**：使用者提供四份真人 run（照常打／全程不抬滑鼠／每次都抬滑鼠／刻意短停頓，指示即 ground truth），交付門檻改為 **`stallMinMs = 150`／`stallOmegaDegPerSec = 2`**（TP 78%、偽陽 3%、baseline 0%、刻意停頓 17%）。⚠️ **合成期的兩個結論都被推翻**：`100 / 15` 會標掉 44% 的對照 run；「分離軸是 ω」也錯了（真人四份 run 的最小 `abs(omega)` 全為 0.0，分離軸實為 **duration @ 緊 ω 門檻**）。仍開放：緊 ω 門檻**條件於錄製機器的取樣特性**、[KI-031](../../../../known_issue/KI-031-detection-sustained-ticks-dies-when-aim-updates-slower-than-sim.md) 修好後須重跑、`cm/360` 方向性未答（感度與指示共線）、n=1。詳見 [progress.md](progress.md) §T5-real。<br>⚠️ **四份匯出不進 repo（D-57.T5-8）⇒ 上述數字在本 repo 內不可重現**，只留 sha256；要動它們必須依 [`spider-wide-recording-spec.md`](../../../../operational/spider-wide-recording-spec.md) 重錄，分析走 `npm run analyze:spider-wide`（對共線 cohort 會拒答） | 使用者（實機）+ 工程 | ~~T5 exit~~ 晉升 WP 前 | 門檻過鬆會標掉正常停頓，過緊則漏掉真實抬滑鼠；v1 practice-only 且旗標不進任何報告，故不阻擋交付 |
...
| **OQ-57.8**（T-exit 後新增，2026-09-08） | 直線透視下**離軸的球投影為橢圓**，軸比 ≈ `1/cos θ`（`cos θ = cos(yaw)·cos(pitch)`），螢幕足跡 ∝ `1/cos³θ`。以實錄的 FOV 75／aspect 2.0031 計：周邊目標軸比 **1.57–1.75**、足跡是中心目標的 **5.4×**、且**窗內**足跡隨 yaw 變動 **1.38×**；FOV 120 則為 **2.51–3.31**、**36.4×**、**2.30×**。足跡隨 yaw 單調變化 ⇒ **與 `D_deg`（條件變因）共變**。要 **(a)** 不動刺激，另交付離線純函式把 θ／軸比／足跡變成可量測的共變量；**(b)** 提高 `kLo` 壓縮窗內變異；還是 **(c)** 限制本 drill 的 FOV 上限？ | **(a)**。理由：這是直線透視的正確行為而非缺陷（渲染器沒錯），也是 FOV 120 玩 CS2 的生態真實；而重建所需的量（目標座標、`meta.scene.eye`、`resolvedFrom.fovDegVertical`／`.aspect`）**匯出裡已全有**，故**不需要新增任何匯出欄位**，形狀比照 `deriveMouseThrow()` 與抬滑鼠旗標。**(b) 是 OQ-57.3 未計價的第二個後果**：`kLo` 0.92→0.97 可把 FOV 120 的窗內足跡變異由 2.05× 壓到 1.34×（16:9 計），代價是 `D_deg` 變異一併變窄。**(c)** 直接違反 D-57.P2「每位選手同樣貼邊」。<br>**明確駁回**：① 讓世界空間球半徑隨 θ 縮小以維持螢幕面積 —— 破壞 GD-7 並使**命中幾何**的角徑不再恆定，等於拿視覺混淆換角度混淆；② 改用永遠正圓的 billboard —— 高 θ 下「看到的」與「要打的」差很多，`fireAngleErrorDeg` 無法判讀；③ 改非直線投影（Panini／圓柱）—— `ndc_x = tan(yaw)/tan(halfHFOV)` 不再成立，T1 resolver、FR-57.4、NFR-57.4 全部作廢 <br><br>✅ **已處置 2026-09-08：採 (a) 並落地**（D-57.T7-1）—— `src/metrics/projectedFootprint.ts` + 13 tests（**閉式解，非小角近似**）。刺激零修改、匯出零新增欄位。左列八個數字由出貨 resolver 的 yaw 窗逐項重現，**唯一更正**：形心外移「1.7–4.9%」是用 **16:9** 算的，與同句其他數字（實錄 aspect 2.0031）不同源 —— 實錄 aspect 下上界為 **5.50%**（結論不變；四個 FOV 檔位的外角一律 < 6%）。另一項實作發現：`resolvedFrom.aspect` **不影響**橢圓在物理螢幕上的形狀（只影響 NDC 水平縮放），它真正的作用點是解析 yaw 窗。詳見 [progress.md](progress.md) §T7 | ✅ 已處置（量化分析仍待研究） | ~~晉升 WP 前~~ 已處置；**把足跡當共變量放進 `switchReaction` 模型的量化分析仍在晉升 WP**（需真人資料 + 統計設計） | `switchReaction`（`tDetectMs`／`reactionMs`）的偵測難度與條件變因共變且**未被記錄** ⇒ 「偏心度越大反應越慢」可能部分是「目標越大越長」。**不影響**命中判定（`HitDetector` 走世界空間 ray–sphere，要打中的是角度上的正圓）與 `firstShot`／`stopControl`（開火時目標已在軸上、渲染為正圓）。`W_deg = 2.0°` 也沒有說謊 —— 球在虛擬眼睛處張的就是圓錐，橢圓是投影到**平面**螢幕的結果 |
...
| **T5** | 抬滑鼠疑慮標註純函式 + 門檻敏感度表 | T4 | Med | 1–2d | ✅ **2026-09-08 完成**：`deriveRepositioningSuspicion()` 對五類合成訊號（真停滯／刻意停頓／一次到位／onset 前停滯／on-target 後停滯）分類正確；窗界兩端取自既有 canonical derivations（與 `movementTimeMs` 同一對邊界），ω 走 `omegaDegPerSec()`，無第二套實作；C-D3 過閘寫成「`src/` 內零 importer」；門檻敏感度表與 `cm/360` 方向性表已交付但**建在合成 cohort 上**（repo 內無真人 wide-flick 匯出），故 **OQ-57.5 標 blocked 而非收斂** |
