...
| **OQ-57.5** | 抬滑鼠疑慮旗標的門檻（movement 窗內角速度停滯的 ms 與角速度閾值）？ | 🟡 **仍開放（2026-09-08，D-57.T5-4）**：T5 已交付網格與參考值 `stallMinMs = 100`／`stallOmegaDegPerSec = 15`（**未凍結、未寫成 production 常數**），但兩張表都建在**注入 ground truth 的合成 cohort** 上 —— repo 內不存在真人的 wide-flick 匯出（`spiderWideDeterminismFixture` 把 aim 固定在中心、`research/fixtures/exports/` 全是 counterstrafe）。T5 另量到一件規劃期未預期的事：分離「被迫抬滑鼠」與「刻意停頓」的是 **ω 軸**而非 duration 軸 | 使用者（實機）+ 工程 | ~~T5 exit~~ 晉升 WP 前 | 門檻過鬆會標掉正常停頓，過緊則漏掉真實抬滑鼠；v1 practice-only 且旗標不進任何報告，故不阻擋交付 |
