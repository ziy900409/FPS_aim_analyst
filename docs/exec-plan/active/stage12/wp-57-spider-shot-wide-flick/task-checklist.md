...
- [x] `spider-shot-v1`／`v2` 的 spawn 序列與既有條件欄位輸出**逐位不變**；`spiderShotMetrics.ts` 零修改。（T2：golden 先錄後改、前 200 spawn byte-identical；T-exit：`spiderShotMetrics.ts` git diff 為空、`spiderShotConditions.ts` 只有 27 行 additive、`DrillMetricRegistry.ts` 本 WP 零 diff）
...
- [x] boundary scans 綠；build／兩個 typecheck／全量 Vitest exit 0（238 files／2,399 tests）。⚠️ **全量 Playwright 與 `test:ci` 未 exit 0**（`test:ci` 實測 exit 1）——唯一成因為既存的 KI-027，非本 WP；另新立 KI-030（全量 Playwright 在多 worker 下不可重現，本 T-exit 以 `--workers=1` 的 90 passed／1 failed 為門檻讀數）。記為**已揭露的殘留**，見 progress §T-exit「未達成的 gate」
