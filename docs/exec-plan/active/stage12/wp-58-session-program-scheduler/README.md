...
| **Status** | 🟡 **T0～T5 ✅（2026-09-08）／T6 ✅（2026-09-09）**，T-exit 未開工。T0：36 個 exact drillId 的歸屬表已凍結（§2.3）、CodeGraph impact 已對帳（§0.1）、OQ-58.1／58.2／58.4 已收斂（§1.5）、[GD-35](../../../DECISIONS.md) 已入帳。T1：`drillFamily.ts` 雙向單一來源上線，4 個新家族純加法納入 allowlist。T2：`sessionProgram.ts` 編譯器交付（尚未接線）。全量 Vitest 2,538 passed／2 skipped。T3：`SessionRunner` 游標化上線，frozen 與 custom 兩軌共用同一 runtime；`main.ts` 完成分支鏈四路收斂為三路；全量 Vitest **2,570 passed／2 skipped**。T4：`SessionPlanSetup` 新增自訂 program 軌（分組 drill 選單／reps／兩級秒數／編譯器驅動的預覽表）、`RestOverlay` 帶邊界與下一個 drill（OQ-58.3 落地）、`main.ts` 兩軌分流；frozen 軌 e2e **零修改**全綠，全量 Vitest **2,595 passed／2 skipped**。T5：5 個 additive optional 稽核欄位上線（寫入端對 `FAMILY_BY_DRILL_ID` 嚴驗、讀取端只驗形狀）、逐 rep 匯出可定位且檔名唯一（`exportBasename` 零修改）、custom run 由新的 `excluded-cohort` status 排除於 frozen trend cohort 而 history 保存不變；**frozen 軌匯出逐位不變**（不寫 `sessionPlanMode`，D-58-T5-1）；8 個既有 fixture digest 逐位相同、`research/` ingest 零修改相容；全量 Vitest **2,661 passed／2 skipped**。T6：`session-orchestrator.spec.ts` 擴充為 4 條自訂 program 表單 DOM 案例 + 3 條真實瀏覽器 live run（custom 3 家族 × 2 reps 的 11 步／逐段休息時長／6 份唯一匯出／收工狀態、frozen 兩家族、中途載入失敗中止），由一條**只跳過資格閘拒入、不偽造通過**的 dev-only seam 驅動；全量 Playwright **99 passed** exit 0、build／typecheck exit 0、`graphify update .` 已同步。⚠️ T6 另發現 **[OQ-58.6](#15-open-questions)**（frozen 軌 `tracking` 家族開場即中止）——建議列為 T-exit blocker。證據見 [progress.md](progress.md) |
...
| **OQ-58.6** | frozen 軌 `tracking` 家族的代表 drill `tracking_v1` 未綁 `sceneId`，於開機場景 `field-low` 載入即 clearance throw ⇒ 勾該家族的 frozen session 開場即中止。要修嗎？怎麼修？ | ① `resolveFamilyDrillId('tracking')` 改為已綁場景的 `tracking_scene_v1`；或 ② 為 `tracking_v1` 補 `sceneId` | 使用者 | T-exit 前 | **建議列為 T-exit blocker**：操作員可選到一個必然中止的家族。T6 已用真實 e2e 釘死故障，未改 production（新增於 T6） |
| **OQ-58.7** | 中止的 session 是否應呼叫 `experimentSession.exit()`？ | 視為缺陷則於 `poll()` 與 `startSessionPlan()` 的 catch 一併 `exit()` | 使用者 | T-exit 前 | 影響面僅 `meta.display.gate`／`suspect` 的後續歸屬；現況已由失效案例斷言釘死（新增於 T6） |
