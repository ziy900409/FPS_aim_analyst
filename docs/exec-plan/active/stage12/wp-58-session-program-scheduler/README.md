...
| **Status** | 🟡 **T0 ✅／T1 ✅／T2 ✅／T3 ✅／T4 ✅（2026-09-08）**，T5～T-exit 未開工。T0：36 個 exact drillId 的歸屬表已凍結（§2.3）、CodeGraph impact 已對帳（§0.1）、OQ-58.1／58.2／58.4 已收斂（§1.5）、[GD-35](../../../DECISIONS.md) 已入帳。T1：`drillFamily.ts` 雙向單一來源上線，4 個新家族純加法納入 allowlist。T2：`sessionProgram.ts` 編譯器交付（尚未接線）。全量 Vitest 2,538 passed／2 skipped。T3：`SessionRunner` 游標化上線，frozen 與 custom 兩軌共用同一 runtime；`main.ts` 完成分支鏈四路收斂為三路；全量 Vitest **2,570 passed／2 skipped**。T4：`SessionPlanSetup` 新增自訂 program 軌（分組 drill 選單／reps／兩級秒數／編譯器驅動的預覽表）、`RestOverlay` 帶邊界與下一個 drill（OQ-58.3 落地）、`main.ts` 兩軌分流；frozen 軌 e2e **零修改**全綠，全量 Vitest **2,595 passed／2 skipped**。證據見 [progress.md](progress.md) |
