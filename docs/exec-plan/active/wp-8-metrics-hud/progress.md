...
## Status: 🟡 T0 STOP 已解除（M3 達成 2026-07-03）；待鎖 OQ-8.5 後翻 T0 ✅

| Phase | State |
|-------|-------|
| T0 Entry gate | 🟡 STOP 已解除（M3 達成）；8 指標對照 + OQ-8.1~8.4 鎖定，待鎖 **OQ-8.5**（GD-4 準心契約 B+C2）後翻 ✅ |
| T1 指標計算 | ⬜ 待執行 |
| T2 結果頁 | ⬜ 待執行 |
| T3 即時 HUD | ⬜ 待執行 |
| T4 控制 | ⬜ 待執行 |
...
### 2026-07-03 — T0 STOP 解除（M3 達成）+ 鎖 OQ-8.5（GD-4 準心契約 B+C2）
...
- **待辦**：翻 T0 ✅ 前完成 OQ-8.5 契約確認（本 log 已鎖建議解法）。
...
| 準心對齊偏移 | `ticks.crosshair`; fire 當下 target/hit context | 開火事件點的準心/射線與 hitbox 中心偏移 | 現有 fire event 未記 `targetId`/offset；T1 需用 tick/target 狀態可還原性確認，若不足要先補記錄契約。 |
| 切換時間 | hit `fire.t` + 下一 target acquisition | `t_next_acquisition - t_prev_kill` | `t_prev_kill` 可由 hit fire 取得；`t_next_acquisition` 目前未直接記錄，T1 需定義由準心/target 狀態還原或新增事件。 |
