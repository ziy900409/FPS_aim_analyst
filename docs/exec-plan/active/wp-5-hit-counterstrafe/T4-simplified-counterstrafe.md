...
## Steps
- [ ] `step` 加反向鍵偵測 → 立即停止 flag + vx=0。
- [ ] fire 結果附 `accurate` + `residualSpeed`。
- [ ] Vitest：移動中按反向鍵 → stopped=true、vx=0；停止下開火 accurate=true、residualSpeed≈0；移動中開火 accurate=false、residualSpeed=|vx|。
- [ ] **回歸**：WP-2/WP-3 決定性測試（納入 movement + 急停）仍綠。
- [ ] 手動驗：移動 → 反向鍵急停 → 停止瞬間開火被標精準。
- [ ] `vitest run` + `tsc` 綠燈。

## Definition of Done
- [ ] 反向鍵立即停止；停止 gate 正確標記開火精準 + residualSpeed；決定性回歸綠。
