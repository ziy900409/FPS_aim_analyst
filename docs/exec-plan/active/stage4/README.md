...
| **WP-31** | [wp-31-advanced-diagnostics/](wp-31-advanced-diagnostics/README.md) | 進階診斷:SPARC + Key-Velocity xcorr(reliability gate)+ Fitts | P2 | — | **M14 ✅** + WP-30 ✅ | 2–3 → **2.5–3.25**(D-31.0) | 🟡 **進行中**:T0 ✅(2026-08-10)—— `gate-v1` 三件組凍結(含 seed,OQ-S4-3 關閉)+ `sparc-v1`/`xcorr-v1`/`fitts-v1` pre-registration + fixture roster 沿用與 strict 閘覆核;T1/T2/T3/T-exit ⬜ |
...
| **T1 SPARC** | `compute_sparc` **逐位移植**(常數以 `Final` 釘死);逐 **`phase-v1` MR 區間**(非整條軌跡);跨 repo golden 對表 + **N=32/64 階梯診斷** | golden 對表 ≤1e-9(含 `max_v`/`n_fft`/`max_mag`/`f_span`/`arc` 等中間值)綠;有效列數與 `phase-v1` 非退化 MR 數一致;真實資料分佈報告;退化輸入(<16 樣本)回傳語意與 performance_analysis 一致;階梯診斷給 `comparable` / `stratified_only` 判定 | Med |
| **T2 Key-Velocity xcorr** | signed A/D state(`ticks[].keys`)vs ω(t);逐 peek peak lag/strength + correlogram;**`gate-v1` 三件組**(circular-shift null / bootstrap CI / 奇偶半分,seeded) | 單元測試綠(含已知相位差合成訊號 → 已知 lag;無耦合輸入 → p 不顯著);gate 報告產出並給**明確判定**(`research_only` / `blocked-by-data`;`coach_report` 由程式碼保證不可達);同輸入同判定(seed 固定)| Med |
| **T3 Fitts** | D = spawn 偏心角(`visible.targetX/Y/Z` + aim@spawn)、W = 目標角尺寸(`meta.targets.hitbox`,GD-7 單一來源);MT = t_firstShot − t_visible;逐 peek ID/MT → TP + 回歸診斷 | 已知幾何 fixture 綠;r² / TP 報告;D 變異不足 → 輸出 `blocked-by-data`(不硬給結論);**D 內生性與 MT 含 RT 兩項限制**逐字進 `fitts-v1` 與報告 | Low |
| **T-exit** | 三指標效度判定收斂 + `analysis-advanced-diagnostics.md` 定稿 | 每指標一份「進教練報告 / 研究向 / blocked-by-data」判定 + 證據;通過者併入報告 v2(**三個全判研究向也是合格交付**,C-D3) | — |
...
| **OQ-S4-18**(新) | SPARC 在 128Hz 的 N=32/64 零填充階梯,是否大到讓「跨段長比較」不成立 | 🟡 **T1 以資料判定**。實測逐 peek `primary_flick` 段長中位數 32 tick 恰在 N=32/64 邊界(L≤32 → df=4Hz/6 bins;L≥33 → df=2Hz/11 bins)。T0 已 pre-register 跳幅門檻 `step_ratio_threshold = 0.5`(兩 bucket 中位 SPARC 差 ÷ bucket 內 IQR 較大者);超標 → SPARC 僅限同 `padded_n` bucket 內分層比較並在報告標限制。**不得**為此改 `FC_HZ`/`AMP_THRESH`/padding 規則(會讓 FR-D13 的跨 repo golden 對表退化成自我對表) | 研究者 | WP-31 T1 | SPARC 的分佈報告能否跨段長解讀 |
| **OQ-S4-19**(新) | Fitts 的 D 為內生(玩家上一 peek 結束時準星停在哪),回歸結果能否作為 TP 的個人基線 | 🟡 **T3 交付數值 + 限制,不作因果主張**。目標只在兩個固定位置出現,D 的變異幾乎全來自前一 peek 的過衝/修正 → 相關性觀察,非 Fitts 典範的受控設計;且 `MT = t_firstShot − t_visible` 含 RT 與 counter-strafe 停止時間,截距 `a` 會吸收兩者(`t_detect` 僅 5–9/20 peek 有值,不足以逐 peek 扣除)。升級為受控設計(drill 端隨機化 spawn 偏心)屬新 WP/新錄製,不在本 stage | 研究者 | pilot 後 | TP 的解讀範圍;不阻塞 T3 交付 |
