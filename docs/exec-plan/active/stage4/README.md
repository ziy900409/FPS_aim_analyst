...
| **WP-32** | [wp-32-dashboard-integration/](wp-32-dashboard-integration/README.md) | 晉升整合:golden parity → TS metrics + 結果頁擴充 + 驗收清單 D | — | **M15** | WP-29 ✅ + WP-30 ✅ + **WP-31 T-exit ✅**([D-32.1](wp-32-dashboard-integration/progress.md)) | 2–3 → **4.5–5.75**([D-32.0](wp-32-dashboard-integration/progress.md)) | 🟡 **T0–T5 完成(2026-08-17)**:T0 晉升清單與移植紀律凍結;T1 ω+SG、T2 seg-v2、T3 phase/sync、T4 curve 均以 golden 對表進 `test:ci`;**T5 結果頁 research-promoted 區塊完成**(封閉 8 個 metric id、n/flags/version、blocked 態、ω/ε inline SVG、同一 `ExportPayload` 派生 stats)且 `test:ci` / `uv run pytest` 雙閘綠。剩 T-exit(M15) |
