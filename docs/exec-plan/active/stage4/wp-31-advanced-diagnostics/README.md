...
| **狀態** | ✅ **完成(2026-08-12)** — T0 ✅(2026-08-10:`gate-v1` 三件組凍結含 seed、`sparc-v1`/`xcorr-v1`/`fitts-v1` pre-registration、fixture roster 沿用與 strict 閘覆核、OQ-S4-3 關閉);**T1 ✅**(2026-08-10:`sparc-v1` 逐位移植 + 兩份跨 repo golden、59/60 段來源一致、階梯 verdict **`stratified_only`**、OQ-S4-18 關閉);**T2 ✅**(2026-08-10:`xcorr-v1` + `gate-v1` 逐 session 判定 → **三 session 全 `research_only`**,09:18/09:37 未過 ① shuffle null、09:24 三件全過;`coach_report` 由 AST 掃描證明不可達;`key` 事件交叉檢核 248/248 全對;新開 **OQ-S4-20**);**T3 ✅**(2026-08-12:`fitts-v1` D/W/ID/MT/TP + `blocked-by-data`;09:18 因 `d_ratio=1.8343 < 2.0` blocked,09:24/09:37 `ok` 但 r² 低;限制已入 registry/artifact);**T-exit ✅**(2026-08-12:三份判定收斂表 + `coach_report.py` 升 `coach-report-v2`(新增研究向區塊 `#advanced` + 缺口說明 `#advanced-gaps`);`analysis-advanced-diagnostics.md` 定稿;9 份 committed 範例重跑;**WP-32 交接清單為空**——三構念在本樣本結構下皆止步研究向/`blocked-by-data`) |
...
pooled 跨度 8.70°–30.72° ≈ **3.5×**,滿足 [../README.md §2.1](../README.md) 對 velocity-scaling / Fitts 的「D 變異跨 ≥ 2 倍」觸發條件。**T3 實測補充(2026-08-12)**:該預期只對 pooled roster 成立;逐 session 判定下 09:18 的 `d_ratio = 1.8343 < 2.0`,因此依 `fitts-v1` 保守判 `blocked-by-data`。09:24 / 09:37 通過資料門檻,但 r² 低(0.0669 / 0.0339)。門檻未放寬,也未為了讓 09:18 通過而跨 session 併池。
...
| **T3** ✅ | [T3-fitts.md](T3-fitts.md) | D/W/ID/MT/TP + ID–MT 回歸 + `blocked-by-data` 判準 + 內生性限制(09:18 blocked;09:24/09:37 ok 但 r² 低) | T0(不依賴 T1/T2) | Low | 0.5–0.75d |
