...
| 2026-08-10 | **T-exit** | ✅ | `coach-report-v1` 以 strict trajectory gate 一次計算 phase/curve rows，報告含 REC/MR/V 主表、REC-end−`t_detect` 研究向摘要、L/R ω(t)/ε(t) inline SVG；三份真實 tick-integral fixture + `synthetic_counterstrafe` 各一鍵產出，legacy 四份 v0 evidence 亦重跑為 v1 並明示不產生 trajectory 數值。新增/擴充 53 個 report 契約測試；9 份 committed HTML byte-deterministic。`analysis-phase-curves.md` 定稿並完成 stage4/exec-plan/CONTEXT 對帳。完整 research 閘 **303 passed**；engine `npm run test:ci` **90 files / 748 tests + 21 Playwright passed**；零 `src/` 變更 |
...
| **D-30.12** | ✅ **T-exit 報告維持 session-local 統計；REC-end−`t_detect` 在單 session `<10` 時標 `session-insufficient`，但明示已登錄的 WP-30 pooled 系統性分歧，不把它降格為 `blocked-by-data` 或主表結論** | `curve-v1` 與 phase 分佈都禁止跨 session 併池；每份一鍵 HTML 也必須遵守該限制。反 vacuous 閘則從 T0 起就是三份 roster 的 pooled n≥10 判定，已由 T2 以 n=21 通過並得出分歧結論。若單份 HTML 把 n=5/8 當成全 WP `blocked-by-data`，會與已交付的 pooled 結論矛盾；若把 pooled 值混入 session 表，又會造成訓練效果/跨 session 併池暗示。故兩層並列：session 值只作描述；pooled verdict 僅作研究向限制文字。Alternatives Considered:①每份報告重算/合併其他 session(拒絕:違反不併池與一份輸入一份報告契約);②省略 pooled finding(拒絕:掩蓋已知系統性分歧，違反 GD-20);③把分歧放主表(拒絕:OQ-S4-17 未解)。 | `coach_report.py::_rec_detect_consistency` / `_render_rec_detect_consistency`;`analysis-phase-curves.md` report carrier contract;T2 pooled evidence |
...
## 8. T-exit 教練報告 v1 + 文件對帳證據

- **一鍵 / deterministic**：`generate_coach_reports.py` 重產 9 份 committed HTML（既有 4 份 v0 evidence 的 v1 rerun、`synthetic_counterstrafe`、三份 2026-08-07 真實 fixture、09:18 `--group-by side`）。`test_repeated_generation_is_byte_identical` 與 committed-artifact 回歸同時通過；單檔只有 inline CSS/SVG、零外部資源。
- **報告 red line**：phase 主表含 REC/MR/V 的 n、excluded flags、`phase-v1` 與新構念限制；兩張 L/R 101 點 SVG 直接消費 `curve_summary` 的 mean/IQR/n/excluded；REC-end−`t_detect` 系統性分歧在研究向 section，非主表、非校正因子。legacy 08:03/09:39 仍可呈現其凍結 timeline/Sync evidence，但 strict trajectory gate 明示拒絕且不產生 phase/curve 值。
- **v0 contract**：既有 timeline/Sync 六項計算與版本不變；必要測試只增加 v1 sections/strict-source/curve-count/group-partition assertions。四份既有 HTML 的差異可完全歸因於 `coach-report-v1` 外殼、新 parameters/limitations、以及 legacy trajectory-unavailable notice，沒有任何 v0 metric 重算。
- **雙閘**：`uv run pytest -q --no-header -p no:cacheprovider --basetemp codex_pytest_tmp_t_exit_full` → **303 passed in 45.34s**；`npm run test:ci` → **90 files / 748 tests passed** + **21 Playwright passed**。`detect-parity.test.ts` 含在 engine 閘內並通過。
- **文件**：`analysis-phase-curves.md` 增 report carrier contract；WP README、stage4 README §3/§6/§9、exec-plan README §2、CONTEXT.md、T-exit checklist 對帳完成。`DECISIONS.md` 已復核：無 `compute.ts` / TS 推導語意分歧，故無新跨 WP decision。
