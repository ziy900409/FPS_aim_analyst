...
- [x] A-65.1～A-65.12 **十二條全數**有具名證據（指令+輸出／檔名+案例名／截圖+數值），無任何一條只寫「已完成」
- [x] A-65.12 的五個實測值（三場乾淨 + 待命期 + ended 期）逐一列出
- [x] 四項最終全量閘的實際輸出已貼入 `progress.md §T-exit`
- [x] 步驟 3 的五條 diff 檢視輸出已貼入（空結果也要貼，證明真的跑過）
- [x] GD-41 本體六項已落入 `docs/exec-plan/DECISIONS.md`，且落帳前的編號重查結果已記錄
- [x] 效度斷代已記錄，且使用者已於 2026-09-11 確認**需要** `meta` 版本標記；依 scope 另立 WP 處理
- [x] `task-checklist.md` 全 ✅；[stage13 README §2](../README.md) 的 WP-65 列已翻 ✅
- [x] OQ-65.1～65.4 四條全數結案（關閉或明確降級為非阻塞觀察項）
