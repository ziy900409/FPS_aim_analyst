...
| 🟡 | **T3** 可分性消融 | [T3-separability-ablation.md](T3-separability-ablation.md) | T2 🟡（資料充分性閘未過） | High |
...
- [x] 評估契約在**看資料之前**凍結，且自 T0 起未被改動 —— 以段落逐位 diff + sha256 覆核（T3 step 1；`git log -p` 單獨不足，見 progress.md §T3 Step 1）。
- [ ] cohort 的每份 run 有可用性與標註完整性的**實際數字**；作廢者具名，作廢規則來自 T0 而非事後。<br>　（稽核程式與 operator 入口已完成並以對照 fixture 驗證；等真人 run。）
- [ ] 四層消融 × 每個 θ 的混淆矩陣、指標與逐層增益齊全，且校準集與 held-out 兩組都已報告。<br>　（消融程式、報表與 operator 入口已完成並以合成對照 fixture 驗證；**數字本身**等真人 run。）
