...
2. 先寫 source/contract 紅測試：每個 curated item 必須有 runtime entry，entry `source === config`、`sceneId === 'field-low'`；未選中 Pilot id 不得出現。
3. 從 curated registry 映射 `availableDrills` entries。不得經 `resolveTrackingPilotBlockConfig()`，不得 clone/config mutate，weapon override 仍由 SessionRunner/WP-62 既有鏈路處理。
4. 依 OQ-64.2：預設為 `showInResearcherControls:false`；Controls projection 過濾該欄，`loadDrillById()` 仍搜尋完整 registry。既有 entries 缺席欄位時行為逐位相同。
