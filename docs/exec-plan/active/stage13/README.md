...
| **WP-66** | [`wp-66-target-hit-visual-feedback/`](wp-66-target-hit-visual-feedback/README.md) | 命中視覺回饋：sim 在既有命中判定點多寫一筆 `targetHits` 環形格，`TargetView` 唯讀據此讓被命中的目標亮起（命中才亮、沒中不亮）；config-gated，只在指名的 tracking drill 啟用 | T-exit（A-66.1～A-66.12） | 無（與 WP-60～65 全數正交、可並行） | 6–8.5 | 🟡 **T1–T3 ✅；T4 部分完成 8/10（2026-09-12）**。⚠️ **效度斷代已生效**：自 2026-09-12 起，`tracking_br_v1` 家族**八個 variant 全數**帶命中視覺回饋（`brTrackingProtocol` 的全部八個 condition）——此日期之後收的資料刺激與之前**不同，兩者不可混池比較**；逐 run 的判別依據是匯出的 `meta.targets.hitFeedback === 'flash'`，不是收集日期。回饋對八格**逐格相同**，故 ads × ballistic × angularHeight 的協定內比較不受污染（見 [progress §T4](wp-66-target-hit-visual-feedback/progress.md#t4--在指名的-tracking-drill-啟用命中回饋2026-09-12)）。⚠️ **T0 清單十個 id 中的兩個尚未啟用**（`tracking_core_pr_pilot_v1_2deg_5dps`／`tracking_reversal_pilot_v1_high`）：T4 執行期發現兩者是 `tracking-pilot-v2` 已版本化協定六個 scored block 中的兩個，啟用將造成協定內 2/6 混淆，且 `checkTrackingCompatibility()` 無 `hitFeedback` 軸、前後無法分池（與 KI-025 同型）⇒ 停手待裁決（OQ-66.5）。規劃 2026-09-11 完成；T0 結果：編號重查確認 WP-66 / GD-42 未被平行 session 取用；四個 OQ 全數照預設收斂，**啟用清單十個 `drillId` 已逐字定案**（見 WP README §1.4a）；假設 #3 證實 `schema.ts` 為白名單重組 ⇒ T3 必須同時改 schema。基線凍結於 `9a03562`（Vitest 3128 passed / regression 319 passed / Playwright 112 passed 0 failed）。核心判斷：tracking 目標是 `persistent: true`（命中不撤除）⇒ 受試者在整個呈現窗內**看不出自己有沒有打中**；而 `impacts` 彈著格**脫靶也寫**，補不上這個缺口。解法是 WP-25 tracer 雙軌分離的第三個實例（`impacts` → `shotRays` → `targetHits`）：**不新增任何命中判定**（GD-7 單一來源），只消費既有 `fire.hit` / `hit` 的成立條件，速度閘與 occlusion 免費繼承。環形格**刻意不帶時間戳**，衰減一律以 render 的 rAF `now` 起算，從結構上消除 sim clock ↔ wall clock 相減。⚠️ **最高風險不在程式而在效度斷代**：改視覺＝改刺激，啟用前後的資料不可混池，故啟用清單須逐字列名、預設排除已凍結的 `hold_track_v1`（`protocolVersion = 1.0.0`，GD-23），並由 `meta.targets.hitFeedback` 逐 run 自述。replay **先不同步**（使用者 2026-09-11 決定）。決策草稿 GD-42（T-exit 入帳）。⚠️ **主題不屬本 stage**（render 視覺回饋／受試者體驗層，非原始輸入取樣）；依使用者 2026-09-11 指示落於此處——承 WP-62／63／64／65 先例，見下方 §3。 |
