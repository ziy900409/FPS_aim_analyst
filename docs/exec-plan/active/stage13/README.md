...
| **WP-62** | [`wp-62-session-plan-per-item-weapon/`](wp-62-session-plan-per-item-weapon/README.md) | Session Plan 自訂 program 的每一列可事先指定武器；選擇進匯出稽核 | T-exit | WP-58 T-exit ✅（只讀不改 frozen 軌） | 8.5 | 🟡 **T0 ✅ / T1 ✅ / T2 ✅ 2026-09-10；T3 未開工**（T0 基線 Vitest 2,822 passed／Playwright 103 passed；T2 後 Vitest 2,896 passed，typecheck ×2／build exit 0；Playwright 留到 T6）。決策 [GD-38](../../DECISIONS.md)。⚠️ **主題不屬本 stage**（session 排程層，非原始輸入取樣），依使用者 2026-09-10 指示落於此處——見下方 §3。 |
