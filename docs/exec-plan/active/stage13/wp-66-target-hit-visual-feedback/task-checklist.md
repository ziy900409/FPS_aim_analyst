...
| ✅ | **T-exit** WP-66 驗收（A-66.1～A-66.12 十二條全數成立）+ **GD-42 已入帳**；一項判準具名改寫（A-66.12 draw call → 「分布無系統性差異 + `poolSize` 不變」），兩項實機證據明帳以單元層證據替代 | [T-exit-gate.md](T-exit-gate.md) | T1–T5 | Low |
...
- [x] 受試者情境「在指名的 tracking drill 開火 → 命中當下目標亮起 → 停火或打偏後熄滅」在實機可完整走通，並在換 drill／換場景兩條路徑行為一致
- [x] 未指名的 drill **逐位不變**：材質三屬性、`meta` 鍵集合、未涉 `meta` 鍵面的 golden fixture 三者皆與本 WP 前相同
- [x] 命中回饋消費的是**既有**命中判定：`HitDetector`／`ballisticRaycast`／`targetAabb`／`sweptHitTest` 的 diff 為空，`SimLoop` 只多兩行寫入
- [x] 脫靶／隔牆／未過速度閘／逾射程一律**不亮**，且各有反證測試
- [x] 命中態跟 `TargetState.id` 走，不跟 mesh pool 槽位走（槽位洩漏專測綠）
- [x] `TargetView` 對 `SharedState` 零寫入（`sync()` 前後 ring 全欄位 `Object.is` 不變）
- [x] 環形格內容在 `src/data/`、`src/metrics/`、`research/` **零 importer**，且由常駐測試守著
- [x] 同一輸入序列跨 ≥ 4 種 render FPS，環形格 `total`／`cursor`／逐槽 `id`/`seq` 逐位一致
- [x] 啟用清單逐字等於 T0 收斂結果；`hold_track_v1` 與所有 formal assessment 協定未被啟用、`protocolVersion` 零變更
- [x] 效度斷代在 `progress.md`、stage13 README、GD-42 三處留下同一句可稽核摘要
- [x] A/B frame-time p95 增量 ≤ 0.2 ms（實測 **+0.025 ms**）、over-budget window 不增（ON 89.7–90.8 % 落在 OFF 68.5–91.0 % 內）、draw call **判準於 T-exit 具名改寫**為「ON/OFF 分布無系統性差異 + `poolSize` 不變」——`drawCalls` 在 br-field 逐幀抖動 114–118、ON/OFF 範圍互相覆蓋，「逐位相同」不是可量測的判準（見 [progress §T-exit.3](progress.md)）
- [x] `npm run typecheck` ×2、`npx vitest run`、`npx playwright test --workers=1`、`npm run build` 四項皆 exit 0
