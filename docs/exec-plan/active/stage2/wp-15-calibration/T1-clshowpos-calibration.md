...
| | |
|---|---|
| **相依** | T0(容差 + 資料就緒)、WP-14 exit(integrator 上線) |
| **Risk / Cplx** | Med / Med |
...
以 CS2 實錄 `cl_showpos` 速度序列為真值,把 WP-14 integrator 的起步與急停曲線
逐 tick 對表(64Hz 對齊),容差 = OQ-S2-2 決議值(FR-B13 前半)。

## In scope
- 參考序列數位化:錄製檔 → `{tick, vel}[]` fixture(起步 0→max、急停 max→0 各一段;
  JSON meta 欄附來源、tickrate、錄製條件)。
...
- [ ] fixture 數位化 + meta 註記(來源、tickrate、標定方法)。
