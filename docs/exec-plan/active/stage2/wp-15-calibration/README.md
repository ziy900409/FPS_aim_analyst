...
| **對應 FR** | FR-B13 |
| **估時** | 1.5–2 dev-days |
| **狀態** | 🛑 T0 STOP(2026-07-07):AK pattern 候選 fixture 已入 repo,`cl_showpos` 仍缺 |

---

## 1. 範圍

**In scope**(測試與 fixture 為主,預期不動 `src/`):
...
|---|---|---|---|---|
| **T0** | [T0-entry-gate.md](T0-entry-gate.md) | OQ-S2-2 容差拍板 + **參考資料備妥檢查(STOP 條件)** | — | Low(STOP:`cl_showpos` 未備) |
