...
| **Status** | 🟡 **T4 完成（2026-08-27）**：研究設計 owner 交付 `docs/algorithm/spider_shot/spider-shot-v2-performance-metrics-design-2026-08-27.html`，解除 T4 entry gate 的殘留 OQ-49.1 子項。新增 `src/history/DrillMetricRegistry.ts`（exact-id `spider-shot-v2` registry：5 個 metric descriptor、guard 順序 unregistered-drill → not-assessment → quality gate → compatibilityKey → 逐 observation finite/unit 驗證；首發命中率直接驗證 `firstFire` 實際 hit outcome，修正設計文件記載的既有 `firstShot.hit` window-outcome 誤判風險，見 D-49.P28）、`server/history/HistoryAnalysisService.ts`（cursor 分頁、limit≤100、全域 concurrency=4、in-flight coalescing、success-only cache）、`src/history/HistoryTrend.ts`（cohort/eligibility/metric selection 純 domain，latest-eligible cohort 預設 + selector，D-49.P10 落地）。`server/history/historyApi.ts`／`src/history/HistoryClient.ts` 新增 `observations` route/method（README §2.6 契約）。`npm run typecheck`/`vitest run`/`npm run build`/`playwright test` 全綠（158 test files / 1388 tests + 2 skipped，較 T3 末新增 29 tests；46 e2e 零回歸）。NFR-49.2 perf 證據：5,000-run fixture 下 cold 100-observation page 55.8ms（gate<2000ms）、warm 0.6ms（gate<300ms）、全量 50 頁掃描 1519.6ms。詳見 [progress.md](progress.md) D-49.P28～P32。T3 完成狀態見 D-49.P23～P27；T2 見 D-49.P20～P22；T1 見 D-49.P14～P19。T5 可開始 |
