...
| **Status** | 🟡 **T3 完成（2026-08-27）**：新增 `src/results/ResultPresentation.ts`（payload→presentation 純 domain，取代 `main.ts` 的 `diagnosisForPayload`/`qualityFlagsForPayload`/`snapshotFromExportPayload`/`exportBasename` 等）與 `src/ui/ResultDetailBody.ts`（唯讀 metrics/diagnosis/quality-flags 共用 body，從 `ResultScreen.ts` 抽出）；current `ResultScreen` 與新 `ui/history/HistoricalRunDetail.ts` 共用同一個 `createResultDetailBody()`（D-49.P4 落地）。`HistoryLibraryController` 的 `runDetail` 從 T1 佔位 `ExportPayload` 換成真正 `HistoricalRunPresentation`（`run`/`payload`/`result`，`run` 由 payload 純函式反推、無需額外 HTTP round-trip）。新增 `ui/history/DrillOverview.ts`（drill route 的 run list 部分，`all/trend-eligible/excluded` filter UI 已就位，後兩者顯示 pending-analysis 佔位訊息，等 T4 metric registry）。依 OQ-49.3/D-49.P11 執行：`main.ts` 移除 `currentHistorySession`/`historyView`/`sessionSummaryFromPayload` 人工 picker 呼叫鏈，刪除 `src/ui/HistoryView.ts`／`src/data/sessionHistoryLoader.ts`（+test）；`src/metrics/sessionHistory.ts`（`buildSessionHistory`/`SessionSummary`）保留不動（blast-radius 記錄明確保留意圖）。`npm run test:ci` 全綠（155 test files / 1359 tests + 2 skipped（vitest，較 T2 末新增 28 tests）+ 46 Playwright e2e，含 2 個新 T3 case：兩個不同 run 的 JSON 下載內容可區分、historical detail 無 restart/save 動作）。詳見 [progress.md](progress.md) D-49.P23～P27。T2 完成狀態見 D-49.P20～P22；T1 見 D-49.P14～P19。T0 決議：OQ-49.1～5 已與使用者收斂：僅註冊 `spider-shot-v2`（peek-click-transfer 需另立跨 WP 決定；`spider-shot-v2` metric descriptor 待 T4 前由研究設計 owner 定義）、latest-eligible cohort 預設+selector、cursor 分頁漸進趨勢、Participant 頁只顯示分類 count。T4 可開始 |
