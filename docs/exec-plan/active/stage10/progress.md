...
- **2026-08-31**：**WP-51 T-exit automated gates 通過（working tree based on HEAD `d142baf`）**——補齊 T2～T4/T-exit 自動化證據並重跑：`npm run typecheck`、`npm run build`、`npm run test`（186 passed + 1 skipped files；1654 passed + 2 skipped tests）、`npm run test:e2e`（71/71）、`npm run test:ci`、`npm run test:stage10`（39.0s）、`RUN_STAGE10_SCALE_BENCHMARK=1 npm run test:stage10:scale`（History first-100 P95 244ms；100-run analysis cold/warm 269/267ms；42k Replay cached-reopen P95 480ms）與 critical repeat bundle（100/100，`--repeat-each=5 --retries=0`）皆 exit 0。修正僅限 E2E harness 穩定性：`stage10-accessibility.spec.ts` 避免 shared dev root future-fixture 排序干擾並改用 frame-aware keyboard assertions；`replay.spec.ts` A-50.5 以 `expect.poll` 等待 seek value commit；`history-library.spec.ts` 清空搜尋後改為尊重 100-row chunked list，避免 shared dev root 內 Stage10 future fixtures 把新 seed participant 擠出首屏造成假陰性。`graphify update .` 已於 test code 變更後執行。**M18 仍未宣告**：KI-017/KI-018 尚待 WP-50/WP-49 owner 收斂，manual browser/GPU/a11y walkthrough 與 independent operator runbook walkthrough 尚未由真人簽核，Chrome/WebGL2 fallback coverage gap 仍需補齊或正式豁免。
- **2026-09-01**：**KI-017 已由 WP-50 owner 修復**——`src/main.ts` 將 `replayController` 改為早期 `let ReplayController | undefined` 綁定，History Run Detail `onReplay` 在 controller 尚未初始化時顯示「Replay 尚未就緒，請稍後再試。」而非拋 TDZ `ReferenceError` 或 silent no-op；新增 `tests/e2e/replay.spec.ts` KI-017 真瀏覽器 regression，不等待 `window.__fpsTest` readiness、捕捉 `pageerror` 並斷言 early click 有明確結果。驗證：`npm.cmd run typecheck` exit 0；`npx.cmd playwright test tests/e2e/replay.spec.ts --project=edge -g "KI-017"` 1 passed；`npx.cmd playwright test tests/e2e/replay.spec.ts tests/e2e/stage10-assessment.spec.ts tests/e2e/stage10-preview.spec.ts --project=edge` 12 passed；`npm.cmd run test:e2e` 73/73 passed。**當時 M18 仍未宣告**：KI-018、manual browser/GPU/a11y walkthrough、independent operator runbook walkthrough 與 Chrome/WebGL2 coverage gap 仍未完成（Chrome/WebGL2 後續已由 OQ-51.2 waiver 改為 non-blocking，見下一筆）。
- **2026-09-01**：**OQ-51.2 Chrome/WebGL2 coverage gap 正式豁免為 non-blocking post-M18 debt**——依使用者/product-tech owner 決策，M18 prototype release 不再要求 Chrome manual walkthrough 或 WebGL2 fallback automated project；release gate 維持系統 Edge 自動化 + Edge manual WebGPU/GPU/a11y walkthrough。理由已寫入 [docs/operational/acceptance-stage-j.md](../../../operational/acceptance-stage-j.md) §3.1：M18 是本機 History + 3D Replay prototype 整合里程碑，不是跨瀏覽器 renderer certification；Chrome/WebGL2 coverage 仍保留為 post-M18 debt，且若 scope 升級為正式跨瀏覽器發布，此 waiver 失效。**M18 仍未宣告**：KI-018、manual Edge/GPU/a11y walkthrough 與 independent operator runbook walkthrough 仍未完成。
