...
- **2026-08-31**：完成 **T6 — Current Result／Historical Run Entry and Return**（5 個垂直切片：`ReplayPresentationSession` onFrame callback／`ReplayController.ts` 新檔＋單元測試／`ResultScreen.ts` replay entry／`HistoricalRunDetail.ts`＋`HistoryScreen.ts` replay entry wiring／`main.ts` 最終整合，各自獨立 commit + 全綠驗證）。詳見下方 T6 Evidence Log 與 Decision Log D-50-P28～P33。
...
## T6 — Current Result／Historical Run Entry and Return（2026-08-31）

Entry gates 重新確認：WP-48 README status ✅ T0～T5＋T-exit 全部完成（2026-08-27）；WP-49 README status ✅ T-exit 完成（2026-08-28）。以 CodeGraph 重新讀當時實際 `ResultScreen.ts`／`HistoricalRunDetail.ts`／`HistoryScreen.ts`／`HistoryClient.ts`／`main.ts` 原始檔（非猜測 approved-contract 文字），確認 `HistoryClient.loadRun(runId, signal?)` 已原生支援 `AbortSignal`、`HistoricalRunDetail.ts` 已有型別化 `onReplay?: (runId) => void` port（T3 佔位版恆 `disabled=true`）、`ResultScreen.ts` 尚無任何 replay 程式碼——與 T0 discovery 記錄一致，無 concurrent drift。

五個垂直切片、各自獨立 commit + 全綠驗證：

1. `feat(replay): expose per-frame sample/playback from ReplayPresentationSession`（`e73a9e1`）——`ReplayPresentationSessionDeps` 新增選填 `onFrame?: (sample, playback) => void`，在既有 `frame()` 內 `render()` 之後呼叫一次；省略時逐位相容 T3/T4 既有呼叫方式與測試。
2. `feat(replay): add ReplayController orchestrating load/generation/session lifecycle`（`d9df1b6`，稍後在 main.ts 整合切片內修正一處排序見 D-50-P29）——`src/replay/ReplayController.ts` 新檔：把 `ReplaySource`（當次記憶體 payload 或 historical `runId`）轉成 `state`＋`subscribe()` 的小型狀態機（比照 `HistoryLibraryController` 慣例，不 import 任何 DOM/Three），scene/session 建構與 canvas ownership 全部透過注入的 `createSession`/`mountViewport`/`unmountViewport` callback 交給 `main.ts`。11 個單元測試（fakes，非真實 renderer）涵蓋 current／historical／API 失敗／unknown drill／close 期間 abort／rapid A→B switch／scene resolution fallback／retry。
3. `feat(replay): add current-Result 3D Replay entry to ResultScreen`（`c1d0498`）——`ResultScreenOptions.onReplay?: () => void` + 恆定可見（不像 History 入口要等 `setHistoryTarget`）的「3D 重播」按鈕；full/partial/unsupported 分類留給 Replay screen 自己在進入後判斷（README UI flow：Result → Replay → Loading → 分流）。
4. `feat(replay): wire the historical Run Detail replay entry (FR-49.13)`（`1b9f6c7`）——把 WP-49 T3 留下的佔位按鈕（恆 `disabled`、文字「尚未提供」）改成真正可用：文字「3D 重播」、只在 run 為 `ready` 時可點（比照 export 按鈕的 `setActionsEnabled` 慣例），並傳遞當前 `runId`；`HistoryScreen.ts` 新增 `onReplay?` 選項透傳給 `HistoricalRunDetail`，讓 `HistoryScreen` 本身不需要認得 `ReplayController`。
5. `feat(replay): wire current/historical replay entries into main.ts`（`c73da99`）——`main.ts` 整合：`PresentationCoordinator` 建構挪到檔案前段（`resize()` 呼叫點之前），讓 `resize()` 能安全讀 `presentation.mode` 並在 replay 期間跳過（見 D-50-P30，回應 T3 D-50-P19 留下的 open follow-up）；新增 `mountReplayViewport`/`resizeReplayViewport`/`unmountReplayViewport` 把共用 canvas 搬進/搬出 `ReplayScreen.viewportElement`（沿用 `applyResolutionMode` 固定解析度分支的 `renderer.setSize(w,h,false)` 慣例，CSS 尺寸留給自己的 `100%/100%`，不被 three.js 蓋成絕對像素）；canvas click handler 新增 `replayScreenHandle?.visible` 擋一層（見 D-50-P31）；`resultScreen`/`historyScreenHandle` 的 `onReplay` 分別接到 `replayController.open({kind:'current',...})`／`replayController.open({kind:'historical',...})`；`showResultAndTrackHistory()`（含 dev-only `__fpsTest.showResult()` E2E hook）新增 `lastResultPayload` 賦值供 current-replay 使用。

`npm run build` / `npm run typecheck` / `npx vitest run`（**185 test files / 1639 tests，1637 passed + 2 skipped，0 failed**）全綠。

### Decision Log（T6）

- **D-50-P28 / ReplayController 走 state+subscribe，不走 callback-per-transition**：一開始的草案是讓 `ReplayController` 直接持有 `ReplayScreenHandle` 並在內部呼叫 `screen.render(...)`，但這會讓 `src/replay/` 這個「WP-50 README §2.1 domain 檔案清單」內的新檔反過來 import `src/ui/replay/ReplayScreen.ts`（UI 層），架構方向錯誤，也讓 controller 無法脫離 DOM 單獨單元測試。改為比照既有 `HistoryLibraryController`（`state` getter + `subscribe(listener)`，零 DOM import）的成熟慣例：`ReplayController` 只發布一個 `ReplayControllerState`（`idle/loading/error/unsupported/ready`），由 `main.ts`（唯一知道 `ReplayScreenHandle` 存在的組合層）訂閱後自行映射成 `ReplayScreenState` 並呼叫 `show()`/`hide()`/`render()`。11 個單元測試因此完全不需要任何 DOM stub。
- **D-50-P29 / `run()` 必須先 `setState('ready')` 才能呼叫 `mountViewport()`，順序不可顛倒**：實作時發現一個真實的量測時序 bug——`ReplayScreen` 的 `viewportHost` 活在 `readyPanel` 底下，只有 `render({kind:'ready',...})` 被呼叫後才會從 `display:none` 變成有實際版面的box；若 `mountViewport()`（內部呼叫 `getBoundingClientRect()` 決定 replay viewport 尺寸）在 `setState('ready')` 之前執行，量到的永遠是「還沒展開」的祖先元素的 0×0。因為 `setState` 內部同步呼叫所有 subscriber（`main.ts` 的 `replayScreenHandle.render(...)` 也在其中同步執行完），只要把 `deps.mountViewport()` 移到 `setState({kind:'ready',...})` **之後**，`mountViewport()` 執行當下 DOM 已經是展開後的真實版面，`getBoundingClientRect()` 才量得到正確尺寸。這個修正在 commit `d9df1b6` 之後、main.ts 整合前就已經做掉並補了測試互不影響（既有 11 個 unit test 只斷言「`mountViewport` 有沒有被呼叫」，不斷言呼叫順序，故不需要改測試本身，但這個順序若沒修正只有接上真實 DOM 才會炸——是這次撰寫 main.ts wiring 時往回發現、往回修的教訓）。
- **D-50-P30 / `PresentationCoordinator` 建構從檔案尾端挪到 `resize()` 定義之前，解決 D-50-P19 留下的 open follow-up**：T3 progress.md 的 D-50-P19 決定「`resize()` 暫不路由到 `presentation`，避免 `presentation` 建構在檔案尾端、但 `resize()` 在檔案前段就已第一次被呼叫」的 TDZ 風險，把這個決定明確標成「T6 建立真正的 replay entry point 時再解」。T6 動工後發現：`liveFrame` 是**模組頂層 `function` 宣告**（會 hoist，不是 `const liveFrame = () => {}`），所以 `createPresentationCoordinator({frame: liveFrame, ...})` 這個呼叫本身只需要 `liveFrame`「這個名字存在」（hoist 保證成立），完全不需要它已經執行到那一行——`liveFrame` 的函式體要等到第一個 rAF frame 才會真的跑,那時全檔案早已求值完畢。因此把 `const presentation = createPresentationCoordinator(...)` 整個搬到 `resize()` 定義之前（`sceneManager`/`renderer` 此時也早已存在），`resize()` 就能直接安全讀 `presentation.mode === 'replay'` 而跳過 live canvas 尺寸重算,不需要任何 KI-013 式的「先宣告 `undefined`、稍後賦值」繞路。
- **D-50-P31 / canvas click handler 需要獨立擋 `replayScreenHandle?.visible`，不能只靠既有的 `historyScreenHandle` 檢查**：README FM-49.10 原本的防禦註解假設「overlay 的 `pointer-events:auto` backdrop 已經讓 canvas click 收不到事件，這條只是防禦性二次防線」——這個假設對 `HistoryScreen`（canvas 一直留在 `document.body`，被更高 z-index 的全螢幕 backdrop 完全蓋住）成立，但對 Replay **不成立**：canvas 在進入 replay 後被 `mountReplayViewport()` 直接搬進 `ReplayScreen` 的 `viewportHost`（`ReplayScreen.ts` 本來就設計成讓呼叫端把畫布掛進這個空 `<div>`），使用者點擊 3D 畫面本身就是直接點在 canvas 上，沒有任何 backdrop 在前面擋。若沒有這條 `replayScreenHandle?.visible` 檢查，使用者點擊 replay 畫面會意外觸發 `pointerLock.request()`，違反 FR-50.11／NFR-50.5「replay 期間 live Pointer Lock request 必須為 0」的可驗證不變量。
- **D-50-P32 / `resolveSessionScene` 對「recorded sceneId 未安裝」與「assetPackVersion 不符」都只降 partial，從不強制 unsupported**：直接落地 D-50-P11 的既有決定（`minimumPlayable` 只含 `['camera']`，`scene` 永遠不在其中）——`ReplayController` 遇到 `resolveReplaySceneConfig` 回傳 `undefined`（sceneId 完全找不到）時退回 `fallbackSceneConfig`（注入的 `placeholderRoom`）並把 `support` 降級（`full→partial`，補 `SCENE_LOAD_FAILED` reason code、`scene` 移入 `missing`）；`versionMismatch===true` 時維持用已安裝的那個場景、只補 `SCENE_ASSET_VERSION_MISMATCH` reason code。兩種情況都不阻擋播放，只在 partial banner 誠實列出差異。
- **D-50-P33 / `ReplayScreen` 的 `onCancelLoad` 選填 port 這次刻意不接**：`ReplayScreen.ts` 的既有註解已載明「取消按鈕永遠先呼叫 `onCancelLoad()` 再呼叫 `onBack()`」，而 `ReplayController.close()` 本身已經完整涵蓋「abort 進行中的 `historyClient.loadRun`＋拆掉 session／viewport＋回到 idle」——把 `onBack` 與 `onCancelLoad` 都接到同一個 `close()` 只會讓它被連續呼叫兩次（雖然驗證過是 idempotent、不會出錯，但沒有任何額外效益）。故只接 `onBack`，讓語意單純一點；`close()` 的 idempotency 仍然保留（防的是 History/Result 兩個入口以外，未來任何其他呼叫端多按一次 Back 的情境）。

### Surprises & Discoveries（T6）

- **`showResultAndTrackHistory()` 不是 `resultScreen.show()` 的唯一呼叫者**：撰寫 `lastResultPayload` 賦值時發現 `main.ts` 另外還有一個 dev-only（`import.meta.env.DEV` 內）`__fpsTest.showResult()` E2E hook 直接呼叫 `resultScreen.show(...)`，繞過 `showResultAndTrackHistory()`。若只在 `showResultAndTrackHistory()` 內賦值 `lastResultPayload`，透過這個 E2E hook 開出來的 Result 會讓「3D 重播」按鈕點了以後 replay 一份過期/undefined 的 payload。已在該 hook 內同步補上 `lastResultPayload = payload`，兩個入口現在共用同一份「目前顯示的是哪個 payload」單一事實來源，沒有重新實作第二套邏輯。
- **`liveFrame` 是 hoisted `function` 宣告這件事，讓 T3 當初標成「留給 T6」的 resize-TDZ 顧慮其實比想像中容易解**：見上方 D-50-P30——一開始以為要沿用 `historyScreenHandle`/`controls` 那套「先宣告 `undefined`、稍後賦值」的 KI-013 繞路,實際檢查後發現只是單純的「宣告順序」問題,直接把 `const presentation = ...` 往前搬即可,不需要引入任何新的防禦模式。

### Evidence Log（T6）

- Baseline（T6 開工前）：`git rev-parse HEAD` 為 T5 收尾的 `feat(replay): add replay transport and HUD` commit（見上方 T5 節）；`git status --short` 乾淨（除本 WP 無關的既有未追蹤 `docs/algorithm/spider_shot/...` mockup 檔）。
- 逐切片驗證：每個 commit 前皆執行對應子集 `npx vitest run` 與 `npm run typecheck`，全綠才 commit；main.ts 整合切片額外執行 `npm run build`（含 `vite build`）確認打包無誤。
- Rapid-switch／abort／unsupported／scene-fallback 證據（`src/replay/ReplayController.test.ts`，11 tests）：以 fake `presentation`/`historyClient`/`createSession`/`mountViewport`/`unmountViewport` 驗證——(a) current payload 直接 loading→ready，零 `historyClient` 呼叫；(b) historical run 透過 `loadRun(runId, AbortSignal)` 正確載入；(c) API 失敗（`HistoryClientError`）與 unknown drillId 分別產生 `error`／`unsupported` 狀態與正確 reason codes；(d) `close()` 呼叫 `abort()` 讓稍後才 resolve 的過期 `loadRun` 結果不會被渲染（stale generation 被丟棄）；(e) A→B rapid switch 時第一個 session 被 `leaveReplay()`／`unmountViewport()` 正確拆除,才進入第二個 `enterReplay()`；(f) sceneId 未安裝／assetPackVersion 不符兩種情境皆降 `partial` 並補對應 reason code、不阻擋播放；(g) `retry()` 重送與上次完全相同的 request。
- 全量證據：`npm run build`（green，既有 chunk-size 警告，屬既存狀態非本次引入）；`npm run typecheck`（`tsc --noEmit` ×2）green；`npx vitest run` 全量 **185 test files / 1639 tests（1637 passed + 2 skipped，0 failed）**——較 T5 收工時的 184 files / 1623 tests（1621 passed + 2 skipped）新增 1 個 test file（`ReplayController.test.ts`）＋既有檔案內新增的斷言（`presentation-session`／`ResultScreen`／`HistoricalRunDetail`／`HistoryScreen`），無任何既有 test 被修改語意或刪除。
- Playwright（`npm run test:e2e`）：本次**未執行**——延續 T0～T5 既有作法；`viewportHost` 量測時序（D-50-P29）、canvas reparent／resize（`mountReplayViewport`/`resizeReplayViewport`/`unmountReplayViewport`）與 Pointer Lock 防護（D-50-P31）目前只在 headless vitest 層級以 fakes／真實 THREE 物件圖（`ReplayController.test.ts`／既有 `presentation-lifecycle.test.ts`）驗證過邏輯正確性,尚未在真實瀏覽器 WebGPU/canvas context 下跑過「Result→3D重播→Back」／「History Run Detail→3D重播→Back」／rapid run switch／resize 的端到端流程。這類真正需要瀏覽器 DOM layout 與 renderer context 的驗收,延續 T0～T5 記錄的既有慣例,留給 **T-exit** 一次補齊（README task table 的 T-exit DoD 本就要求 Playwright／live determinism regressions）。

### Open Questions／Follow-ups for T-exit
