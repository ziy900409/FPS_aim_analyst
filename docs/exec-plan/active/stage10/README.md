...
| **狀態** | 🟡 WP-48 本機歷史儲存、WP-49 History Library/Assessment Trends 與 WP-50 3D Replay 皆已完成 T-exit；WP-51 M18 integration 尚未開始 |
...
| OQ-S10-3（已收斂） | 現有每一種 drill 的 schema v2 是否足夠 full replay？ | 不足以一律full：legacy缺target ID/lifecycle與部分weapon/fire visual。WP-50 T0凍結逐exact drill matrix、T1補additive replay v1（`replayTargetId`＋`meta.replay`），6個official Assessment exact drillId皆已驗證classify為`full`（見WP-50 T-exit progress.md） | WP-50 T0/T1，T-exit 確認全數收斂 |
