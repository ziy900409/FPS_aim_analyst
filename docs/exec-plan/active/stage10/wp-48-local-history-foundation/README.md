...
| **Status** | 🟡 T0～T4 完成(filesystem PoC 綠燈、OQ-48.1／48.2 已凍結;單一 parseExportPayload/canonicalExportJSON 契約落地;`HistoryRepository` 落地含 5,000-run benchmark;Node History API + Vite dev/preview adapter 落地,六 routes 契約測試全綠;typed `HistoryClient` + `HistoryPersistence` state machine 落地,含 generation-race/retry-existing/unhandled-rejection 測試),T5 尚未開始 |
