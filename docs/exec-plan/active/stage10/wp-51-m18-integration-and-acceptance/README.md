...
| **Status** | 🟡 T0/T1 完成（2026-08-28/31）、T2 完成（2026-08-31）、T3 完成（2026-08-31，見 [progress.md](progress.md)）。**T4 大致完成但暫不打勾（2026-08-31）**——新增 4 支 spec（`stage10-accessibility.spec.ts` keyboard-only journey、`stage10-lifecycle-scale.spec.ts` 50-cycle+abort resource lifecycle、`stage10-projection-shape.spec.ts` network response-shape inspection）+ 1 支 opt-in benchmark（`tests/stage10/stage10-scale.perf.ts`，`npm run test:stage10:scale`）+ `tests/stage10/cli.ts` acceptance command wall-time記錄。5 個 T4 DoD行中 4 行綠燈；「42k-tick Replay cached-reopen P95」在本機重複量測介於 884-1927ms、對 1500ms budget 時而通過時而略超標，判斷像本機當下背景負載jitter但未經乾淨環境對照驗證，如實記錄不強行判定通過。過程中發現並記錄兩個真實 upstream 缺陷：[KI-017](../../../known_issue/KI-017-history-replay-tdz-referenceerror-on-early-replay-click.md)（WP-50，Run Detail「3D 重播」過早點擊TDZ ReferenceError，與已修的KI-013同一類根因）與[KI-018](../../../known_issue/KI-018-history-search-keystroke-focus-steal.md)（WP-49，History搜尋欄逐字輸入焦點被搶走），皆已完整診斷+記錄修法方向但不在WP-51授權內修復。全部 71 個既有+新 Playwright spec、186 Vitest files/1654 tests 無 regression。T4 task-checklist暫留⬜（見task-checklist.md與T4-scale-lifecycle-a11y.md DoD明細），待KI-017/KI-018修復與cached-reopen P95重新確認後可收尾。**T5 進行中（2026-08-31）**——新增 [`docs/operational/history-center-replay.md`](../../../operational/history-center-replay.md)（operator runbook，啟動/root/backup/troubleshooting/security 皆對照實際原始碼並以本機 `npm run dev`+`curl /api/history/health` 驗證過措辭）與 [`docs/operational/acceptance-stage-j.md`](../../../operational/acceptance-stage-j.md)（M18 dossier 草案，README §10 全部 11 項條件 + WP-51 FR/NFR 逐項連到 automated/measurement/inspection artifact，無 orphan）。**尚未完成**：manual browser/GPU/a11y walkthrough（本機無 Chrome，且需要真人操作 Pointer Lock/滑鼠/視覺判斷，AI agent 依協議不得代為宣告 pass，dossier §4 留空白 checklist 供人工簽核）、由未撰寫本 WP 主要程式碼者依 runbook 重新走一次的獨立驗證。T5 task-checklist暫留⬜，5 項 DoD 中 3 項完成（runbook、dossier traceability、無真實資料）、2 項待人工執行。 |
