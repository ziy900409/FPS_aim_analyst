...
- WP-55 T6 完成：README §6 新增 M21 evidence ledger，逐項標示 automated / measurement / inspection / manual evidence 或 T-exit/adjacent owner；T-exit 仍維持待開工。
- `docs/operational/analysis-tracking.md` 補強 WP-55 contact artifact contract：coverage/replay/report 只消費 contact artifact；pure tracking 以 exact-hitbox aim-ray `onTarget`、TOT、RMS/median/P95 epsilon 判定；blocked uses closed vocabulary；HP/damage/hit/kill 不是跟隨判定來源；BR/projectile 只作 companion evidence。
- T6 verification：focused contact/report/replay suite 5 files / 40 tests passed；contact + legacy tracking/BR baseline 14 files / 81 tests passed；`npm.cmd run typecheck` exit 0；full `npm.cmd test` 210 files / 2021 tests passed（1 file / 2 tests skipped）。Full suite 只作 broad regression smoke，不作 WP-54 pilot gate evidence。
- T6 未修改 production code；`graphify update .` skipped，`graphify-out` 未 staged。WP-54 active files 未修改、未 staged。

### 2026-09-03 — WP-55 T5 report and quality integration

- WP-55 T5 完成：新增 `src/metrics/trackingContactReport.ts` 與 `src/metrics/trackingContactReport.test.ts`。Report artifact 只消費 `TrackingContactCoverageReport`、contact artifact samples 與可選 replay trace frame count，不直接讀 raw payload 重新定義 contact。
- Report projection 顯示 acquisition failure rate、per-presentation `tAcquireMs`、pursuit TOT、RMS/median/P95 epsilon、contact timeline；每個 metric cell 帶 `n`、`durationMs`、condition/drill id、`analysisVersion` 與 source id/source run/export basename。
- Blocked/protocol-incompatible runs 以 `tracking-contact-blocked-reason-v1` closed vocabulary 顯示，不帶 summary/timeline，不輸出 fake zero；aggregate condition count 只列 included sources，並保留 excluded source/reason counts。
- BR companion report 以 `companion-only-not-pure-tracking` 標記，把 aim-ray on-target rate 與 ballistic hitscan/projectile hit count 分欄；pure tracking summary 仍只來自 contact coverage，不讀 hit/damage/kill。
- 同步修正 `trackingContactArtifact` 的 hitbox provenance：well-formed `shape:'sphere'` 會在 artifact/report 中保留 shape，malformed sphere 仍 blocked `invalid-hitbox`。

### 2026-09-03 — WP-55 T4 replay observability

- WP-55 T4 完成：新增 `src/replay/replayContact.ts` 與 `src/replay/replayContact.test.ts`。`sampleReplayContact()` 將 replay time deterministic 對表到 contact artifact row，輸出 sample `t`、target id、target center、aim、`onTarget`、`epsilonDeg`、presentation/window；缺資料與 blocked artifact 以 reason-coded unavailable 呈現。
- 依 OQ-S11-4 / OQ-55-1，第一版不做產品 Replay overlay；交付 `buildReplayContactTrace()` 與 self-contained `renderReplayContactTraceHtml()`，HTML 內嵌 JSON/CSS/JS，contact state 有文字 label（`on-target` / `off-target` / `unavailable: reason`），不只靠顏色。
- T4 verification：T4 focused 11/11 passed；replay/contact focused 43/43 passed；T4 + legacy tracking/BR baseline 13 files / 75 tests passed；`npm.cmd run typecheck` exit 0；`graphify update .` 已執行並保留 graphify-out 更新。
...
| 2026-09-03 | WP-55 T4（detail 見 [wp-55-tracking-on-target-observability-no-health/progress.md](wp-55-tracking-on-target-observability-no-health/progress.md)） | `src/replay/replayContact.test.ts` 11/11 passed；replay/contact focused suite 43/43 passed；T4 + legacy tracking/BR baseline 13 files / 75 tests passed；`npm.cmd run typecheck` exit 0；`graphify update .` executed and graphify-out retained |
| 2026-09-03 | WP-55 T5（detail 見 [wp-55-tracking-on-target-observability-no-health/progress.md](wp-55-tracking-on-target-observability-no-health/progress.md)） | `src/metrics/trackingContactReport.test.ts` 5/5 passed；contact report/artifact/coverage/replay focused suite 40/40 passed；T5 + legacy tracking/BR baseline 14 files / 81 tests passed；`npm.cmd run typecheck` exit 0；`graphify update .` executed but graphify-out restored/not staged because it indexed unrelated WP-54 active document state |
| 2026-09-03 | WP-55 T6（detail 見 [wp-55-tracking-on-target-observability-no-health/progress.md](wp-55-tracking-on-target-observability-no-health/progress.md)） | T6 no-health audit found no target health/HP/damage/health-bar tracking contract；focused contact/report/replay suite 5 files / 40 tests passed；contact + legacy tracking/BR baseline 14 files / 81 tests passed；`npm.cmd run typecheck` exit 0；`npm.cmd test` 210 files / 2021 tests passed（1 file / 2 tests skipped）；docs-only, graphify skipped |
