...
| OQ-54-2 | Core matrix 是否沿用 `2.0 deg / 0.5 deg x 5 deg/s / 20 deg/s`？ | 採為 **calibration candidates**（T2 config 初值），非正式凍結值；T7 依 floor/ceiling 證據決定 retained/revise/remove。**T6（2026-09-03）**：四個候選值全部保留，但實現方式再參數化——size 改為真的目標角尺寸、speed 改以頻帶 + 共用振幅交付（KI-020 §6）。reversal 視窗放寬到 ±13°（KI-019 §5）。**T7（2026-09-03）**：size 兩個候選值 `2.0 / 0.5 deg` **retained**；speed **revise 為 `5 / 14 deg/s`**，頻帶降為 **`[0.15, 1.05]` Hz**、共用振幅 ±16°——理由是凍結準心比值在舊頻帶下的上界只有 1.61（達不到 Gate B 的 2.0），而該頻帶下 20 deg/s 會讓目標沉到地面下（[OQ-54-15](progress.md#open-questions)）。**且 [KI-024](../../../known_issue/KI-024-field-low-eye-not-anchored-halves-delivered-angles.md) 之前所有角度只交付一半**，故 T6 之前各輪宣稱的角度值均非受測者所見 | 研究者 | ✅ Resolved（candidate，非 hard freeze；現行值見 [T7 gate](T7-difficulty-calibration-gate.md) §2） |
