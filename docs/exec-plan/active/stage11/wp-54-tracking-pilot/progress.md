...
- **本 slice 起,WP-54 進入 `tracking-pilot-v2`**：scored 窗從「禁止開火」改為「**全程按住左鍵**」,
  右鍵改為**無效但仍記錄**。研究者動機是**生態效度**（真實 CS2 的追蹤是邊噴邊跟），不是為了消滅
  右鍵違規——見 **D-54.49**。這是換世代,**G5 乾跑（slice 11）連同其 §3.3 結論一併作廢**,§3 的四項
  篩選要在 G6 上重跑。
- **新增 `WEAPONS.tracking_pilot_hold`**（`src/weapon/weapons.ts`）。三個欄位各自扛一條硬約束,
  不是體感選擇：
  - `recoil` 全 0 ⇒ 受測者的**壓槍補償會進 `aimSink`**（記錄的 `ticks[].aim`），有後座力就會讓
    ε(t) 從「純追蹤」變成「追蹤 + 壓槍」,同一構念出現第二定義 ⇒ 牴觸 **C-D4**、並讓 T3 以來所有
    tracking 指標失效。
  - `ads` 省略 ⇒ `CameraController` 的 `effectiveActive = active && adsConfig !== undefined`
    讓右鍵對 FOV／感度完全無效。**這就是「停用右鍵」的全部實作**：不攔輸入、不改事件鏈,
    `heldAds` 與 `ads` event 照記,稽核不損失。
  - `inaccuracy` 全 0 ⇒ 彈著點 = 準心,於是逐發 shots-on-target 與離線 TOT 走**同一套 sphere
    幾何**（引擎 `HitDetector` vs 離線 `trackingDerivation`,GD-7/KI-021 單一來源）,兩者一致性
    本身就是一條 fidelity 交叉驗證。見 **D-54.51**。
- **`magSize` 512 是效度問題,不是體感問題**：`SimLoop.scheduleFire` 的空倉分支會把
  `state.heldFire` **強制歸 false**——彈匣打完,受測者連「按住」的意圖旗標都被清掉、會被記成放開。
  26 000 ms ÷ 100 ms cycle = **260 發**,512 給約 2× 裕度。這道不等式在 slice 17 由 drill 的
  `endCondition.value` **導出**驗證,不寫死候選值（slice 6 的教訓）。
- **查證更正兩則**（與 T7 交接敘述不同）：① 兩個 pilot drill **都沒有 `weaponId`** ⇒ 實際吃
  `main.ts:310` 的預設 **`ak47`**,而 ak47 **有** `ads: { fovDeg: 40 }` 與 recoil magnitude 30
  ——這才是右鍵真的會毀 block 的原因,`usp_s_laser` 從來沒被用上;② 交接稿估的 magSize 256 是按
  0.17 s cycle 算的,在 AK 節奏下**不足**（需 260）。
- **順手修掉一個既有脆弱測試**：`getWeapon` 的錯誤訊息斷言原本寫死八個 id 的字面列表,改由
  `Object.keys(WEAPONS)` 導出——它要測的是「訊息列舉所有已註冊 id」,寫死只會為了無關的理由紅。
- **本 slice 未接線**：新武器尚未掛到任何 drill（slice 17 才接）,故對現行匯出與 sim **逐位無影響**。
- 驗證：`npx vitest run` **218 files / 2101 tests passed**（+2）、`npx tsc --noEmit` exit 0。

### 2026-09-04 — T7 slice 11：G5 乾跑完成 + 研究者決定照原樣招募（docs-only）

- **G5 乾跑**（操作員 P06，9 block + 1 retry，gate **§3.3**）：`atEye` **3.99–4.01 u / 100–103% /
  1.999–3.006°**、layer 3b **10/10 match**（≤ 8.9e-16 u）、layer 5 比值 **2.06–3.80**、
  覆蓋率 3201–3203 ticks / 25000–25016 ms、overflow 全 false、`crossOriginIsolated` 全 true、
  stderr **零警示**。
- **三個 G4 沒有的正面結果**（都非 Gate B 判定——n=1 且乾跑不計入證據）：
  ① **B-3a 方向四項全部成立**——**第一次在真正交付的刺激上**驗證到 size×speed 操弄有效
  （`size@5dps` 57.7<80.7、`size@14dps` 11.1<41.3、`speed@2deg` 2.476>1.044、`speed@3deg` 2.169>1.158）；
  ② **B-3c 全在 ±20% 內**（−16.3…+14.0%），優於 G4 的 −21.0/+38.7 ⇒ 25 s 目前看不出要調；
  ③ `reversal_high` 比值 **2.06** 與 G4 的 2.05 幾乎逐值重現。
- **唯一出界**：`3deg_5dps` **TOT = 80.7%**，超 §3 篩選窗上緣 0.7 點。這正是 §3.2 早已入帳的
  ceiling 風險——但**實測比 G4 重算預測的 86.2% 好一截**。
- **使用者決定（D-54.48）**：**照原樣招募，風險入帳**，不再換世代。gate 新增 **§3.4** 寫明這是
  刻意偏離 §3 第 4 點及其三個理由，**未放寬任何判準**（§2.2 門檻一字未動）。
- **`3deg_5dps` 仍是 Gate B 最可能判 revise 的 cell**，分析時優先看；且真正的風險不是 TOT 本身，
  而是**逼近天花板會壓縮受測者間差異** ⇒ **B-2a 的 CV ≥ 15% 子句比 TOT 子句更難救**，而它在
  n=1 完全測不到。
- **operator 紀錄**：`calibration_vertical` 第一份 `protocol_violation` ⇒ BLOCKED ⇒ 重跑後 eligible
  （`n=2 eligible=1`，舊 attempt 依 D-54.32 append-only 留檔）。違規 `kind` 又是 **`ads`**——
  歷來 **6/6 全是右鍵**，再次印證 OQ-54-13 的防線只有事前提醒。
- **未動 production code**；乾跑資料不進 git（`.pilot-analysis/` 已 gitignore）。
- **下一步 = 招募 12–20 人**（gate §4）：全員 `Session index = 0`，6–8 人加跑 `= 1`；
  **刻意涵蓋 ≥ 2 種顯示器刷新率**（T6 至今只有 60 Hz 一種）。
...
| D-54.48 | **G5 乾跑的 TOT 檢查對 `3deg_5dps` 不成立（80.7% > 80% 上緣）,研究者仍決定照原樣招募**,不再改刺激 | gate §3 第 4 點寫「任一項不成立 ⇒ 不招募」,故續行是一次**刻意的、有理由的偏離**,以本列存證,不是默默通過篩選。三個理由：①乾跑是 **n=1,且是跑過 G3/G4/G5 最熟練的那一位**,而 B-2a 實際判的是 **12–20 人的中位數**,招募條件本就是「不同 tracking 程度」⇒ 母體中位數很可能明顯低於 80.7%;②G4 用錄音重算預測 86.2%,**實際交付只有 80.7%**,比預測好一截且只超 0.7 點;③§2.4 的 go 只需 ≥1 core + ≥1 reversal 判 retained,其餘三個 core cell 落在 11.1–57.7%、兩個 reversal 落在 41.2–54.4% ⇒ **即使 `3deg_5dps` 最後判 revise,這批 12–20 人也不會白收**。**未因此放寬任何判準**——§2.2 門檻一字未動。被否決的兩個備選都要換世代 + 重跑乾跑：慢速候選值 5→7 deg/s（還得先驗 7 deg/s 放得進 field-low 垂直包絡,同 OQ-54-15 的衝突類型）、大尺寸層 3.0→2.5°（兩層只差 0.5°,**很可能讓本輪剛第一次證實成立的 B-3a 尺寸效果變得測不出來**） | ✅ Confirmed（使用者拍板 2026-09-04，T7 slice 11，**收資料前**） |
