...
| **0.5° 條件在修正後的速度下的主觀可辨識性** | ❌ **仍然「幾乎看不見/只能猜」,且連兩個單軸 axis calibration block 也看不見**(操作員 2026-09-03 回報,修正後的 5 / 20 deg/s 下)。依 §10.5 不放大目標;客觀對照見 **§12.8** |
...
### 12.8 主觀「看不見」的客觀對照:band-limited 核心矩陣的 ε 幾乎沒有動態範圍 ⚠️

操作員回報 0.5° 目標「幾乎看不見/只能猜」,**連單軸 calibration 也看不見**(§12.5)。這句話碰到
§9 的 stop 例句,所以本輪把它拿去對客觀資料:**如果把準心凍結在該受測者自己的 aim 中位數上**
(對「看不見目標」的人來說,這是最好的「不動」策略——中位數本身已落在目標平均方向上),
重算同一份 run 的 ε(t)(用 `angularEccentricityDeg()`,不另立定義,C-D4):

| 條件家族 | 凍結準心看到的最大 sweep | 「完全不動」RMS ε | 實際 RMS ε | 比值 |
|---|---|---|---|---|
| 5 deg/s 三個 cell(practice / 2deg_5dps / 0p5deg_5dps)+ **兩個 calibration** | **1.26–1.95°** | **0.75°** | 0.61–0.72° | **1.05–1.25** |
| 20 deg/s 兩個 cell | 5.5–6.1° | 3.06° | 2.05–2.24° | 1.38–1.49 |
| reversal medium / high | 3.7–8.2° | 2.2–3.4° | 1.05–1.23° | **2.08–3.26** |

**讀法**:比值 = 「完全不動」的誤差 ÷ 真人的誤差,是這個條件**能不能分辨會跟槍與不跟槍**的上界。

- **5 deg/s 家族與兩個 calibration block:ε 的整個動態範圍只有 0.75°**(從完美跟槍 0° 到完全不動
  0.75°),而真人落在 0.61–0.72°,也就是**離「完全不動」只有 10–25%**。這些條件**在測量上無法
  分辨跟槍能力**——不是受測者不努力,是**刺激本身沒有給 ε 留下可分辨的空間**。這與「看不見所以
  不知道要往哪動」完全自洽。
- **reversal 家族相反:比值 2.1–3.3,測得到跟槍**。第三輪唯一確定有效的家族。
- **根因是刺激幾何,不是儀器**:`band-limited-2d-v1` 的行程由速度與頻帶共同釘死
  (amplitude ≈ speed / 2πf);在預註冊的 `frequencyBandHz [0.3, 2.1]` 下,5 deg/s **必然**只能走
  ±0.4–2.7°。**要「慢又走得遠」就必須降低頻帶下限**——而速度與頻帶都是預註冊參數。
- **TOT 為什麼看起來還可以**:`2deg_5dps` 與 `0p5deg_5dps` 的 RMS ε 幾乎相同(0.62–0.72°),
  TOT 卻是 43–49% vs 2.5–4.5% —— 差別**幾乎全部來自 hitbox 尺寸**,不是跟槍品質。TOT 在這些
  cell 主要是尺寸的函數,不宜當成能力指標(C-D3 的警戒範圍)。

> **帳本更正**:本輪分析初稿曾以 calibration 的 TOT 6.6–10.5% 與 `tAcquireMs ≤ 328 ms` 推論
> 「受測者顯然看得見並跟得上」,那個推論**是錯的**——它把「不動」基線估成 ±3.7° 掃幅(2.6° RMS),
> 但 5 deg/s cell 的實際掃幅只有 ±2.2°,凍結準心的 RMS 只有 0.75°。修正後的結論見上表。

### 12.7 判定

**儀器面(資料鏈路)通過了資料能檢驗的每一項**(§12.2、§12.3),**且本輪沒有 instrumentation
defect**;§6 的 tester 份量已由使用者決定以 2 位結案(§12.6 第 1 項)。

**但判定不能因此是 GO**:§12.8 顯示 **band-limited 核心矩陣的 5 deg/s 家族與兩個 axis calibration
block,ε 的動態範圍只有 0.75°,真人成績離「完全不動」只有 10–25%** ⇒ 這些條件**測不出跟槍能力**,
與操作員「連 calibration 都看不見」的回報自洽。這正是 Gate A 存在的目的:**儀器忠實地記錄了一個
問不出答案的問題**。

- 這**不是** instrumentation defect(四層對帳全綠、刺激逐位符合宣稱),所以不是 §9 定義的
  「可修的 instrumentation defect ⇒ REVISE」。
- 是否構成 §9 的 **stop**(「0.5° 條件完全不可辨識且連 calibration 都無法判斷」)取決於
  **修法是否落在 WP-54 範圍內**:降低 `frequencyBandHz` 下限可以在同樣速度下換到更大行程,
