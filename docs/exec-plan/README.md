...
| WP | 子資料夾 | 目標 | 里程碑 | 相依 | 估時 | 狀態 |
|---|---|---|---|---|---|---|
| **WP-60** | [`active/stage13/wp-60-raw-mouse-sample-capture/`](active/stage13/wp-60-raw-mouse-sample-capture/README.md) | 逐筆原始滑鼠取樣入匯出（columnar + 整數 µs delta、opt-in 預設關閉）＋ `SimLoop` 消費點擷取 ＋ 時間間隙切段原語 ＋ 操作者取樣健康度報告 | —（exit gate 即交付） | 無（WP-57 已交付，只讀不改） | 5.5–9.5 | ✅ **T-exit 交付 2026-09-09**（**帶三項具名缺口**）—— A-60.1～16 **14 ✅／1 ✅ 帶上界告警（OQ-60.7 長 drill 匯出體積）／1 🟡（F6）**，無 ❌；typecheck ×2、全量 Vitest（**2614 passed**）、`vite build` 三閘 exit 0；決定性以 `TickRecord` 全欄位 `Object.is` 開／關對照釘死。T-exit 另修掉一個會誤判每份真人 run 的事件率 blocker（**D-60.X1**）。<br>⚠️ **缺口**：① F6 瀏覽器 frame log 開／關對照未量測；② T3 DoD 的真人取樣分布未取（逐筆軌跡依 D-57.T5-8 不入 repo）；③ **全量 Playwright 未取讀數** —— port 5173 被主 checkout 的 dev server 占用且服務不含 WP-60 的程式碼，跑了會沉默地測錯的樹並寫進真實 history root；屆時預期仍因既存 [KI-027](../known_issue/KI-027-overlay-layering-researcher-submenu-guard-dead.md) exit 1。故 **T0／T3 維持 🟡** |
| **WP-61** | [`active/stage13/wp-61-lift-off-validation/`](active/stage13/wp-61-lift-off-validation/README.md) | 先驗證空洞前後運動學的可分性，再決定判準移植與校準 | 待指派 | WP-60 T-exit ✅ + 高刷（≥ 144 Hz）真人標註 cohort | 待資料稽核 | 🟡 **範圍草案**；R2 已收斂 2026-09-09（**空洞長度不足以可靠分離抬滑鼠與停頓**，D-60.R2-1）⇒ 不得以時間間隙長度直接當判準。尚未開工；T0 第一件事為 **OQ-60.4 構念歸屬拍板** |
