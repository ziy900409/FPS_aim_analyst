...
| **WP-63** | [`active/stage13/wp-63-micro-flick-v8-measurement-foundation/`](active/stage13/wp-63-micro-flick-v8-measurement-foundation/README.md) | `micro_flick_three_target_test_v8`（全 repo 唯一**三顆同時存活**）的量測基礎層：population-aware per-target 窗界原語 + 四層**事件錨定**指標（結果／幾何／免閾值微調／選擇策略）+ v8 宣告零散布武器 + [KI-035](../known_issue/KI-035-mouse-gain-stale-after-sensitivity-or-fov-change.md) 修復 | —（T-exit 即交付） | WP-56 T-exit ✅（三顆 population 生命週期）· WP-60 T-exit ✅（`?rawMouse=1`）；與 WP-61／62 無相依、可並行 | 12–16.5 | ⬜ **未開工（2026-09-10 規劃完成）**。核心判斷：既有分析棧從 [`firstShot.ts`](../../src/sim/firstShot.ts) 到 [`peekWindows.ts`](../../src/metrics/peekWindows.ts) 都建立在「一次只一個 active 目標」這個從未寫下的前提上，v8 違反它會產生**四個靜默錯誤**（歸屬到錯的目標，數字看起來合理）；但**蒐集層已經足夠**（`visible` 事件逐顆帶座標、v8 目標靜態、`fire` 帶 `viewYaw/viewPitch`）⇒ 缺的是離線重建層，不需改 recorder 或 schema。**不引入任何 movement-onset 判準**（`t_detect` 為既有構念，C-D4；且 v8 上受 KI-031／KI-034 雙重阻塞）——全部指標改為事件錨定。交付宣稱上限 = 「可算、可重現、可稽核」，**不含效度**（C-D3 未過，不得進教練報告）。決策草稿 [GD-39](DECISIONS.md)（T-exit 入帳）。⚠️ **主題部分不屬階段 M**，依使用者 2026-09-10 指示寄放，明帳見 [stage13 README §3](active/stage13/README.md)。 |
