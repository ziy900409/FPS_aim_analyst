...
> **M14 已達成(2026-08-05)**：匿名真實 drill 匯出完成 ingest/dt、19/20 分段成功率與 20 張疊圖人工檢核；WP-30/31 entry 已解鎖。效度範圍仍限單一 counter-strafe 樣本。

| WP | 子資料夾 | 目標 | 里程碑 | 相依 | 估時 | 狀態 |
|---|---|---|---|---|---|---|
| **WP-28** | [`active/stage4/wp-28-research-foundation/`](active/stage4/wp-28-research-foundation/README.md) | research 地基：四目錄制 + schema v2 ingest + ω(t)/ε(t)（**含 ε 雙向 parity 閘**）+ submovement 分段（參數凍結）+ quality flags + 一鍵 pipeline | **M14 ✅** | M4 ✅ + WP-16 ✅ + M11/M12 ✅ | 3.5–4.5 | ✅ **完成(2026-08-05);M14 六項全綠** |
...
| **M14 ✅**<br>(2026-08-05) | research 地基六項全綠：① 真實匯出 3,507 ticks / dt 7.8125ms / gap 0 ② ε parity `test:ci` 綠（82 files / 641 tests + 19 e2e）③ 合成邊界 max error 1 tick ④ 真實資料 `seg-v1` 19/20(0.95)+20 張疊圖 ⑤ pre-registered 參數不變且真實人工檢核/限制回寫 `analysis-segments.md` ⑥ `uv run pytest` 74 passed。證據見 [WP-28 T-exit](active/stage4/wp-28-research-foundation/T-exit-gate.md) | WP-28 | research 分段 + ε 地基成立;**WP-30/31 entry blocker 已解除**。效度聲稱限單一匿名 counter-strafe 樣本 |
...
- 階段 D 全在 `research/`（Python 離線）＋兩個對表閘，引擎零改動（例外：WP-29 T3 選配 `key` 事件、WP-32 metrics/UI）；與其他 stage 零檔案熱區重疊。**M14 已於 2026-08-05 通過,WP-30/31 可進場**；WP-31 為 M15 選項（未過 reliability gate 的指標不晉升，GD-20）。詳見 [`active/stage4/README.md §5`](active/stage4/README.md)。
