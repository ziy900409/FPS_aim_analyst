...
| WP | 子資料夾 | 目標 | 里程碑 | 相依 | 估時 | 狀態 |
|---|---|---|---|---|---|---|
| **WP-28** | [`active/stage4/wp-28-research-foundation/`](active/stage4/wp-28-research-foundation/README.md) | research 地基：四目錄制 + schema v2 ingest + ω(t)/ε(t)（**含 ε 雙向 parity 閘**）+ submovement 分段（參數凍結）+ quality flags + 一鍵 pipeline | **M14 🟡** | M4 ✅ + WP-16 ✅ + M11/M12 ✅ | 3.5–4.5 | ✅ **task 全數完成(2026-08-05);M14 ①⑥維持/②已重新宣告(2026-08-06)/③④⑤ 仍撤回**(KI-005 A1 已落地/A2 待排程,KI-006 C 已落地(2026-08-07)/B 待 A2) |
...
| **M14 🟡**<br>(①⑥ 維持;②於 [KI-004](../known_issue/KI-004-sim-world-unit-domain-mismatch.md) S1 落地後重新宣告 2026-08-06;③④⑤ 因 [KI-005](../known_issue/KI-005-omega-render-sim-aliasing.md)/[KI-006](../known_issue/KI-006-m14-sample-no-counterstrafe.md) 仍撤回) | ① 真實匯出 3,507 ticks / dt 7.8125ms / gap 0(維持)⑥ `uv run pytest` 74→183 passed(維持)。**②** 原六項全綠宣告(2026-08-05)因 ε(t) 量測原點錯誤(D2a/D2b,實測偏差 12.52°/67.11°)撤回;**KI-004 S1 落地後以新證據重新宣告**(閘 ① `fire.offsetDeg` oracle ≤0.5°、閘 ② 閉式幾何 ≤1e-9、parity fixture 重產綠、`test:ci` 88 files/694 tests + 19 e2e、`uv run pytest` 183 passed;詳見 [WP-28 progress.md](active/stage4/wp-28-research-foundation/progress.md)「M14 ② 重新宣告」段)。**③④⑤ 另因 KI-005(ω(t) render/sim aliasing)+ KI-006(真實樣本無 counter-strafe 構念)於 2026-08-06 撤回,解除條件均未滿足**:KI-005 的**選項 A(A1)已於 2026-08-06 落地**——`ticks[].dYaw/dPitch` 依事件時間戳積分,四種刷新率下逐位相同(NFR-A-4),但這只是「量測儀器修好」,**不是「效度恢復」**——③④⑤ 的解除需 **A2**(新採樣 → 複驗 → `seg-v2` 重掃)完成,見 [KI-005-A/A2-blocked-plan.md](../known_issue/KI-005-A/A2-blocked-plan.md);KI-006 的 **C**(construct presence gate)已於 2026-08-07 落地([KI-006-C/](../known_issue/KI-006-C/README.md)),但**只補閘、不恢復效度**——解除需**新採樣時明確要求 counter-strafe 行為**(**B**,委派 [A2-T1](../known_issue/KI-005-A/A2-blocked-plan.md),驗收清單見 [KI-006-C/README.md §6](../known_issue/KI-006-C/README.md) B-1~B-5) | WP-28 | research ingest/parity 機制 + ε 地基(僅 KI-004 原因)已修正;量測儀器層面 KI-005 A1 已修好、KI-006 的構念存在性閘(C)已落地,**但 M14 ③④⑤ 與 WP-30/31 entry blocker 因 A2(新採樣)與 KI-006-B(重新採樣)均未落地而仍維持**,尚不得展開。效度聲稱限單一匿名 counter-strafe 樣本 |
...
- 階段 D 全在 `research/`（Python 離線）＋兩個對表閘，引擎零改動（例外：WP-29 T3 選配 `key` 事件、WP-32 metrics/UI）；與其他 stage 零檔案熱區重疊。**M14 原六項全綠宣告(2026-08-05)已分兩次撤回**（② 因 KI-004、③④⑤ 因 KI-005/KI-006）；**② 已於 KI-004 S1 落地後重新宣告(2026-08-06)**，但 **WP-30/31 entry blocker 因 KI-005/KI-006 兩條獨立理由的解除條件均未落地仍維持，尚不得進場**（KI-005：儀器修法 A1 已落地、複驗與重掃 A2 待排程；KI-006：構念存在性閘 C 已落地、重新採樣 B 待 A2）；WP-31 為 M15 選項（未過 reliability gate 的指標不晉升，GD-20）。詳見 [`active/stage4/README.md §5`](active/stage4/README.md)。
