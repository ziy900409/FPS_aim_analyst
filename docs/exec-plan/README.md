...
**階段 M（`active/stage13/`，🟡 active；WP-60 ~ WP-61；原始滑鼠取樣與抬滑鼠判準驗證）**
...
| **WP-61** | [`active/stage13/wp-61-lift-off-validation/`](active/stage13/wp-61-lift-off-validation/README.md) | 先驗證空洞前後運動學的可分性，再決定判準移植與校準 | —（T-exit 即交付；三種合法結案：通過／不可靠分離／證據不足） | WP-60 T-exit ✅ + 真人標註 cohort（**實錄於 240 Hz**，逐份條件 `meta.displayHz === 240`，禁止與 60 Hz 混批 —— D-61.U3。兩個門檻並存且都成立：**≥ 120 Hz** = 資格閘地板 `PERF_FLOOR_MS`；**≥ 144 Hz** = KI-031 完全緩解點） | 9–15（T4 條件式；負面結論路徑 6.5–11） | 🟡 **執行計畫已制定 2026-09-09**（T0～T4 + T-exit）；**尚未開工**。R2 已收斂（**空洞長度不足以可靠分離抬滑鼠與停頓**，D-60.R2-1）⇒ 不得以時間間隙長度直接當判準。<br>✅ **四個使用者決策已於 2026-09-09 收斂**（D-61.U1～U4）：構念**並存**且新構念定名「**感測器離地／sensor lift**」（＝ WP-60 交不出來的第四項 handoff，OQ-60.4 就此結案）／標註採**自報鍵 + block 冗餘**（僅支撐事件級匹配，不支撐起點精度）／cohort 錄於 **240 Hz**（同時滿足 120 Hz 資格閘地板與 144 Hz 的 KI-031 緩解點；兩者非矛盾）／n=1 宣稱上限 = 本操作者 × 本硬體 × 本 drill，一律 `research_only` ⇒ **T4 的 `src/` 零 importer 為終局狀態**。<br>⚠️ 計畫相對草案新增 **T1 標註通道儀器**：repo 內 `KEY_CODE` 為四鍵封閉集、`DrillEvent` 無標註型別 ⇒ 草案要求的「獨立於空洞的逐次標註」在物理上無法產生（D-61.P2）。剩餘阻塞 = T0（評估契約 pre-registration）+ cohort 尚未錄製。決策帳本預留 **GD-37**（佔位符） |
