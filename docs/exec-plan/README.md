...
**階段 M（`active/stage13/`，🟡 active；WP-60 ~ WP-62；原始滑鼠取樣與抬滑鼠判準驗證 ＋ 一個寄放的排程層 WP）**
...
| **WP-62** | [`active/stage13/wp-62-session-plan-per-item-weapon/`](active/stage13/wp-62-session-plan-per-item-weapon/README.md) | Session Plan 自訂 program 的**每一列可事先指定武器**；編譯期擋掉未知武器與覆蓋實驗格；選擇進匯出稽核並與逐 run 實際武器對帳 | —（T-exit 即交付） | WP-58 T-exit ✅（只讀不改 frozen 軌）；與 WP-60／61 無相依、可完全並行 | 8.5 | 🟡 **T0 ✅ / T1 ✅ / T2 ✅ 2026-09-10；T3 未開工**（T0 基線 Vitest 2,822 passed／Playwright 103 passed；T2 後 Vitest 2,896 passed，typecheck ×2／build exit 0；Playwright 留到 T6）。⚠️ **主題不屬階段 M**（session 排程層，屬 WP-58 的延伸），依使用者 2026-09-10 指示寄放於 stage13；明帳見 [GD-38](DECISIONS.md) ① 與 [stage13 README §3](active/stage13/README.md)。原始需求「全域 pin 單一武器（`usp_s_laser`）」已於規劃期否決：該武器 `inaccuracy` 三項全 0 會讓 counter-strafe 的「急停時機 → 首發命中」因果通道消失（構念失效，非資料斷代），且 `magSize 12` + 全 repo 無 reload 會造成 run 中途靜默停火（GD-38 ②）。 |
