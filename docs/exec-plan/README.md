...
| **WP-68** | [`active/stage13/wp-68-micro-flick-v9-measurement-parity/`](active/stage13/wp-68-micro-flick-v9-measurement-parity/README.md) | `micro_flick_three_target_test_v9` 的量測基礎層對齊：宣告零散布武器 `usp_s_laser` + `DECLARED_WEAPON_ROSTER` 登記 + 計時制 drill 的計分窗右界 | —（T-exit 即交付） | **WP-63 T-exit ✅**（唯一上游）；與 WP-67 無相依、可並行 | 3 | ⬜ **未開工（2026-09-14 規劃完成）**。承 WP-63 釋出（v0.1.1）後的實測發現：WP-63 的窗界原語與四層指標對 v9 **零修改可用**，但 v9 **沒宣告 `weaponId`** ⇒ 吃預設 `ak47`（實測 33/33 發帶散布、32/33 帶 punch），命中與否不再是開火瞬間角誤差的純函式；只換武器一項即讓 L3 與方向預測曲線從 `n = 0` 回到 `n = 17`。第二個發現：`validSpanMs` 是為 kill-budget drill 定義的，套到 `timeLimit` 的 v9 上會系統性**高估** `killRateHz`（與 [KI-037](../known_issue/KI-037-valid-duration-includes-countdown.md) 偏誤方向相反，但 KI-037 在 history 投影路徑、談左界，**本 WP 不碰它**）。使用者 2026-09-14 拍板 v9 用 `usp_s_laser`；連帶後果為 v8／v9 在 `meta.weaponId` 上不可分，分析側必須以 `drillId` 分池。決策草稿 `GD-45`（T-exit 入帳）。⚠️ 主題不屬階段 M，依使用者指示寄放於 stage13，明帳見 [stage13 §3](active/stage13/README.md)。 |
