...
| ✅ | **T1** `availableDrills` 補三個缺口 config;`SessionPlan`/`sessionPlanPresets.ts`/`SessionRunner`(手動固定順序)+ 熱身降級;`SessionPlanSetup.ts`;`main.ts` 接線;`metadata.ts` additive `sessionPlanPreset`（三個新增 drill 的完整鏈路已於 T-exit 用 e2e 補證） | [T1-session-plan-runner.md](T1-session-plan-runner.md) | T0 | **Med–High** |
| ✅ | **T2** `RestOverlay.ts` + `SessionRunner.poll(nowMs)` 接入既有 renderLoop（含休息的 session plan 已於 T-exit 用真實 DOM 接線 e2e 補證） | [T2-rest-overlay.md](T2-rest-overlay.md) | T1 | Low |
