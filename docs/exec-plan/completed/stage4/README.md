...
| **狀態** | ✅ **stage4 交付(2026-08-17,M15 達成)**:WP-28 task 全數完成,M14 六項全數恢復/重新宣告(見下)· WP-29 ✅ 完成(2026-08-05,**不受 KI-004/005/006 阻塞**,只吃 `events`/`ticks[].keys`)· WP-30 ✅ 完成(2026-08-10)· WP-31 ✅ **完成(2026-08-12)**:T0~T3 + T-exit 全數交付,三個 P2 指標(SPARC/xcorr/Fitts)收斂為研究向/`blocked-by-data`,`coach-report-v2` 已含研究向區塊,WP-32 交接清單為空· WP-32 ✅ **T0–T5 + T-exit 全數完成(2026-08-17)**:晉升清單三項進 `src/metrics/` + 結果頁擴充 + 驗收清單 D 八項全通過([acceptance-stage-d.md](../../../operational/acceptance-stage-d.md)),C-D5 入 CLAUDE.md §4 + GD-21 入 DECISIONS.md(關閉 OQ-S4-24)。**M14 ①⑥ 維持**(ingest/dt、pytest 綠,與 aim 差分/行為內容無關)。**M14 ②** 因 ε(t) 量測原點錯誤(D2a/D2b,實測偏差 12.5°/67°)於 2026-08-05 撤回,**已於 [KI-004](../../../known_issue/KI-004-sim-world-unit-domain-mismatch.md) S1 落地後重新宣告(2026-08-06)**。**M14 ③④⑤** 另因 [KI-005](../../../known_issue/KI-005-omega-render-sim-aliasing.md)(ω(t) render/sim aliasing)+ [KI-006](../../../known_issue/KI-006-m14-sample-no-counterstrafe.md)(真實樣本無 counter-strafe 構念)於 2026-08-06 撤回,**已於 [A2-T4](../../../known_issue/KI-005-A/A2-blocked-plan.md#a2-t4--m14-③④⑤-重新宣告-✅-已完成2026-08-07)** 重新宣告(2026-08-07)——KI-005 A1+A2 全數落地、KI-006(C+B)全數落地並 CLOSED。效度聲稱不擴大:仍限單一匿名受試者、n=3 session、非母體層級證據 |
...
| **WP-32** | [wp-32-dashboard-integration/](wp-32-dashboard-integration/README.md) | 晉升整合:golden parity → TS metrics + 結果頁擴充 + 驗收清單 D | — | **M15 ✅** | WP-29 ✅ + WP-30 ✅ + **WP-31 T-exit ✅**([D-32.1](wp-32-dashboard-integration/progress.md)) | 2–3 → **4.5–5.75**([D-32.0](wp-32-dashboard-integration/progress.md)) | ✅ **完成(2026-08-17)**:T0 晉升清單與移植紀律凍結;T1 ω+SG、T2 seg-v2、T3 phase/sync、T4 curve 均以 golden 對表進 `test:ci`;T5 結果頁 research-promoted 區塊(封閉 8 個 metric id、n/flags/version、blocked 態、ω/ε inline SVG、同一 `ExportPayload` 派生 stats);**T-exit ✅**:`acceptance-stage-d.md` 驗收清單 D 八項全通過、C-D5 入 CLAUDE.md §4、**GD-21** 入 DECISIONS.md(關閉 OQ-S4-24)。**stage4 交付** |
...
| **M15 ✅**<br>(2026-08-17) | 驗收清單 D 全項通過:教練報告一鍵產出(FR-D16)、晉升指標 TS golden 對表綠、`test:ci` exit 0 **且** `uv run pytest` 綠、每指標附效度證據(fixture + 真實檢核 + 限制)、P2 三指標各有明確進退判定 | WP-32 | **stage4 交付達成**:瞄準 × 急停教練分析管線 pilot-ready。八項驗收全通過([acceptance-stage-d.md](../../../operational/acceptance-stage-d.md));P2 三指標(SPARC/xcorr/Fitts)全數判定不晉升(合格交付,C-D3);C-D5 雙實作對表紀律入 [CLAUDE.md](../../../../CLAUDE.md) §4 + **GD-21** 入 [DECISIONS.md](../../DECISIONS.md) |
...
| **T5 結果頁擴充** ✅ | `MetricsDashboard`/`ResultScreen` 增 research-promoted 區塊(純 TS + DOM,D1,n/flags/version/效度層級 + `blocked` 態);「統計 = 匯出」E2E | ✅ `npm.cmd run test:ci` → Vitest `98 passed / 810 tests` + Playwright `21 passed`;metric id 集合 = 封閉清單;E2E 證明結果頁 promoted 非 blocked 且 UI value = 同一次 export payload | Med |
...
| ~~**OQ-S4-21**~~ | ~~scipy `savgol_filter(mode='interp')` 的 edge polyfit 以凍結矩陣重現後,能否在三份真實 fixture 上穩定達 ≤1e-9~~ | ✅ **關閉(2026-08-17,WP-32 T1)**:SG 係數表 ≤1e-12、真實 ω 訊號 SG smoothing ≤1e-9 通過 | 研究者 | WP-32 T1 ✅ | unblocked |
| ~~**OQ-S4-22**~~ | ~~結果頁單 drill n ≈ 20 peeks(phase 非退化約 59/60 → 單 drill ~19),phase/sync 的均值是否穩定到可對選手呈現~~ | ✅ **關閉(2026-08-17,WP-32 T5,D-32.8)**:不顯示單一分數;Phase/Sync 卡以 p50 為主值,detail 強制列 mean/SD/n,flags+version 永遠顯示;`n=0` 顯示 `No samples` | 使用者 / 研究者 | WP-32 T5 ✅ | unblocked |
| ~~**OQ-S4-23**~~ | ~~`curve-v1` 在結果頁的縮圖形式(L/R 疊圖 + IQR 帶 vs 只給 n 與帶寬摘要)~~ | ✅ **關閉(2026-08-17,WP-32 T5,D-32.8)**:結果頁顯示 `omega` 與 `epsilon` 兩張 inline SVG;每張 L/R mean 疊線 + IQR band + `n(L)`/`n(R)` + flags/version | 使用者 | WP-32 T5 ✅ | unblocked |
| ~~**OQ-S4-24**~~ | ~~晉升後 Python/TS 雙實作的長期維護紀律要不要升為硬約束~~ | ✅ **關閉(2026-08-17,WP-32 T-exit)**:已升為 [CLAUDE.md](../../../../CLAUDE.md) §4 **C-D5** 硬約束(任一端改動 `seg-v2`/`phase-v1`/`curve-v1`/`sync-v1`/`sg-seg-v2` 語意須同步重跑 golden 對表,版本字串只能升版);入 [DECISIONS.md](../../DECISIONS.md) **GD-21** | 使用者 | WP-32 T-exit ✅ | unblocked |
...
- [x] `docs/operational/acceptance-stage-d.md`(新,WP-32 T-exit:驗收清單 D 八項全通過 + 已知限制七條,2026-08-17)。
- [x] 規格書版本對帳:新增「階段 D」節 + 附錄 E 增「驗收清單 D」(2026-08-17,WP-32 T-exit)。
