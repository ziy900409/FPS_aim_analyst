...
### GD-32 🟡 spawn 幾何雙 frame 並存 — eye-frame 球面(新 wide drill)vs origin-frame 圓錐(spider-shot-v1/v2);匯出角度欄位的 frame 語意待拍板(2026-09-07,WP-57 T0)
...
| **問題/決策** | **決議(工程,2026-09-07):三套幾何並存,各自劃定適用範圍,不做統一重構。**<br>① **新 drill `spider-shot-wide-v1` 用 eye-frame 球面**:`pos = eye + d·(sin yaw·cos pitch, sin pitch, −cos yaw·cos pitch)`,`eye = (0, PLAYER_EYE_HEIGHT_U, 0)`。理由:本 drill 要把「貼近玩家 FOV 水平極限」與「相對中心目標 ±p 度」當**條件變因**,兩者都必須以玩家實際所見的角度定義才有構念效度;`abs(pos − eye) ≡ d` 也讓目標角徑真正恆定。<br>② **`spider-shot-v1`／`v2` 的 origin-frame 圓錐不動**。參數已於 WP-39／WP-44 凍結;且其 spawn 與離線推導（`deriveSpiderShotTransitions()` 同樣以 `normalize(targetPoint)` 取世界原點）**兩端一致地偏移** ⇒ 指標內部自洽、既有結論不失效。回頭改會作廢既有 baseline,而收益只是幾何語意的整齊。<br>③ **`angularSpawnPose()` 的圓柱語意不動**。改它會動到 `micro_flick` 家族的逐位行為與 golden。<br>④ **匯出角度欄位的 frame 語意 = 🟡 待使用者拍板（WP-57 OQ-57.7,T4 前）**,見下。 |
...
| **待辦/結論** | ① WP-57 T3 必須把 arena 的 `eyeZ: 0`、`depth ≥ 17.28`(KI-012 後牆淨空)與 `eyeHeight === PLAYER_EYE_HEIGHT_U` 寫成測試。<br>② **🟡 待使用者拍板(WP-57 [OQ-57.7](active/stage12/wp-57-spider-shot-wide-flick/README.md),T4 前)**:匯出的 `angularDistanceDeg`／`angularSizeDeg` 要 **(a)** 照 FR-57.11 原樣不動、只在 [analysis-spider-shot.md](../operational/analysis-spider-shot.md) 記載離線換算式(建議預設,偏差已收斂且資訊無損)、**(b)** 視為 `spiderShotConditions.ts` 的 bug 另開 KI 修成 eye-frame(正確的長期解,但會改動 v1/v2 已凍結的匯出值、需重錄 baseline,不該由 WP-57 夾帶)、還是 **(c)** 為 wide drill 加 drill-scoped eye-frame 欄位(**有 C-D4「既有構念不得有第二定義」之虞**)。<br>③ `CONTEXT.md` 需補「eye-frame 球面 spawn」術語與 `side` 的跨 drill 語意差異(WP-57 T4／T-exit)。 |
| **狀態** | 🟡 幾何並存的決議已定(2026-09-07,WP-57 T0);**匯出欄位 frame 語意待使用者拍板**(OQ-57.7,WP-57 T4 前)。實作落在 WP-57 T1～T6,尚未有 production diff。 |
