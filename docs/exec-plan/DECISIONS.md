...
| **狀態** | 🟡 **T0 entry gate ✅(2026-09-10)**,T1 未開工。基線全綠:typecheck ×2、`vite build`、Vitest **2,822 passed／2 skipped**、Playwright **103 passed**(`--workers=1`)皆 exit 0。計畫見 [active/stage13/wp-62-session-plan-per-item-weapon/](active/stage13/wp-62-session-plan-per-item-weapon/README.md),T0 逐項證據見其 [progress.md §T0](active/stage13/wp-62-session-plan-per-item-weapon/progress.md)。<br>OQ-62.1/62.2(表單顯示與 ADS 警告,owner = 研究者)待 **T4 開工前**、OQ-62.3(對表測試覆蓋率,owner = 實作者)待 **T1 開工時**;另新增 ⑧ 的 `loadSceneById` 處置待 **T3 開工時**——三者皆有明文預設假設,非阻塞。 |
