...
| OQ | 問題 | 待決者 |
|---|---|---|
| **OQ-KI6-1** | ~~採選項 A(換 09:39)、B(重採)或兩者併行~~ → **A 已於 2026-08-06 隨 [KI-005](KI-005-omega-render-sim-aliasing.md) 「不做選項 C」的拍板自動出局**(§4)。**剩 B(重新採樣)為唯一路徑**,待確認採集時機與規模(與 OQ-KI5-6 同一件事)。✅ **時機與規模已決(2026-08-07)**:與 KI-005 A2-T1 合併為同一次採集,n > 2 session(見下 OQ-KI6-4)。B 的驗收清單見 [KI-006-C/README.md §6](KI-006-C/README.md)(B-1~B-5);**實際執行(A2-T1)仍待研究者排程** | 使用者 |
