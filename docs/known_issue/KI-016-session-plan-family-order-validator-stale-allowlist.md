...
> 狀態：🔴 診斷完成，修法待落地（使用者拍板先只交文件，程式碼未改動）。
...
- [ ] `sessionSchedule.ts` 匯出單一 `SessionFamilyId` 允許清單常數；`SessionRunner.ts` 改為 import，刪除本地重複定義。
- [ ] `metadata.ts` 的 `requireSessionPlanFamilyOrder` 改用同一份允許清單驗證。
- [ ] 新增回歸測試（`metadata.test.ts`）：`sessionPlanFamilyOrder` 含 `'peek-click-transfer'` 時 `buildMetadata()` 不再 throw。
- [ ] 既有四家族的 `sessionPlanFamilyOrder` 正向/負向 case（`metadata.test.ts`）零修改全綠。
- [ ] `sessionPlanPresets.test.ts` / `SessionRunner.test.ts` 既有行為零修改全綠（不得動到凍結順序邏輯）。
- [ ] `npm.cmd test -- src/session/sessionSchedule.test.ts src/session/SessionRunner.test.ts src/data/metadata.test.ts` exit 0。
