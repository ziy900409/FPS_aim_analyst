...
| KI | 症狀 | 修復決策 | 狀態 |
|---|---|---|---|
| [KI-039](KI-039-session-plan-weapon-select-value-assigned-before-options.md) | Session Plan 自訂 program 的逐列武器 `<select>` 在任何 `<option>` 建立**之前**就指派 `value`（[`SessionPlanSetup.ts:408`](../../src/ui/SessionPlanSetup.ts) 修前），HTML selectedness 規則讓該指派被**靜默丟棄**（`selectedIndex` 停 -1），隨後插入的第一個 option「—（drill 預設）」自動被選取。`renderItems()` 在新增／刪除／▲▼／拖曳時重建每一列 ⇒ **每次重繪都把所有武器選單的顯示重設回「drill 預設」，而閉包裡的 `item.weaponId` 原封不動**。後果一：weapon-pinned drill（`tracking_core_pr_3deg_14dps_feedback_v1` 等）上產生**無法從 UI 清除**的編譯失敗死結 —— 清除覆寫需發一次 `change` 到空值，但選單已經顯示空值，重選同一項不觸發事件。後果二（更危險，靜默）：殘留 weaponId 對該 drill 合法時，編譯通過、直接開跑，受測者用的是操作員沒看到的武器，而 `CompatibilityKey` 含 `weaponId` ⇒ 趨勢線被靜默切成兩群。不影響 sim／命中／決定性／schema。使用者 2026-09-14 於另一台 Chrome 實測回報 | `BD-037` —— 採**選項 A**：`weapon.value = …` 移到 option 迴圈之後（與同檔案 drill picker `:287` 的既有寫法對齊）。同時修掉讓它藏了六次綠燈的成因：`FakeElement` 的 `value` 改 accessor，`<select>` 只接受當下已有 `<option>` 承載的值 | ✅ 已修 + 落地（2026-09-14；`main`） |
| [KI-038](KI-038-spider-v3-peripheral-side-hardcoded-collapses-lr-aggregates.md) | `spider-shot-v3`（`center-peripheral-eye-stratified`）的**周邊** spawn 在 [`TargetManager.ts:486`](../../src/sim/TargetManager.ts#L486) 寫死 `side: 'R'` —— 三份真人匯出的**全部 119 個周邊 `visible` 事件 `side` 相異取值數 = 1**。下游 `PeekWindowTs.side` → `curve-v1` 的左右分群因此整組塌到單邊：實測 `omega.left.n === 0`／`omega.right.n = 74/78/86`，而 `NormalizedCurve` 的 `n: 0` 是**合法值**、`flagCounts` 沒有任何欄位說「左側整組不存在」⇒ 與「這位受試者從不往左打」完全不可分（KI-031 同型的靜默歸零）。對照組就在同一個函式裡：`wide-v1` 的 `sampleSpiderWidePeripheralPose()` 回的是真的 `cell.side`（[`:551`](../../src/sim/TargetManager.ts#L551)）。根因是 v3 的分層軸是 `azimuth × cosRadius`、**沒有 side 這一維**，呼叫端於是填了佔位值卻沒有機制讓下游知道。正確的 v3 左右來源已存在 = `deriveSpiderShotTransitions()` 的 eye-frame `side`（實測 L/R 均衡 18/19、19/20、21/22）。`v1/v2` 同樣寫死 ⇒ 這是**整個 spider 家族**的問題。今日無錯資料流出（左右曲線零消費者） | 尚無 `BD-036` —— 修改計畫見 KI doc §4：建議**選項 A**（`visible.side` 改 optional、v3/v1/v2 一律省略而非填 `'R'`、`curveRows`/`syncRow` 缺 side 時**不出列**並記 `no_side_channel` 旗標）。⚠️ 動到 schema 與 `curve-v1` 輸出形狀 ⇒ **C-D5 兩端同步 + golden 重跑 + 版本升號**，而 **[KI-032](KI-032-promoted-golden-parity-asserts-bit-equality-on-floats.md) 未修前 golden 無法在本機驗證** ⇒ 硬相依 KI-032 → 本 KI。**選項 B（在 sim 側由 `pos.x` 符號填 side）已否決** —— 那是 C-D4 的第二定義。選項 C = 不修只在分析端標註（本次落地的做法：報告的方位分箱一律走 eye-frame `side`，並回報 `rawSideDistinctValues` 讓佔位值看得見） | 🔴 診斷完成（2026-09-09，stage14 v3 真人報告），修法被 KI-032 排序阻塞 |
| [KI-037](KI-037-valid-duration-includes-countdown.md) | `validDurationMs()`（[`DrillMetricRegistry.ts:427`](../../src/history/DrillMetricRegistry.ts#L427)）= **最後 tick − 第一 tick**，含 `timing.countdownMs = 3000` 的開場倒數與 1.4–2.1 s 的尾段 tick，而分子（周邊命中窗數）全部落在 60.0 s 計分窗內 ⇒ **主指標** `spider-v3.peripheral-hits-per-minute` 恆向低估。三份真人匯出實測分母 65.148／64.656／64.406 s vs 協定 60.0 s ⇒ **+8.6%／+7.8%／+7.3%**（HANDOFF 預估 8–9%，實測略低）；33.2/36.2/40.1 應為 36.0/39.0/43.0 hits/min。同 cohort 內排序不受影響（偏差同向），但**絕對值不可對外報數**，且低估幅度逐場差 1.3 個百分點 ⇒ 也是一個與表現無關的雜訊源，未來真 MDC 會把它算進雜訊帶。`spider-shot-v2` 同受影響（v3 是 v2 的機械 rename）。其餘四個指標無時間分母，不受影響 | 尚無 `BD-041` —— ⚠️ **不得就地改 `validDurationMs()` 語意**：v2/v3 皆為凍結協定，既有趨勢點用現行分母算出，而 `CompatibilityKey` 十欄**沒有一欄會因此改變** ⇒ 不可比的兩組數字會被靜默合併成同一條趨勢線。修改計畫見 KI doc §4：建議**選項 A**（新增 `…-scored` descriptor、分母自凍結 `endCondition.value` 讀、registry version 1.0.0 → 1.1.0、與舊值並存）。選項 B（改用實測計分窗）不比 A 省。選項 C = 不修只標註（本次落地的做法：報告對每個速率量**同時給兩個分母**並在方法欄寫明差異來源）。**排序建議：併入 OQ-66.2 的 descriptor 重選一起做**，單獨加一個會讓「五個指標」這個已寫進三份設計文件的說法立刻過期 | 🔴 診斷完成（2026-09-09，stage14 v3 真人報告） |
| [KI-036](KI-036-registry-projection-failed-hides-which-precondition-broke.md) | `DrillMetricRegistry.project()` 的 `catch` 是**單一 catch-all**（[`:413`](../../src/history/DrillMetricRegistry.ts#L413)，連 `error` 都沒綁定）：`meta.session.participantId` 缺席、`weaponId`/`movementModel`/`fovDeg` 缺席、`protocolGuard.noMovement` 不為真、`targets.hitbox`/`scene.eye` 缺席、**宣稱角徑與匯出 hitbox 不符**（一道真正的資料完整性檢查）等**至少七類**失敗共用 `reasonCode: 'projection-failed'`。三份真人 `spider-shot-v3` 匯出全數落在這裡，實際真因只有一項 —— **`meta.session` 整個區塊缺席**（錄製時未填 SessionSetup 的受試者代號，與 `meta.dpi` 同型：瀏覽器讀不到、只能人填）。五個 descriptor 的值**本身完全算得出來**（`registration.project()` 不需要 compatibility key）⇒ 失效的是 metadata 不是指標，但輸出上不可分。後果：整批 run 靜默退出趨勢、`HistoryTrend` 只顯示「沒有這幾場」、`deriveSessionId()` 也擋住。與 KI-031 同型（合法值掩蓋可修的問題，CI 全綠）。⚠️ HANDOFF §9 的 DoD「三份都回 `ready`」是**未實跑 registry** 寫下的預期，照抄會得到永遠打不了勾的 DoD | 尚無 `BD-040` —— 修改計畫見 KI doc §4：**①（診斷面）+ ③（蒐集面）兩面都要處理**，只做其一無效。建議**選項 A**（前提擲帶結構的 `MetricPreconditionError`，`catch` 回 `{ reasonCode: 'missing-precondition', field }`；不新增狀態、不改既有三個 code 的意義，只把 `projection-failed` 縮小成「真正未預期的例外」）+ **選項 C**（Assessment 模式沒填 `participantId` 不讓開跑）。選項 B = 只在離線分析端補診斷（本次落地的做法：`spiderShotV3CoachRunner.ts` 的 `diagnoseProjectionFailure()` 逐條複驗前提並點名缺了什麼）——**這是繞道不是修復**，它是 registry 前提的第二份清單，A 落地後應刪掉 | 🔴 診斷完成（2026-09-09，stage14 v3 真人報告），修法待落地 |
| [KI-034](KI-034-prestimulus-baseline-overlaps-prior-engagement.md) | `deriveDetectionMetrics()` 的預設 `preStimulusMs: 500` 在快速連續呈現中把前一次拉槍當成即將出現目標的 baseline：三份 240 Hz 真人 `spider-shot-wide-v1`、132 個周邊呈現，500 ms 只 detected **50/132**（baseline SD 中位 140.9 deg/s、threshold 422.7），120 ms 敏感度分析為 **132/132**（SD 8.0、threshold 23.9）。被預設漏掉的 82 筆 reaction 中位 **171.9 ms**，原本留下的 50 筆為 **234.4 ms**；原有配對 trial 也逐筆晚約 35.2 ms ⇒ 非隨機漏失、反向懲罰快表現。82 個 timeout 的 `baselineInsufficient` 全為 false，故與合法「受測者沒反應」不可分。與 KI-031 不同：500 ms 把 `sustainedTicks` 降到 1 仍僅 117/132；120 ms 下 1..6 ticks 全 132/132，主因是 baseline 時段而非連續性 | 尚無 `BD-038` —— 先加 additive、reason-coded baseline provenance/validity 並 fail closed；正式修法建議以明示 quiet/calibration interval 建 session-level noise floor，或只用前次 terminal 後 quiet interval且不足就不可推導。**禁把 120 ms 當正式修法**（仍屬前次 presentation 尾段且本批全被 `baselineInsufficient` 標記）。TS/Python parity + operational spec + fixtures 必須同步升版，並與 KI-031 分別保留低更新率/短週期 acceptance case | 🔴 診斷完成（2026-09-10，3 份 240 Hz 真人匯出）；修法待決策 |
| [KI-033](KI-033-repositioning-flag-omits-acquisition-failures-and-undercounts-stall-start.md) | 抬滑鼠疑慮旗標兩處**系統性漏報**。① `arrivalMs === undefined`（該次抵達整段沒有任何 `onTarget` 樣本 = acquisition failure）被輸出成 `suspected: false`，與「窗可推導且窗內無停滯」**共用同一個編碼** ⇒ 「無從判定」被歸進「乾淨」，而「拉槍拉不到目標」正是抬滑鼠最強的行為訊號之一，方向恰好最壞。與 [KI-031](KI-031-detection-sustained-ticks-dies-when-aim-updates-slower-than-sim.md) 耦合後放大：真人 run 的 `detected = 0/113` ⇒ `onsetMs` 幾乎全 `undefined`，旗標在真人資料上**近乎恆偽**卻看起來像「量過且乾淨」。② `omega[i]` 描述的是區間 `(t[i−1], t[i]]`，但區段起點取「首個合格樣本的時間戳」⇒ 第一個合格區間的左端整段丟掉，長度少算最多一個 tick（128 Hz = 7.8125 ms，對 `stallMinMs=150` 為 **~5.2% 恆向低估**）。兩項皆漏報方向 ⇒ 無假陽性、零 `src/` importer（C-D3 未違反）、今日無錯資料流出。①在合成 cohort 上**永不觸發**（合成資料每次抵達都會 on-target），這正是 19 個既有測試沒測出來的原因 | 尚無 `BD-033` —— ⚠️ **雙重排序閘**：本修法不得在 **WP-61 T3 判定產出前**落地（[GD-37 ②](../exec-plan/DECISIONS.md) 把本函式語意與真人校準值 `stallMinMs=150`／`stallOmegaDegPerSec=2` 凍結為 sensor lift 構念的邊界前提，WP-61 DoD 要求「語意一行未改」），且 ① 須待 **KI-031** 修好才能在真人資料上驗證。修改計畫見 KI doc §4：建議**選項 A**（採納 archive 版的兩個判定，保留 `main` 的簽章與真人校準值）；選項 B = 加第三態（上游 `acquisitionFailure` 旗標可用，但改動已凍結的輸出形狀須另立 GD）；選項 C = 不修只註記。另一份實作已存為本機 tag `archive/ki-033-wp-57-t5-alt-impl`（→ `a0437ae`，remote 分支已刪） | 🔴 診斷完成（2026-09-09，審分支合併時發現），修法**被 WP-61 T3 與 KI-031 排序阻塞** |
| [KI-032](KI-032-promoted-golden-parity-asserts-bit-equality-on-floats.md) | C-D5 晉升指標的四個 Python golden 測試以 **dict `==`（浮點逐位相等）**比對 committed golden ⇒ 在錄製機以外的任何機器必紅。`main`（`445fb7c`）實測 `uv run pytest` **4 failed／488 passed**，差異全落在尾位（最大相對偏差 **~2e-14**，如 `half_delta` 0.011883647677947917 vs …695）；**所有決定性欄位逐位相同**（`verdict` 三筆全 `research_only`、`tickRange`、`gate_thresholds`、`xcorr_params`）⇒ 無任何結論或門檻翻面，**不需重跑分析或撤回宣稱**。根因為 numpy／scipy wheel 的 OpenBLAS **`DYNAMIC_ARCH`**（SIMD kernel 由執行期 CPU 決定）⇒ 逐位相等不是這條鏈可滿足的契約。已實測排除：版本漂移（`uv.lock` 自 `012eddc` 未更新，`.venv` 與 lock 一致）、產生器被改（`git log 8f8048c..` 零 commit）、BLAS 執行緒 reduction order（1／2／4 皆同樣失敗）、本機非決定性（連跑三次 sha256 完全相同）。同檔內即有對照：量同一統計量的 `pytest.approx(…, abs=1e-12)` **通過**、逐位 `==` **失敗** ⇒ 失效在比對紀律不在演算法。真正的損害是 **C-D5 的 Python 半邊守門能力已喪失**（真回歸會被當成「又是那四個浮點紅燈」放行，與 KI-030 對 Playwright 同型），且任何寫「`uv run pytest` exit 0」的 DoD 皆無法達成 | 尚無 `BD-032` —— 修改計畫見 KI doc §4：建議**選項 A**（共用 `assert_golden_close(rtol=1e-12)` helper：float 走 `math.isclose`，`int`／`str`／`bool`／結構鍵**維持逐位相等**）。**不重錄 golden**（只會把紅燈移到下一台機器，並稀釋 golden 作為回歸基準的意義）；**不動任何 `version` 字串**（C-D5：只能升版不得原地改語意）。選項 B（釘 wheel）無效——`DYNAMIC_ARCH` 是執行期分派；選項 C（`xfail`）會連真回歸一起吞掉 | 🔴 診斷完成（2026-09-09，審分支合併時發現），修法待落地 |
| [KI-031](KI-031-detection-sustained-ticks-dies-when-aim-updates-slower-than-sim.md) | `firstSustainedDecrease()` 要求 **`sustainedTicks`（預設 4）個連續**樣本低於門檻，隱含「每個 sim tick 的 `aim` 都會更新」這個從未寫下的前提。在 aim 更新率低於 128 Hz 的機器上（本例 60 Hz 顯示 ⇒ 有效 ≈64 Hz），負值與 0 **嚴格交替**，連 2 連都湊不出來。四份真人 `spider-shot-wide-v1` 匯出實測 **detected = 0/113**;`sustainedTicks: 1` → 111/113、2/3/4 → 0/113。**不是門檻造成的**（門檻降到 2.2 deg/s + `sustainedTicks: 2` 仍 0/34）。下游後果：`switchReaction`、`movementExecution.movementTimeMs` 在真人 run 上 **0/34**，而 `status: timeout` 是合法值 ⇒ 與「受測者沒反應」**完全不可分**,CI 全綠。144 Hz 以上不可見、60 Hz 硬性失敗的**懸崖**,故既有 fixture 從未現形 | 尚無 `BD-031` —— ⚠️ **C-D5 雙實作**（TS `detectionDerivation.ts` ↔ Python `detect.py` + 凍結的 `detect-parity` golden），任何判準變更須兩端同步並升版。修改計畫見 KI doc §5：① 先加 additive 的取樣充足性旗標讓失效可見（不動語意、不影響 parity）;② 判準由「連續 N tick」改為「持續 N 毫秒」（建議）;③ 重跑 golden;④ 補一份 60 Hz 回歸 fixture。繞道 = 以 `sustainedTicks: 1` 呼叫並標註 | 🔴 診斷完成（2026-09-08，WP-57 T5 真人校準） |
| [KI-030](KI-030-history-e2e-flaky-under-parallel-workers.md) | 同一 HEAD 連跑三次全量 Playwright 得到兩種結果：預設 worker 數首次 **87 passed／4 failed**，`--workers=1` 與第二次預設執行皆 **90 passed／1 failed**。多出來的 3 個是 `history-library.spec.ts:196`／`:288` 與 `history-persistence.spec.ts:44`，三者單獨重跑全綠。已排除 production 缺陷（隔離與單 worker 皆過）、資料安全（真實 `data/session-history/` 前後皆 41 個目錄、無新增）、KI-028（本次 5173／4173 事前確認無人佔用，兩個 server 皆由 Playwright 自起）與本次程式碼變更（WP-57 T5 只加一個零 importer 的離線模組）。⇒ `npm run test:e2e` 目前**不是可重現的門檻**，真正的回歸可能被當成「又是那三個 flake」放行 | 尚無 `BD-030` —— **根因未定**：失敗當下的 `test-results/**/error-context.md` 已被後續隔離重跑覆寫，兩個假設（跨 worker 共用 history temp root 互相干擾 / 首次執行時 preview 的 `npm run build` 造成冷啟動時序）無法分辨。修改計畫見 KI doc §4／§6：**先保存錯誤文字**再動測試；修好後須連跑三次全量且結果一致才可關閉 | 🔴 觀察完成、根因未定（2026-09-08，WP-57 T-exit） |
| [KI-029](KI-029-prop-clearance-inflates-sphere-hitbox-to-box-corner-radius.md) | `clearance.ts` 的 `targetHitboxRadius()` 回**box 角點半徑**（半對角線），卻被 `validateClearance()` 無條件套用在 `shape:'sphere'` 的 hitbox 上 ⇒ prop 淨空膨脹量高估 `√3×`（`0.732·r`）。這是 **KI-021 同一病理的第三處、也是唯一未修的一處**（引擎 `HitDetector` 與 KI-021 修好的離線推導都已 shape-aware）。**方向保守**：只會誤拒、不會產生錯資料，故從未在任何資料稽核中現形。今日唯一可達組合 = WP-54 tracking pilot（sphere）× `field-low`（16 props），閘比宣稱的幾何**嚴 9–13%**（3.0° 檔 +0.0767 u、2.0° 檔 +0.0511 u），實測仍全綠 ⇒ 無觀測失敗。真正的風險是反方向：WP-57 T3 已另立 shape-aware 的 `spiderWideTargetRadiusU()`，「為了一致性」把任一邊改成另一邊都會踩到 | 尚無 `BD-029` —— 修改計畫見 KI doc §4：建議**選項 A**（`targetHitboxRadius()` 改 shape-aware、WP-57 那份改為呼叫它以消掉第二定義），代價是 sphere drill 的 prop 淨空刻意放寬 9–13%，須先答 OQ-KI29-1（量出 `field-low` × pilot 的最小餘裕）。選項 B = 只記載「box 專用」保留兩份定義；選項 C = 不修 | 🔴 診斷完成（2026-09-07），修法待落地 |
| [KI-028](KI-028-capture-script-orphans-dev-server-hijacking-e2e-history-root.md) | `scripts/capture-wp56-visuals.mjs` 在 Windows 以 `cmd.exe` wrapper 起 dev server，`finally` 的 `server.kill()` 只殺最外層 ⇒ npm→cmd→vite 三層孤兒化、5173 持續被佔（實測 PID 祖先鏈）。之後 `npm run test:e2e` 依 `reuseExistingServer` **靜默重用**它，整個 suite 改跑 `FPS_HISTORY_ROOT=.wp56-capture-tmp/history` 而非 config 的 `.playwright-tmp/history-dev`。測試全綠所以無聲；後果是 NFR-48.6／FM-48.4 的 dev/preview 雙 root 前提被換掉、root-lock（HTTP 423）失敗停止重現而可能被誤讀為已修、history-root 相依的 e2e 證據落在未宣告組態 | 尚無 `BD-028`——修改計畫見 KI doc §4：win32 tree-kill 或去掉 shell wrapper（真的殺掉 server 樹）＋ capture server 改用非 5173 埠（即使留下孤兒也不可能被 Playwright 誤採）。production runtime 0 改動 | 🔴 診斷完成（2026-09-07），修法待落地 |
| [KI-027](KI-027-overlay-layering-researcher-submenu-guard-dead.md) | `overlay-layering.spec.ts` 展開研究員子選單後的斷言寫死 7 顆按鈕，但 WP-54 / T6 加入第 4 個子選單項後實際是 8 顆；helper 在數量檢查就 `return null`，**重疊過濾根本沒執行** ⇒ KI-003 的不變式自 WP-54 起未被真正驗證，而錯誤訊息 `Expected [] Received null` 看起來像重疊結果、會誤導。這是目前 `npm run test:e2e` 唯一的紅燈（86 passed／1 failed）；WP-56 T0 在 production diff=0 時即記錄過 | 尚無 `BD-027`——修改計畫見 KI doc §3：常數 7→8，**並且**讓 helper 回傳結構化結果（`{count, overlapping}`）把「數量不符」與「發生重疊」分成兩句斷言，消除短路失效模式。probe 已量測確認 layout 未回歸（8 顆相交清單為 `[]`、面板 top 188→405.5），錯的是常數。production 0 改動 | 🔴 診斷完成（2026-09-07），修法待落地 |
...
| [KI-012](KI-012-spider-shot-target-occluded-by-placeholder-room-back-wall.md) | WP-46 T-exit 手動驗收回報「spider-shot-v2 沒有看到任何球體」——追碼證實 v1 的方塊目標在同一位置也一樣看不到:`placeholder-room` 北牆(z=-5)比 spider-shot 目標距離(z=-8)更靠近相機,整顆目標(不分形狀)被牆體完全遮擋;這正是 WP-5 T1 早已文件化過但被 WP-36 重蹈的坑 | BD-012(§3,只放大 `placeholder-room` 的 `roomSize` depth 10→20(北牆退到 z=-10),明確釘住 `eyeZ:4` 避免連動改變 camera/raycast 原點;2026-09-01 後續視覺調整 width 10→16、height 3→6;不動任何 drill config 凍結值) | ✅ 已修(2026-08-26;2026-09-01 視覺調整) |
...
| | |
|---|---|
| KI | [KI-024](KI-024-field-low-eye-not-anchored-halves-delivered-angles.md) |
...
| **狀態** | ✅ F-A1（slice 6）+ F-A2（slice 9，研究者選定「放寬角度視窗至 ±13°」＋建構期一致性守衛）皆已落地（2026-09-03）。殘差（medium 交付 36 次 vs 宣稱 23 次，源於設計本身的邊界截斷）記於 KI-019 §5.3，是否再放寬到 ±25° 留給 T7。 |
...
### BD-037 ✅ KI-039 — Session Plan 逐列武器 select 的 value/option 指派順序（2026-09-14）

| | |
|---|---|
| **發現處 / 根因** | [KI-039](KI-039-session-plan-weapon-select-value-assigned-before-options.md) / 使用者 2026-09-14 在另一台機器的 Chrome 實測回報「Session Plan 預覽畫面消失」。根因：`renderItems()` 建立逐列武器 `<select>` 時，`weapon.value = item.weaponId ?? ''` 寫在所有 `weapon.appendChild(option)` **之前**。HTML selectedness：沒有相符 `<option>` 時 `selectedIndex` 停 -1、指派靜默丟棄，隨後插入的第一個 option（「—（drill 預設）」）自動被選取 ⇒ 每次重繪都讓**顯示**與 `item.weaponId` 脫鉤。真 Chromium 兩種順序對照實測：value 先 → `''`；option 先 → `usp_s_laser`。 |
| **決策(修法選項)** | 採 **選項 A**：把 `weapon.value = …` 移到 option 迴圈之後（一行位移，與同檔案 drill picker `:287` 的既有寫法對齊）。**不採 B**（逐 option 設 `selected`）—— 等價但要自行處理「預設」項，多一個分支、無收益。**不採 C**（重繪時保留舊 DOM、只更新變動列）—— 那是 `renderItems()` 的架構改寫，遠超本 bug 範圍，且 NFR-58.4 實測 p95 1.7ms／50ms 預算顯示全量重建毫無壓力。 |
| **理由** | 錯的只有「操作員看到什麼」：`item.weaponId`、送出的 plan、預覽所編譯的 program 三者一直一致（同源）。修法因此應該最小、只動顯示層，不得為此改動 program 編譯或列表重繪的既有語意。 |
| **偏離計畫** | 無偏離協議。依 BD-001 慣例：測試於工作區證實修前為紅（`expected '' to be 'usp_s_laser'`、`expected '' to be 'm4a1s'`），測試與修法合併為單一已驗證綠的 commit。**額外動到測試替身**：`FakeElement` 的 `value` 由純欄位改為 accessor，`<select>` 只接受當下已有 `<option>` 承載的值（遞迴含 optgroup）—— 這不是順帶重構，而是讓回歸測試能成立的前提：假 DOM 不跑 selectedness 演算法，正是這個 bug 自 `2806c37` 起藏過六次綠燈的原因。 |
| **遺留 OQ / 未做** | **OQ-KI39-1**：`FakeElement` 在 23 個測試檔各有一份複本，本次只讓 `SessionPlanSetup.test.ts` 那份忠實；其餘 22 份仍會原諒同類順序錯誤。是否抽成共用測試工具？**刻意不做** —— 跨檔重構不該搭 bug 修復的便車。**OQ-KI39-2**：修前若已有以「看不見的武器」跑出的 custom Session Plan 匯出，需逐份以 `meta` 的 `weaponId` 對帳；無法從既有資料反推操作員當時看到什麼。 |
| **影響面** | 改 [src/ui/SessionPlanSetup.ts](../../src/ui/SessionPlanSetup.ts)（一行位移 + 註解）、[src/ui/SessionPlanSetup.test.ts](../../src/ui/SessionPlanSetup.test.ts)（`FakeElement` select 模型 + 2 個回歸測試）。**不動** sim／命中／決定性／schema／event 對表，亦不動 `sessionProgram.ts` 的編譯與 `requireWeapon()` 語意。驗證：`vitest run` 265 files / 3205 tests passed、`tsc --noEmit` 與 `-p tsconfig.node.json` 皆 exit 0。 |
| **狀態** | ✅ 已修 + 落地（2026-09-14；branch `main`）。全 repo 其餘 select（同檔 drill picker `:287`、`SettingsPanel`、`TrackingPilotOperatorScreen`、`Controls`）順序均已掃過，皆為 option 先、value 後，無同類缺陷。 |
...
| **決策** | 比照 KI-012 做法:`SceneConfig.ts` 的 `ProceduralRoomConfig` 新增選填 `floorY?: number`(比照既有 `eyeZ?` 慣例,省略=0 逐位不變);`SceneManager.ts` 的 `#buildRoom()` 用 `floorY` 定位地板,並讓四面牆下緣跟著一起下移(牆高=`height-floorY`,置中於 `[floorY,height]`,上緣當時仍固定於 `height=3`,不影響 KI-012 §6 OQ-KI12-1 記錄過的「周邊目標探出牆頂仍可見」行為);`placeholder-room.ts` 設 `floorY:-3`,覆蓋 v2 實測最深 y≈−1.99,留約 0.87 安全邊界。2026-09-01 使用者依 spider-shot-v2 目視回饋拍板後續視覺調整:`roomSize` 改為 `[16,20,6]`,只拉寬左右邊界與牆上緣,不改 `floorY`、`eyeZ` 或 spider-shot config。 |
...
| **影響面** | **受影響**:`placeholder-room` 場景的地板/牆體下緣視覺呈現;2026-09-01 後續調整另影響房間寬度與牆高視覺。**不受影響**:`spider_shot_v1.ts`(協定凍結,零改動)、`spider_shot_v2.ts`、`TargetManager.ts`/`HitDetector.ts` 任何判定邏輯、`clearance.ts` 驗證結果、`placeholder-room` 上其他既有 drill 的 camera/raycast 原點、匯出資料格式。`floorY` 為新增選填欄位,省略時對其他所有場景逐位不變。 |
...
| **決策** | 只放大 `placeholder-room` 的 `roomSize` depth(10→20，北牆退到 z=-10，給 distance=8 目標 2u 淨空)，並新增顯式 `eyeZ:4` 釘住 `resolveEyeWorldBase()` 原本 `depth/2-standoff` 的 fallback 值——避免 depth 改動連動搬動 camera/raycast 原點，牽動 `placeholder-room` 上其他既有 drill 的命中判定距離。2026-08-26 當下只改 depth，不改 width/height；2026-09-01 使用者依 spider-shot-v2 視覺回饋另行拍板 width 10→16、height 3→6，仍不改 `eyeZ` 或任何 drill config。 |
...
| **遺留 OQ** | ~~**OQ-KI12-1**~~ ✅ 關閉(2026-09-01):`placeholder-room` height 3→6,目前 v2 25° 周邊目標不再探出牆頂;仍不加天花板。**OQ-KI12-2**：`clearance.ts` 仍不查房間牆體幾何，只查 `propBounds`；若日後新 drill 又選到超出房間邊界的 distance，同一類坑可能第三次發生，是否要補一個房間邊界檢查屬獨立架構決策。**OQ-KI12-3**：`DEFAULT_PROCEDURAL_ROOM`(`eyePose.ts`)與 `field-low`/`urban-high` 的 `roomSize` 仍是舊值(depth=10)未動——這些場景要嘛是 GLTF 資產(牆體來自 `.gltf`，不受 `roomSize` 影響)要嘛實務上未曾以 `asset:null` 形態建構，本次未發現受影響路徑。 |
| **影響面** | **受影響**：`placeholder-room` 場景的房間深度視覺呈現;2026-09-01 後續調整另影響房間寬度與牆高視覺。**不受影響**：`spider_shot_v1.ts`(協定凍結，零改動)、`spider_shot_v2.ts`、`TargetManager.ts`/`HitDetector.ts` 任何判定邏輯、`clearance.ts` 驗證結果、`placeholder-room` 上其他既有 drill 的 camera/raycast 原點(`eyeZ` 明確釘住)、匯出資料格式。 |
