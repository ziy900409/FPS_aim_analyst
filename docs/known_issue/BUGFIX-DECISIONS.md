...
| [KI-005](KI-005-omega-render-sim-aliasing.md) | ω(t) 受 render(240Hz)/sim(128Hz)**zero-order-hold aliasing** 汙染:每 8 tick 一個假凹口 → `merged_adjacent_peaks` 15/19,有效產率僅 4/19。**推翻 KI-004「①③④⑤⑥ 不受影響」的豁免,M14 ③④⑤ 撤回** | BD-005(§2,**A1 已落地**:選項 A + 感度由 meta 重建 + 不做過渡期 C;**A2-T1/T2/T3 已完成**,FM-1 關閉、`seg-v2` 凍結)· 計畫 [KI-005-A/](KI-005-A/README.md) | 🟡 A1+A2-T1/T2/T3 已完成,A2-T4(M14 重新宣告)待辦 |
...
| **遺留 OQ** | ~~**OQ-KI6-1**~~(收斂)A 已出局,剩 B;✅ **時機與規模已決(2026-08-07)**:與 KI-005 A2-T1 合併為同一次採集,n > 2 session —— 驗收清單見([KI-006-C/README.md §6](KI-006-C/README.md) B-1~B-5),**實際執行仍待研究者排程** · ~~OQ-KI6-2~~ ✅ 關閉(2026-08-06):construct presence gate 納入本輪 · ~~OQ-KI6-3~~ ✅ 關閉(2026-08-07):門檻以 `construct-v1` 凍結,證據見上 · ~~**OQ-KI6-4**~~ ✅ **關閉(2026-08-07)**:**n > 2**(至少 3 個 session),嚴於原建議的 n ≥ 2([KI-006-C/README.md §6](KI-006-C/README.md) B-4)。 |
...
| **狀態** | ✅ **A1 已落地(2026-08-06),A2-T1/T2/T3 已完成(2026-08-07)**。根因經測試 A 證實,修法依 2026-08-06 拍板(A / meta 重建 / 不做 C)落地;真實硬體資料複驗支持修法有效,FM-1 關閉;`seg-v2` 已重掃凍結並在真實資料上驗證優於 `seg-v1`。**下一步:[A2-T4](KI-005-A/A2-blocked-plan.md#a2-t4--m14-重新宣告) M14 ③④⑤ 逐項重新宣告**。A1+A2-T1/T2/T3 交付的是量測儀器的正確性且已在真實硬體上驗證有效,**M14 ③④⑤ 於 A2-T4 完成前仍維持撤回**。 |
