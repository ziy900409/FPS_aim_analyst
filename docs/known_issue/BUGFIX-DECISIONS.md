...
| **狀態** | 🟡 **A1 已落地(2026-08-06)**;根因經測試 A 證實,修法依 2026-08-06 拍板(A / meta 重建 / 不做 C)落地。**A2-T1 前置決策已於 2026-08-07 拍板並全數落地**(合併採集、`recordKeyEvents` 開且已接線、n > 2)。**A2(新採樣 → 複驗 → `seg-v2` 重掃 → M14 重新宣告)僅剩研究者排程**,見 [A2-blocked-plan.md](KI-005-A/A2-blocked-plan.md)。A1 交付的是量測儀器的正確性,**不是效度恢復**——M14 ③④⑤ 仍撤回。 |
