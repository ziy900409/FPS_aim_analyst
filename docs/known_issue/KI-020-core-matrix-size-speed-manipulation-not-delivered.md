...
> ⚠️ **[KI-023](KI-023-target-speed-set-point-is-per-axis-not-2d.md) 事後更正**：上面「5.05 / 20.21」
> 是**每軸** RMS，不是螢幕上目標的速度。當時的測試與這裡的量測都只看單軸，因此看起來已經修好；
> 兩軸合成後實際交付 **7.14 / 28.3 deg/s**（√2 倍）。KI-023 已把 `targetRmsSpeedDegPerSec` 改為
> 2D 語意（每軸目標 = set-point / √活躍軸數），修後實測 1.000–1.014。本節其餘結論（頻帶提高、
> 共用振幅、speed 為乾淨的 4× 因子）不受影響；**振幅在修後同步縮為 1/√2**（20 deg/s cell 走到
> 約 ±9–11° 而非 ±13°），離地板與 FOV 邊界更遠。
