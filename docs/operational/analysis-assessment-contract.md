...
| `events.target_stop.t` / `tStop` | WP-35 additive event; do not redefine | `TargetManager` writes the sim-clock stop witness; `SimLoop` exports one `DrillEvent.type === 'target_stop'` with frozen target coordinates | The tick where a hold-track target stops moving and its fire gate is released; it anchors stop-to-first-shot metrics. Counter-strafe retains its separate velocity-derived stopping semantics. |
