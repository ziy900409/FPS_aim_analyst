...
**進行中（`active/`）**：[`active/stage4/`](exec-plan/active/stage4/README.md) = **階段 D 選手表現分析管線**（✅ 採納 2026-08-04，[DECISIONS.md](exec-plan/DECISIONS.md) **GD-19**/**GD-20**；WP-28~32 / M14~M15 / 驗收清單 D）。新增 **`research/` Python 離線分析層**（Python 3.12 + uv，四目錄制學 performance_analysis），引擎零改動；`research/` ↔ `src/` **單向隔離**，parity **雙向**（既有構念 ε(t)/t_acquire 以 TS + `docs/operational/analysis-*.md` 為權威；新構念 Python 為權威）且兩向對表閘皆落在既有 `npm run test:ci`。當前：[WP-28 research-foundation](exec-plan/active/stage4/wp-28-research-foundation/README.md)（M14，T0–T4 ✅ + T-exit 交付物 ✅；**M14 ①⑥ 維持，② 已於 [KI-004](known_issue/KI-004-sim-world-unit-domain-mismatch.md) S1 落地後重新宣告(2026-08-06)，③④⑤ 仍撤回 —— [KI-005](known_issue/KI-005-omega-render-sim-aliasing.md) A1（量測儀器修法）已落地(2026-08-06)但 A2（新採樣＋複驗＋`seg-v2`＋M14 重新宣告）待排程、[KI-006](known_issue/KI-006-m14-sample-no-counterstrafe.md) 的 C（construct presence gate）已落地(2026-08-07)但 B（重新採樣）待 A2，兩條解除條件均未滿足（C 落地不解除任何 M14 撤回）→ WP-30/31 entry blocker 仍維持，不得開工**）。分段參數 registry、quality flags 詞彙表與一鍵 pipeline 契約見 [analysis-segments.md](operational/analysis-segments.md)。
...
| [known_issue/KI-005-omega-render-sim-aliasing.md](known_issue/KI-005-omega-render-sim-aliasing.md) | KI-005：ω(t) 受 render(~240Hz)/sim(128Hz) **zero-order-hold aliasing** 汙染 — 每 8 tick 一個假凹口，`merged_adjacent_peaks` 15/19、有效產率 4/19。**✅ 選項 A 已落地（A1，2026-08-06：tick 窗內積分 mouse delta + `meta.fovDeg`/`meta.mouseIntegration` + `omega_deg_s` 雙 source）；A2（新採樣 → 複驗 → `seg-v2`）待排程。碰 `ticks[].aim` 逐 tick 差分（ω/角加速度/jerk）或 `seg-*` 參數前必讀。既有兩份匯出的 ω(t) 一律不可用（不做回溯清洗），M14 ③④⑤ 仍撤回** |
| [known_issue/KI-006-m14-sample-no-counterstrafe.md](known_issue/KI-006-m14-sample-no-counterstrafe.md) | KI-006：M14 ④/⑤ 效度閘所用樣本（08:03）**不含 counter-strafe 構念** — `vx ≡ 0`、`keys` 全空、`counter` 事件 0。**🟡 C（construct presence gate）已落地（2026-08-07，[KI-006-C/](known_issue/KI-006-C/README.md)）；B（重新採樣）待 [A2](known_issue/KI-005-A/A2-blocked-plan.md)。C 落地不解除 M14 撤回 —— 挑選真實匯出樣本或宣告效度閘前必讀** |
