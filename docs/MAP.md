...
| [known_issue/KI-017-history-replay-tdz-referenceerror-on-early-replay-click.md](known_issue/KI-017-history-replay-tdz-referenceerror-on-early-replay-click.md) | KI-017：Run Detail「3D 重播」在 `main.ts` 頂層 `const replayController` 初始化完成前被點擊會拋 TDZ `ReferenceError`（與 KI-013 同一類根因、不同變數，發現於 WP-51 T4）。**🔴 診斷完成，修法待落地，owner=WP-50** |
