...
#### 階段 F（`completed/stage6/`，✅ 交付 2026-08-25：WP-33~39/M16 達成；[DECISIONS.md](exec-plan/DECISIONS.md) GD-22/GD-23）

> stage6 tech spec：[README.md](exec-plan/completed/stage6/README.md) · 需求草稿：[aim-assessment-framework-v1.md](exec-plan/completed/stage6/aim-assessment-framework-v1.md) · 驗收清單 F：[acceptance-stage-f.md](operational/acceptance-stage-f.md)。**WP-33~39 全部 T-exit ✅**;M16 驗收清單 F 全 12 項通過;`protocolVersion=1.0.0` 為無真人 pilot 資料下的暫定凍結（GD-23）。

| WP | 子資料夾 | 目標 | 里程碑 | 相依 |
|---|---|---|---|---|
| **WP-33** | [`wp-33-assessment-contract/`](exec-plan/completed/stage6/wp-33-assessment-contract/README.md)（✅） | 共同 Assessment/Practice 契約 + metadata + 事件時間線 + 品質旗標 | — | M4 + WP-20 |
| **WP-34** | [`wp-34-hold-click-visibility/`](exec-plan/completed/stage6/wp-34-hold-click-visibility/README.md)（✅） | 架槍 `hold-click-v1` + 遮蔽物可見度時間線 | — | WP-33 |
| **WP-35** | [`wp-35-hold-track/`](exec-plan/completed/stage6/wp-35-hold-track/README.md)（✅） | 架槍 `hold-track-v1`：移動期間鎖 fire + 追蹤窗指標 | — | WP-34 |
| **WP-36** | [`wp-36-spider-shot/`](exec-plan/completed/stage6/wp-36-spider-shot/README.md)（✅） | Spider Shot `spider-shot-v1`：單目標約束 + 中心—周邊排程 | — | WP-33 |
| **WP-37** | [`wp-37-counterstrafe-protocols/`](exec-plan/completed/stage6/wp-37-counterstrafe-protocols/README.md)（✅） | 急停三協定包裝（cued/reversal/free）+ 對稱指標 | — | WP-33 |
...
| [known_issue/KI-016-session-plan-family-order-validator-stale-allowlist.md](known_issue/KI-016-session-plan-family-order-validator-stale-allowlist.md) | KI-016：`metadata.ts` 的 `requireSessionPlanFamilyOrder` 仍寫死驗證 `TEST_FAMILY_IDS`，未跟上 WP-45 T5 在 `SessionRunner.ts` 建立的 `KNOWN_SESSION_FAMILY_IDS` 聯集 — 目前被「preset 切換未接進操作端 UI」意外遮住未觸發，一旦 `families` 含 `'peek-click-transfer'` 會在匯出時 throw。**🔴 診斷完成，修法待落地** |
| [known_issue/KI-017-history-replay-tdz-referenceerror-on-early-replay-click.md](known_issue/KI-017-history-replay-tdz-referenceerror-on-early-replay-click.md) | KI-017：Run Detail「3D 重播」在 `main.ts` 原頂層 `const replayController` 初始化完成前被點擊會拋 TDZ `ReferenceError`（與 KI-013 同一類根因、不同變數，發現於 WP-51 T4）。**✅ 已修復（2026-09-01，owner=WP-50）：`replayController` 提早宣告 + early-click 可見尚未就緒訊息 + Playwright regression** |
| [known_issue/KI-018-history-search-keystroke-focus-steal.md](known_issue/KI-018-history-search-keystroke-focus-steal.md) | KI-018：History「搜尋 Participant」逐字輸入時 `HistoryScreen.ts` 的 focus-on-navigation guard 用參考相等誤判 in-place `navigator.replace()` 為導覽，搶走焦點，遺失除首字元外的所有按鍵（發現於 WP-51 T4）。**🔴 診斷完成，修法待落地，owner=WP-49** |
| [known_issue/KI-019-reversal-2d-v1-bound-pinned-schedule-degeneration.md](known_issue/KI-019-reversal-2d-v1-bound-pinned-schedule-degeneration.md) | KI-019：`reversal-2d-v1` 的 leg 長度由兩軸共用、貼牆的軸把整個 leg 壓成 0、sign 又無條件翻面 ⇒「兩軸同側貼牆」成為吸收態，排程以 1ms 步長寫到結束——實測 `tracking_reversal_pilot_v1_medium` 產生 6644 筆 leg、32.6% 時間目標凍結在角落，WP-54 T6 首份真人資料該 block 作廢（發現於 T6 真人資料分析）。**✅ 已修（2026-09-03，F-A1 room-aware 方向選擇 + F-A2 視窗 ±13° + 建構期幾何守衛，BD-019）** |
| [known_issue/KI-020-core-matrix-size-speed-manipulation-not-delivered.md](known_issue/KI-020-core-matrix-size-speed-manipulation-not-delivered.md) | KI-020：WP-54 core 2×2「size × speed」matrix 兩個自變數**都沒有被交付**——`boundedSpeedScale` 靜默取 min 使 5 與 20 deg/s 都只交付約 1.2 deg/s，而「size」被接到行程振幅、沒有任何 cell 設 `targets.hitbox`（四個 cell 目標同大、TOT 全部 100%）。**✅ 已修（2026-09-03，size→真的角尺寸 hitbox、頻帶 [0.3,2.1]Hz + 共用振幅 ±16°、建構期速度守衛，BD-020）** |
| [known_issue/KI-021-tracking-derivation-ignores-sphere-hitbox-shape.md](known_issue/KI-021-tracking-derivation-ignores-sphere-hitbox-shape.md) | KI-021：on-target 離線推導（`trackingDerivation.isOnTarget()`）是 ray/AABB test 且 `hitboxFromMeta()` 丟掉 `shape`，`shape:'sphere'` 目標被當成外接立方體——與 `HitDetector` 的球體相交**不同幾何**，違反 GD-7 與 CONTEXT.md §23。**已在正式 Assessment drill 生效**（`spider-shot-v2` sphere 2.0°@8u，`firstOnTarget` 最多寬鬆 41%）。**✅ 已修（2026-09-03，BD-021：`HitboxSize` 帶 `shape` + `isOnTarget()` 加 ray/sphere 分支 + WP-55 閘門放寬並補三軸相等檢查；並依 [GD-30](exec-plan/DECISIONS.md) 把 WP-54 hitbox 改回 sphere）** |
| [known_issue/KI-027-overlay-layering-researcher-submenu-guard-dead.md](known_issue/KI-027-overlay-layering-researcher-submenu-guard-dead.md) | KI-027：**`npm run test:e2e` 目前唯一的紅燈就是這支**（86 passed／1 failed）。`overlay-layering.spec.ts` 展開研究員子選單後的斷言寫死 7 顆按鈕，但 WP-54 / T6 加入第 4 個子選單項後實際是 8 顆；helper 在數量檢查就 `return null`，因此 KI-003 的重疊不變式**自 WP-54 起未被真正驗證**（錯誤訊息 `Expected [] Received null` 看起來像重疊結果，會誤導）。probe 量測確認 layout 沒回歸（8 顆的相交清單為 `[]`、面板 top 188→405.5），錯的是常數。**🔴 診斷完成（2026-09-07，發現於 WP-56 T-exit），修法待落地；production 0 改動。跑任何 exit gate 前先讀，避免重複診斷** |
| [known_issue/KI-028-capture-script-orphans-dev-server-hijacking-e2e-history-root.md](known_issue/KI-028-capture-script-orphans-dev-server-hijacking-e2e-history-root.md) | KI-028：`scripts/capture-wp56-visuals.mjs` 在 Windows 以 `cmd.exe` wrapper 起 dev server，`finally` 的 `server.kill()` 只殺最外層 ⇒ npm→cmd→vite 三層孤兒化、5173 持續被佔。之後 `npm run test:e2e` 依 `reuseExistingServer` **靜默重用**它，整個 suite 改跑 `FPS_HISTORY_ROOT=.wp56-capture-tmp/history`（非 config 的 `.playwright-tmp/history-dev`）。測試全綠故無聲；後果是 NFR-48.6／FM-48.4 的 dev/preview 雙 root 前提被換掉、dev/preview root-lock（HTTP 423）失敗停止重現而可能被誤讀為已修、history-root 相依的 e2e 證據落在未宣告組態。**🔴 診斷完成（2026-09-07，含 PID 祖先鏈證據），修法待落地。引用任何 e2e 證據前先確認 5173 的擁有者** |
| [known_issue/KI-029-prop-clearance-inflates-sphere-hitbox-to-box-corner-radius.md](known_issue/KI-029-prop-clearance-inflates-sphere-hitbox-to-box-corner-radius.md) | KI-029：`clearance.ts` 的 `targetHitboxRadius()` 回**box 角點半徑**（半對角線），卻被 `validateClearance()` 無條件套在 `shape:'sphere'` 的 hitbox 上 ⇒ prop 淨空高估 `√3×`（`0.732·r`）。這是 **KI-021 同一病理的第三處、也是唯一未修的一處**（引擎 `HitDetector` 與 KI-021 修好的離線推導都已 shape-aware）。方向保守 ⇒ **只會誤拒、不會產生錯資料**，故從未在任何資料稽核中現形；今日唯一可達組合（WP-54 tracking pilot sphere × `field-low`）閘比宣稱幾何**嚴 9–13%**、實測仍全綠。真正的風險是反方向：WP-57 T3 已另立 shape-aware 的 `spiderWideTargetRadiusU()`，「為了一致性」把任一邊改成另一邊都會踩到。**🔴 診斷完成（2026-09-07),修法待落地。動任何淨空/hitbox 半徑常數前先讀,避免製造第二定義** |
| [known_issue/KI-030-history-e2e-flaky-under-parallel-workers.md](known_issue/KI-030-history-e2e-flaky-under-parallel-workers.md) | KI-030：同一 HEAD 連跑三次全量 Playwright 得到兩種結果 —— 預設 worker 數首次 **87 passed／4 failed**，`--workers=1` 與第二次預設執行皆 **90 passed／1 failed**；多出來的 3 個是 `history-library.spec.ts:196`／`:288` 與 `history-persistence.spec.ts:44`，單獨重跑全綠。已排除 production 缺陷、資料安全、KI-028 與當次程式碼變更。⇒ **`npm run test:e2e` 目前不是可重現的門檻**，真正的回歸可能被當成「又是那三個 flake」放行。**🔴 未修:已觀察並縮小範圍但根因未定**（失敗當下的 `test-results/**/error-context.md` 已被隔離重跑覆寫,兩個假設無法分辨）。發現於 2026-09-08 WP-57 T-exit。**修好前須連跑三次全量且結果一致才可關閉;動測試前先保存錯誤文字** |
| [known_issue/KI-031-detection-sustained-ticks-dies-when-aim-updates-slower-than-sim.md](known_issue/KI-031-detection-sustained-ticks-dies-when-aim-updates-slower-than-sim.md) | KI-031：`firstSustainedDecrease()` 要求 **`sustainedTicks`（預設 4）個連續**樣本低於門檻，隱含「每個 sim tick 的 `aim` 都會更新」這個從未寫下的前提。aim 更新率低於 128 Hz 時（本例 60 Hz 顯示 ⇒ 有效 ≈64 Hz）負值與 0 **嚴格交替**，連 2 連都湊不出來 —— 四份真人 `spider-shot-wide-v1` 匯出實測 **detected = 0/113**（`sustainedTicks: 1` → 111/113；**不是門檻造成的**）。下游 `switchReaction`／`movementTimeMs` 在真人 run 上 **0/34**，而 `status: timeout` 是合法值 ⇒ 與「受測者沒反應」**完全不可分**、CI 全綠。144 Hz 以上不可見、60 Hz 硬性失敗的**懸崖**,故既有 fixture 從未現形。⚠️ **C-D5 雙實作**（TS `detectionDerivation.ts` ↔ Python `detect.py` + 凍結的 `detect-parity` golden）,判準變更須兩端同步升版。**🔴 未修（2026-09-08,發現於 WP-57 T5 真人校準）。解讀任何真人 run 的 detection/reaction 指標前必讀;繞道 = 以 `sustainedTicks: 1` 呼叫並標註** |
| [known_issue/KI-032-promoted-golden-parity-asserts-bit-equality-on-floats.md](known_issue/KI-032-promoted-golden-parity-asserts-bit-equality-on-floats.md) | KI-032：C-D5 晉升指標的四個 Python golden 測試以 **dict `==`(浮點逐位相等)**比對 committed golden ⇒ **在錄製機以外的任何機器必紅**。`main`（`445fb7c`）實測 `uv run pytest` **4 failed／488 passed**，差異全落在尾位（最大相對偏差 **~2e-14**）;**所有決定性欄位逐位相同**（`verdict` 三筆全 `research_only`、`tickRange`、`gate_thresholds`、`xcorr_params`）⇒ 無任何結論或門檻翻面,**不需重跑分析或撤回宣稱**。根因為 numpy／scipy wheel 的 OpenBLAS **`DYNAMIC_ARCH`**（SIMD kernel 由執行期 CPU 決定）;已實測排除版本漂移、產生器被改、BLAS 執行緒 reduction order、本機非決定性（連跑三次 sha256 相同）。同檔內即有對照:量同一統計量的 `pytest.approx(…, abs=1e-12)` **通過**、逐位 `==` **失敗** ⇒ 失效在比對紀律不在演算法。損害是 **C-D5 的 Python 半邊守門能力已喪失**（與 KI-030 對 Playwright 同型）,且任何寫「`uv run pytest` exit 0」的 DoD 皆無法達成。**🔴 診斷完成（2026-09-09,發現於分支合併審查）,修法待落地。引用 Python 側 parity 讀數前先讀;修法禁重錄 golden、禁動 `version` 字串** |
| [known_issue/KI-033-repositioning-flag-omits-acquisition-failures-and-undercounts-stall-start.md](known_issue/KI-033-repositioning-flag-omits-acquisition-failures-and-undercounts-stall-start.md) | KI-033：抬滑鼠疑慮旗標（WP-57 T5）兩處**系統性漏報**。① acquisition failure（整段無 `onTarget` 樣本）被輸出成 `suspected: false`,與「窗可推導且無停滯」**共用同一編碼** ⇒ 「無從判定」被歸進「乾淨」,而「拉槍拉不到目標」正是抬滑鼠最強的訊號,方向恰好最壞;與 **KI-031** 耦合後放大（`onsetMs` 幾乎全 `undefined` ⇒ 旗標在真人資料上近乎恆偽卻像「量過且乾淨」）。② `omega[i]` 描述區間 `(t[i−1], t[i]]`,但區段起點取首個合格樣本的時間戳 ⇒ 長度少算最多一個 tick（128 Hz = 7.8125 ms,對 `stallMinMs=150` 為 **~5.2% 恆向低估**）。兩項皆漏報方向、零 `src/` importer（C-D3 未違反）、今日無錯資料流出;①在合成 cohort 上**永不觸發**,故 19 個既有測試沒測出來。⚠️ **雙重排序閘**:[GD-37 ②](exec-plan/DECISIONS.md) 把本函式語意與真人校準值凍結為 WP-61 消融基線 ⇒ **T3 判定產出前不得落地**;① 須待 **KI-031** 修好才能在真人資料上驗證。**🔴 診斷完成（2026-09-09,發現於分支合併審查）,修法被排序阻塞。另一份實作存為本機 tag `archive/ki-033-wp-57-t5-alt-impl`（`a0437ae`）** |
| [known_issue/KI-034-prestimulus-baseline-overlaps-prior-engagement.md](known_issue/KI-034-prestimulus-baseline-overlaps-prior-engagement.md) | KI-034：`t_detect` 預設 `preStimulusMs:500` 在快速連續呈現中把前一次接戰當成新目標 baseline，抬高 SD/threshold 並以合法 `timeout` 靜默漏掉快 trial。三份 240 Hz 真人 wide run：500 ms detected **50/132**、120 ms 敏感度分析 **132/132**；漏掉 82 筆的 reaction 中位 171.9 ms，原本留下 50 筆 234.4 ms，且 82 個 timeout 的 `baselineInsufficient` 全 false。與 KI-031 的低 aim-update 連續性缺陷是不同失效族；TS/Python parity 需同步升版。**🔴 診斷完成（2026-09-10），修法待決策；禁把 120 ms 敏感度值當正式替代** |
| [known_issue/KI-035-mouse-gain-stale-after-sensitivity-or-fov-change.md](known_issue/KI-035-mouse-gain-stale-after-sensitivity-or-fov-change.md) | KI-035：操作員若在**載入 drill 之後**才調感度或 FOV，`ticks[].dYaw`/`dPitch` 沿用**舊 gain** 積分，而匯出時的 `meta.mouseIntegration` 以**當下**設定重算 ⇒ 兩者發散且**離線不可察覺**（匯出無逐 tick gain 欄位，`aim` 與 `dYaw` 本就不同時鐘域故無法對帳）。根因：`onSensitivityChange`/`onFovChange`（`main.ts:493-494`）只呼叫 `cameraController.set*`，而 `recorder.configureMouseIntegration()` 只在**換武器**（`main.ts:1347`）與**換 drill**（`main.ts:1417`）時被呼叫。`main.ts:712-713` 明文宣稱兩者「不可能發散」——該不變式在這條路徑上不成立。`aim`、命中判定、彈道與 sim 決定性**皆不受影響**，受污染的只有 tick 窗積分角位移這一條流。修法採 **(a) 補接線 + (b) 錄製中鎖定設定**併行（(c) provenance 化未採，要動匯出 schema）。**✅ 已修（2026-09-10 登記 → 2026-09-14 修復於 [WP-63](exec-plan/active/stage13/wp-63-micro-flick-v8-measurement-foundation/README.md) T2，決策 [`BD-039`](known_issue/BUGFIX-DECISIONS.md)；號撞了兩次：規劃期的 `BD-035` 被 KI-038 取用、改取的 `BD-038` 又被 `main` 保留給 KI-034）** |
| [known_issue/KI-036-registry-projection-failed-hides-which-precondition-broke.md](known_issue/KI-036-registry-projection-failed-hides-which-precondition-broke.md) | KI-036：`DrillMetricRegistry.project()` 的 `catch` 是**單一 catch-all**（連 `error` 都沒綁定）—— `meta.session.participantId` 缺席、`weaponId`/`movementModel`/`fovDeg` 缺席、`protocolGuard.noMovement` 不為真、`targets.hitbox`/`scene.eye` 缺席、**宣稱角徑與匯出 hitbox 不符**（一道真正的完整性檢查）等**至少七類**失敗共用 `reasonCode: 'projection-failed'`。三份 2026-09-09 真人 `spider-shot-v3` 匯出全數落在這裡，真因只有一項:**`meta.session` 整個區塊缺席**（錄製時未填受試者代號，與 `meta.dpi` 同型:瀏覽器讀不到、只能人填）。五個 descriptor 的值**本身完全算得出來** ⇒ 失效的是 metadata 不是指標，但輸出上不可分;整批 run 靜默退出趨勢。與 KI-031 同型（合法值掩蓋可修的問題、CI 全綠）。⚠️ `HANDOFF-v3-real-data.md` §9 的 DoD「三份都回 `ready`」是**未實跑 registry** 寫下的預期。**🔴 診斷完成（2026-09-09，stage14 v3 真人報告），修法待落地。讀任何「這批 run 沒進趨勢」的結論前先讀** |
| [known_issue/KI-037-valid-duration-includes-countdown.md](known_issue/KI-037-valid-duration-includes-countdown.md) | KI-037：`validDurationMs()` = **最後 tick − 第一 tick**，含 3 秒開場倒數與 1.4–2.1 s 尾段 tick，而分子全部落在 60.0 s 計分窗內 ⇒ **主指標** `spider-v3.peripheral-hits-per-minute` **恆向低估**。三份真人匯出實測分母 65.148／64.656／64.406 s ⇒ **+8.6%／+7.8%／+7.3%**;33.2/36.2/40.1 應為 36.0/39.0/43.0 hits/min。同 cohort 內排序不受影響（偏差同向），但**絕對值不可對外報數**，且低估幅度逐場差 1.3 個百分點 ⇒ 也是一個與表現無關的雜訊源。`spider-shot-v2` 同受影響。⚠️ **不得就地改語意** —— v2/v3 皆凍結協定，而 `CompatibilityKey` 十欄沒有一欄會因此改變 ⇒ 不可比的兩組數字會被靜默合併。**🔴 診斷完成（2026-09-09，stage14 v3 真人報告），修法待落地;建議併入 OQ-66.2 的 descriptor 重選。引用任何 hits/min 絕對值前先讀** |
| [known_issue/KI-038-spider-v3-peripheral-side-hardcoded-collapses-lr-aggregates.md](known_issue/KI-038-spider-v3-peripheral-side-hardcoded-collapses-lr-aggregates.md) | KI-038：`spider-shot-v3` 的**周邊** spawn 在 [`TargetManager.ts:486`](../src/sim/TargetManager.ts#L486) 寫死 `side: 'R'` —— 三份真人匯出的**全部 119 個周邊 `visible` 事件相異取值數 = 1**。下游 `PeekWindowTs.side` → `curve-v1` 左右分群整組塌到單邊（實測 `omega.left.n === 0`／`right.n = 74/78/86`），而 `NormalizedCurve` 的 `n: 0` 是**合法值**、無任何旗標 ⇒ 與「這位受試者從不往左打」不可分（KI-031 同型的靜默歸零）。對照組在同一個函式裡:`wide-v1` 回的是真的 `cell.side`。根因是 v3 的分層軸為 `azimuth × cosRadius`、**沒有 side 這一維**。正確的 v3 左右來源已存在 = `deriveSpiderShotTransitions()` 的 eye-frame `side`（實測 L/R 均衡）。`v1/v2` 同樣寫死 ⇒ **整個 spider 家族**的問題。今日無錯資料流出（左右曲線零消費者）。⚠️ 修法動到 `curve-v1` 輸出形狀 ⇒ **C-D5 兩端同步 + golden 重跑**，而 **KI-032 未修前 golden 無法在本機驗證** ⇒ 硬相依。**🔴 診斷完成（2026-09-09，stage14 v3 真人報告），修法被 KI-032 排序阻塞。對任何 spider 家族 payload 讀 `.side` 前先讀** |
| [known_issue/KI-039-session-plan-weapon-select-value-assigned-before-options.md](known_issue/KI-039-session-plan-weapon-select-value-assigned-before-options.md) | KI-039：Session Plan 逐列武器 `<select>` 在任何 `<option>` 建立**之前**就指派 `value` ⇒ HTML selectedness 讓指派靜默丟棄、顯示落回「—（drill 預設）」，而 `item.weaponId` 原封不動。`renderItems()` 每次重繪（新增／刪除／▲▼／拖曳）都製造一次「顯示 ≠ 狀態」：weapon-pinned drill 上是**清不掉**的「預覽不可用」死結，未 pin 的 drill 上則**靜默**以操作員沒看到的武器開跑（`CompatibilityKey` 含 `weaponId` ⇒ 趨勢線被切成兩群）。**✅ 已修（2026-09-14，BD-037：value 指派移到 option 之後；並讓 `FakeElement` 忠實模型 `<select>`，那是它藏過六次綠燈的原因）** |
| [known_issue/KI-040-fullscreen-suspect-never-resets-and-restart-cannot-recover.md](known_issue/KI-040-fullscreen-suspect-never-resets-and-restart-cannot-recover.md) | KI-040：`experimentSession.suspect` 一旦為 true **永不復位**（全檔只有初值 `false` 與失效 `true` 兩個賦值，無 reset API）⇒ 同一分頁內**之後每一場**匯出都被標 `meta.suspect=true`，**靜默**；而全 repo 唯一的 `requestFullscreen()` 綁在資格閘 `onEnter`，plan 進行中重開會撞 `SessionRunner is already active` ⇒ Session Plan 的該項在 Esc 之後**沒有任何操作可以救回有效**，只能 reload；橫幅又由「有沒有人呼叫 `hideSuspectWarning()`」驅動而非旗標真值 ⇒ 資格閘重入時**UI 說沒事、資料說 suspect**。WP-69 的 pause 把觸發窗口顯著放大（非迴歸）。**✅ 已修（2026-09-15，[WP-70](exec-plan/active/stage16/wp-70-run-scoped-condition-validity/README.md) T1–T6，[`BD-040`](known_issue/BUGFIX-DECISIONS.md)／[`GD-47`](exec-plan/DECISIONS.md)）**：`suspect` 的 fullscreen 成分改為 **run 級**（`meta.validity.fullscreenExited`，`resetState()` 每場歸零），protocol 路徑補上同一個 KI-007 錄製窗閘，橫幅改由旗標真值驅動，並新增不重啟 plan 的「恢復條件」入口。GD-10 **補澄清註記而非修訂**。⚠️ 引用 2026-09-15 之前的匯出時注意：那些 payload 的 `suspect=true` **無法歸因**，且不回填 |

---

## Operational Pause / Attempt-Validity Entry

**暫停、失效與整場 Restart**：[operational/pause-invalid-restart.md](operational/pause-invalid-restart.md)（WP-69 交付，2026-09-15；WP-70 於同日補上「恢復條件」一節）——錄製中 Pointer Lock 遺失即暫停且該 attempt **永久失去實驗採納資格**；收工由單一 finalization gate 三分為 `eligible-candidate`／`invalid-retained`（只有手動下載的 `.invalid-paused` 稽核檔）／`discarded`（無 payload、清空 recorder）。`meta.suspect` 是品質警告，**不是**這三態的替代品。操作步驟與現場檢查清單見該檔；術語見 [CONTEXT.md](../CONTEXT.md)「paused-invalid attempt／attempt disposition」；schema 欄位見 [operational/schema.md](operational/schema.md#metavalidity)；操作員速查列在 [guideline/operator-manual.md §4.4／§8.3](guideline/operator-manual.md)。

**條件失效的效力單位是 run**：[WP-70](exec-plan/active/stage16/wp-70-run-scoped-condition-validity/README.md)（2026-09-15，[`GD-47`](exec-plan/DECISIONS.md)）——`meta.suspect` 的 fullscreen 成分改由「**該次 run** 的錄製窗內是否退出過全螢幕」判定並以 `meta.validity.fullscreenExited` 自述來源，與已是 per-run 的效能地板成分對齊；一次中斷不再污染其後每一場（[KI-040](known_issue/KI-040-fullscreen-suspect-never-resets-and-restart-cannot-recover.md)）。fullscreen 退出、Pointer Lock 遺失、pause 是**三個構念**，效力單位與後果都不同——對照表見 [operational/schema.md](operational/schema.md#metavalidity)。掉出全螢幕後的**恢復條件**入口（不重啟 plan、重跑同三項檢查、通過才 restart 本項）操作步驟見 [operational/pause-invalid-restart.md](operational/pause-invalid-restart.md)；**FM-70.4（`requestFullscreen()` 必須在第一個 `await` 之前）e2e 永遠測不到**，實機手動清單見 [operational/fullscreen-recovery-manual-check.md](operational/fullscreen-recovery-manual-check.md)。
