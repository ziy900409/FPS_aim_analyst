...
| [known_issue/KI-005-omega-render-sim-aliasing.md](known_issue/KI-005-omega-render-sim-aliasing.md) | KI-005：ω(t) 受 render(~240Hz)/sim(128Hz) **zero-order-hold aliasing** 汙染 — 每 8 tick 一個假凹口，`merged_adjacent_peaks` 15/19、有效產率 4/19。**✅ 選項 A 已落地（A1，2026-08-06）；A2-T1(新採樣)+A2-T2(四項複驗，FM-1 以機器精度關閉)已完成(2026-08-07)；A2-T3(`seg-v2` 重掃)+A2-T4(M14 重新宣告)待辦**。碰 `ticks[].aim` 逐 tick 差分（ω/角加速度/jerk）或 `seg-*` 參數前必讀。既有兩份匯出的 ω(t) 一律不可用（不做回溯清洗），M14 ③④⑤ 於 A2-T4 完成前仍撤回 |
