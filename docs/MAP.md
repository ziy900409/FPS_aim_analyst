...
| [known_issue/KI-034-registry-projection-failed-hides-which-precondition-broke.md](known_issue/KI-034-registry-projection-failed-hides-which-precondition-broke.md) | KI-034：`DrillMetricRegistry.project()` 的 `catch` 是**單一 catch-all**（連 `error` 都沒綁定）—— `meta.session.participantId` 缺席、`weaponId`/`movementModel`/`fovDeg` 缺席、`protocolGuard.noMovement` 不為真、`targets.hitbox`/`scene.eye` 缺席、**宣稱角徑與匯出 hitbox 不符**（一道真正的完整性檢查）等**至少七類**失敗共用 `reasonCode: 'projection-failed'`。三份 2026-09-09 真人 `spider-shot-v3` 匯出全數落在這裡，真因只有一項:**`meta.session` 整個區塊缺席**（錄製時未填受試者代號，與 `meta.dpi` 同型:瀏覽器讀不到、只能人填）。五個 descriptor 的值**本身完全算得出來** ⇒ 失效的是 metadata 不是指標，但輸出上不可分;整批 run 靜默退出趨勢。與 KI-031 同型（合法值掩蓋可修的問題、CI 全綠）。⚠️ `HANDOFF-v3-real-data.md` §9 的 DoD「三份都回 `ready`」是**未實跑 registry** 寫下的預期。**🔴 診斷完成（2026-09-09，stage14 v3 真人報告），修法待落地。讀任何「這批 run 沒進趨勢」的結論前先讀** |
| [known_issue/KI-035-valid-duration-includes-countdown.md](known_issue/KI-035-valid-duration-includes-countdown.md) | KI-035：`validDurationMs()` = **最後 tick − 第一 tick**，含 3 秒開場倒數與 1.4–2.1 s 尾段 tick，而分子全部落在 60.0 s 計分窗內 ⇒ **主指標** `spider-v3.peripheral-hits-per-minute` **恆向低估**。三份真人匯出實測分母 65.148／64.656／64.406 s ⇒ **+8.6%／+7.8%／+7.3%**;33.2/36.2/40.1 應為 36.0/39.0/43.0 hits/min。同 cohort 內排序不受影響（偏差同向），但**絕對值不可對外報數**，且低估幅度逐場差 1.3 個百分點 ⇒ 也是一個與表現無關的雜訊源。`spider-shot-v2` 同受影響。⚠️ **不得就地改語意** —— v2/v3 皆凍結協定，而 `CompatibilityKey` 十欄沒有一欄會因此改變 ⇒ 不可比的兩組數字會被靜默合併。**🔴 診斷完成（2026-09-09，stage14 v3 真人報告），修法待落地;建議併入 OQ-62.2 的 descriptor 重選。引用任何 hits/min 絕對值前先讀** |
| [known_issue/KI-036-spider-v3-peripheral-side-hardcoded-collapses-lr-aggregates.md](known_issue/KI-036-spider-v3-peripheral-side-hardcoded-collapses-lr-aggregates.md) | KI-036：`spider-shot-v3` 的**周邊** spawn 在 [`TargetManager.ts:486`](../src/sim/TargetManager.ts#L486) 寫死 `side: 'R'` —— 三份真人匯出的**全部 119 個周邊 `visible` 事件相異取值數 = 1**。下游 `PeekWindowTs.side` → `curve-v1` 左右分群整組塌到單邊（實測 `omega.left.n === 0`／`right.n = 74/78/86`），而 `NormalizedCurve` 的 `n: 0` 是**合法值**、無任何旗標 ⇒ 與「這位受試者從不往左打」不可分（KI-031 同型的靜默歸零）。對照組在同一個函式裡:`wide-v1` 回的是真的 `cell.side`。根因是 v3 的分層軸為 `azimuth × cosRadius`、**沒有 side 這一維**。正確的 v3 左右來源已存在 = `deriveSpiderShotTransitions()` 的 eye-frame `side`（實測 L/R 均衡）。`v1/v2` 同樣寫死 ⇒ **整個 spider 家族**的問題。今日無錯資料流出（左右曲線零消費者）。⚠️ 修法動到 `curve-v1` 輸出形狀 ⇒ **C-D5 兩端同步 + golden 重跑**，而 **KI-032 未修前 golden 無法在本機驗證** ⇒ 硬相依。**🔴 診斷完成（2026-09-09，stage14 v3 真人報告），修法被 KI-032 排序阻塞。對任何 spider 家族 payload 讀 `.side` 前先讀** |
