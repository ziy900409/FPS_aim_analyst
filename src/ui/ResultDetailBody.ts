/**
 * WP-49 T3 — shared read-only Result body (README §2.9 "current/historical presentation parity",
 * D-49.P4). Renders a `ResultPresentation` (metrics grid, reaction histogram, recoil path,
 * research-promoted diagnostics, training diagnosis, quality flags) with zero restart/save/export
 * affordances — those stay in the wrapper that mounts this component (`ResultScreen.ts` for the
 * current run, `ui/history/HistoricalRunDetail.ts` for a historical run).
 */

import type { AimOffset } from '../metrics/compute.ts';
...
import type { RecoilCompensationSummary, ResultCard, ResultPresentation, QualityFlagsInput } from '../results/ResultPresentation.ts';
...
export interface ResultDetailBodyHandle {
  readonly element: HTMLElement;
  render(result: ResultPresentation): void;
  dispose(): void;
}

export function createResultDetailBody(): ResultDetailBodyHandle {
  const root = document.createElement('div');
  root.dataset.section = 'result-detail-body';
  ...
  root.append(method, grid, chartTitle, chart, recoilTitle, recoilChart, promotedSection, diagnosisSection, qualityFlagsSection);

  function render(result: ResultPresentation): void {
    const { summary, promoted, diagnosis, qualityFlags } = result;
