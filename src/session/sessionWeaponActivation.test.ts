...
describe('WP-62 T3 — resolveActiveWeapon() 是武器 precedence 的唯一定義', () => {
  ...
  it('每個自宣告武器的 drill 逐一經 resolver 解析回自己宣告的武器（Controls 面板顯示值的來源）', () => {
    ...
    // 回饋獨立 drill + 使用者 2026-09-14 的 30 s／無置中準備窗變體（皆沿用 pilot 的
    // tracking_pilot_hold）+ WP-63 T1 的 micro flick v8 與 WP-68 T1 的計時制姊妹 v9（兩者同宣告
    // `usp_s_laser`，為量測儀器而非操作員可選變項）。數字寫死，讓
