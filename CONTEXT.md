...
| **eye-frame 球面座標(`spiderWideEyePos()`,WP-57)** | `pos = eye + d·(sin(yaw)·cos(pitch), sin(pitch), −cos(yaw)·cos(pitch))`,`eye = (0, PLAYER_EYE_HEIGHT_U, 0)`([spiderEyeFrame.ts](src/sim/spiderEyeFrame.ts))。`abs(pos − eye)` 恆等於 `d` ⇒ 目標角徑恆定;`yaw`/`pitch` 即玩家的螢幕水平/垂直視角,中心目標為 `yaw = pitch = 0`。與 v1/v2 `peripheralPos()` 的**世界原點**錐軸(GD-32)、與 `angularSpawnPose()` 的**圓柱**參數化(`y = TARGET_Y + tan(pitch)·d`,3D 距離隨 pitch 漂)三者刻意分離,不互相取代。 |
