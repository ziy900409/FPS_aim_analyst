...
| **原始滑鼠取樣（raw mouse sample）** | Pointer Lock 期間由 `pointermove`／`getCoalescedEvents()` 取得的逐事件滑鼠樣本：`dx`、`dy` 為未經跨事件聚合的 raw counts，`t` 來自事件自身 `event.timeStamp`，與 tick 時間戳同屬 `performance.now()` time origin。WP-60 的 `mouseSamples` 以 columnar 格式匯出：`t0Ms` + `dtUs`（整數微秒 delta）+ `dx` + `dy`。 |
| **時間間隙（time gap）** | 相鄰 raw mouse sample 之間的時間差；當 `dt` 超過呼叫端明確給定的門檻時，代表硬體取樣串流出現空洞。它是中性的時序原語，不等同於「抬滑鼠」判定；Pointer Lock 中斷、drill 尚未進行／已結束也會形成空洞，需要後續消歧。 |
| **感測器離地（sensor lift）** | WP-61 的待驗證構念：以事件級 raw mouse sample stream 的時間間隙作候選，再用獨立 self-report `annotation` event（`kind:'sensor_lift'`，`KeyL` down/up interval）與空洞前後運動學驗證。訊號來源是 `mouseSamples.dtUs/dx/dy` 與 `events[].annotation`，同一 `event.timeStamp` 時鐘域；時間粒度約 1 ms。它回答「這個候選空洞是否可被判為感測器離地」，不回答「玩家是否重新定位滑鼠」或「lift 真實起點在第幾毫秒」。未通過 WP-61 gate 前一律 `research_only`，不得進教練報告。 |
| **取樣區段（sample segment）** | 由 raw mouse sample 串流依時間間隙切出的連續樣本區間，index 範圍以 `startIndex`／`endIndex` 表示。它只描述取樣連續性，不自行宣稱 movement archetype、repositioning suspicion 或 sensor lift。 |
| **未取鎖區間（unlocked interval）** | 由 `pointer_lock` 事件（`{ locked, t }`）推導出的「Pointer Lock 中斷中」時間區間：`locked: false` 開一段，下一個 `locked: true` 關掉它。它的用途是**歸因**：與之實質重疊（重疊長度 > 0）的時間間隙屬於「取鎖中斷造成的空洞」，不是感測器離地，故由 `segmentByTimeGap()` 收進 `lockGapIndices` 並排除在 `gaps` 之外。端點相接不算重疊 —— 恰在間隙起點收掉的中斷不解釋它後面那個間隙。 |
...
| **`spiderShot` schedule(中心—周邊排程,WP-36／KI-026)** | `DrillConfig.spiderShot?: SpiderShotScheduleConfig` 是獨立於 L/R 的排程分支。v1/v2 的 `center-peripheral(-stratified)` 保留 legacy origin-frame spawn；現行 Session Assessment `spider-shot-v3` 使用 `center-peripheral-eye-stratified`，由 canonical eye 生成固定距離球面座標。`TargetManager` 內部 `nextSpiderZone: 'center' \| 'peripheral'` 取代該分支的既有 `nextSide`；`schema.ts` 拒絕與 legacy seeded-spawn 欄位併用。定義見 [analysis-spider-shot.md](docs/operational/analysis-spider-shot.md)。 |
| **`zone`(中心/周邊標記,`spider-shot-v1`,WP-36 T1)** | `DrillEvent{type:'visible'}` 的 additive 欄位 `zone?: 'center' \| 'peripheral'`,由 `TargetManager` 於 spawn 時蓋章;省略時(既有 drill)零回溯相容成本。`side` 在 `center-peripheral(-stratified/-eye-stratified)` 三支恆為 `'R'`,僅維持既有型別相容位置,不承載象限語意——象限改由 `zone` + 抵達座標離線重建(WP-57 的 `-yawpitch` 是唯一例外,見下方「`side` 的兩個來源」)。定義見 [analysis-spider-shot.md](docs/operational/analysis-spider-shot.md)。 |
| **`D_deg`／`W_deg`／象限標籤(Spider Shot,WP-36 T2／BD-026)** | `D_deg` 是前一與抵達目標各自由其 visible tick 的**玩家眼睛**指向目標中心之無號球面夾角；eye 由 `meta.scene.eye` + tick `px/pz × meta.simToWorld` 還原，複用 `resolveEyeOrigin()`／`eyeOriginForTick()`／`angularDistanceDeg()`。`W_deg = 2 × atan((hitbox 半寬)/眼睛至目標距離) × 180/π`,`hitbox` 唯一來自 `meta.targets.hitbox`(GD-7)。象限是呈現層標籤；transition cell 格式仍為 `spider:d=<6 位小數 D_deg>;w=<6 位小數 W_deg>`。定義見 [analysis-spider-shot.md](docs/operational/analysis-spider-shot.md)。 |
| **`spider-shot-v3` 交付契約(KI-026／BD-026)** | 正式 Assessment 新世代：專用 `spider-shot-room` eye=`(0,1.6,0)`，eye-frame center/peripheral 距離恆 8 u、sphere 角徑 2.0°、半徑 10–25°、translation locked + noMovement。`loadDrill()` 驗證 eye anchor、距離、角徑/hitbox 與 room envelope；protocol=`spider-shot-v3@1.0.0`，history condition cell 從 payload snapshot 建立。v1/v2 legacy spawn/golden 不變，Session `spider-shot` 家族解析到 v3。 |
| **`center-peripheral-yawpitch`(大幅度拉槍排程,WP-57)** | `SpiderShotScheduleConfig` 的第四支 `kind`(`spider-shot-wide-v1`,practice-only)。與 v1/v2/v3 是**同輩不同構念**而非後繼版本:刺激的眼睛所見角位移由 v2 的 ~10–25° 推到 ~40–70°,周邊落點貼近當次水平 FOV 極限。周邊 spawn 取自 seeded 的 `side (L/R) × pitchBand` 洗牌佇列(耗盡即重建重洗,沿用 WP-44 慣例);pitch 分層只為**平衡**,不是條件變因。`schema.ts` 逐欄位 strict 驗證,v1/v2/v3 三支的型別、驗證與 spawn 結果逐位不變。定義見 [analysis-spider-shot.md](docs/operational/analysis-spider-shot.md)。 |
| **eye-frame 球面座標(`spiderWideEyePos()`,WP-57)** | `pos = eye + d·(sin(yaw)·cos(pitch), sin(pitch), −cos(yaw)·cos(pitch))`,`eye = (0, PLAYER_EYE_HEIGHT_U, 0)`([spiderEyeFrame.ts](src/sim/spiderEyeFrame.ts))。`abs(pos − eye)` 恆等於 `d` ⇒ 目標**角徑**恆定;`yaw`/`pitch` 即玩家的螢幕水平/垂直視角,中心目標為 `yaw = pitch = 0`。⚠️ **角徑恆定 ≠ 畫面形狀恆定**:直線透視下離軸的球投影為**橢圓**,軸比 ≈ `1/cos θ`、螢幕足跡 ∝ `1/cos³θ`(實錄 FOV 75/aspect 2.0031 下軸比 1.57–1.75、足跡為中心目標的 5.4×,且窗內隨 yaw 變動 1.38× ⇒ **與 `D_deg` 共變**)。命中判定不受影響(ray–sphere,要打中的是角度上的正圓),但它是 `switchReaction` 的混淆因子 —— 見 OQ-57.8 與 [analysis-spider-shot.md](docs/operational/analysis-spider-shot.md)。與 v1/v2 `peripheralPos()` 的**世界原點**錐軸(GD-32)、與 `angularSpawnPose()` 的**圓柱**參數化(`y = TARGET_Y + tan(pitch)·d`,3D 距離隨 pitch 漂)三者刻意分離,不互相取代。 |
| **`resolvedFrom`(arm-time 解析 provenance,WP-57)** | `spiderShot.resolvedFrom` 記錄解析當時的 `fovDegVertical`/`aspect`/`screenMargin`/`kLo`/`targetAngularDiameterDeg` 五個量,隨 `meta.spawn.spiderShot` 原樣落匯出。**刻意的冗餘**:`aspect` 在 WP-57 之前完全不在任何匯出欄位裡,沒有它就無法離線重建 yaw 窗(同 FOV 75 下 4:3 與 21:9 的 `yawMax` 相差 15.3°)。yaw 窗只在 **drill arm 時**解析一次並凍結成 config 常數,run 中 resize/切解析度不重解析(GD-10)。 |
| **`side` 的兩個來源(spawn side vs 推導 side,WP-57 T4)** | ①`DrillEvent{type:'visible'}.side` 由 `TargetManager` 於 spawn 蓋章:`center-peripheral(-stratified/-eye-stratified)` 三支恆為 `'R'`(型別佔位),`-yawpitch` 的**周邊** spawn 帶真實左右(中心仍 `'R'`)。②`SpiderShotTransition.side?: 'L' \| 'R'`(additive,離線)由抵達點的 **eye-frame `x` 符號**推導,只對 `center-to-peripheral` 輸出,`x === 0` 時**省略而非猜測**。②不是第二套幾何(C-D4):讀的就是 `D_deg`/`W_deg` 已在用的同一組 eye-frame 座標。⚠️ v1/v2 近垂直呈現的 `x` 可能是 `sin(180°)=1.22e-16` 的浮點殘值,判讀時先以 `quadrant` 篩掉 `vertical`。 |
| **`counts/360`／`cm/360`(離線行程推導,WP-57 T4)** | `deriveMouseThrow(payload)`([mouseThrow.ts](src/metrics/mouseThrow.ts))由 `meta.sensitivity` + `meta.dpi` + `meta.fovDeg` + `meta.weapon.ads` 推導,**不新增任何輸入欄位**;gain 一律來自 `resolveMouseGain()`(KI-005 唯一定義,C-D4)。`meta.dpi` 是 self-reported,缺席時 `cmPer360` 回 `undefined` 而非猜一個 DPI(`countsPer360` 不需 DPI 仍成立);`meta.fovDeg` 缺席時 ADS 感度鏈不可稽核,ADS 兩欄一併 `undefined`。用途是抬滑鼠混淆因子的**資料品質標註**輸入,不得進教練報告(C-D3)。 |
| **抬滑鼠疑慮旗標(repositioning suspicion,WP-57 T5)** | `deriveRepositioningSuspicion(payload, options)`([spiderShotRepositioning.ts](src/metrics/spiderShotRepositioning.ts))對每個 `zone: 'peripheral'` 抵達輸出 `{ targetId, suspected, stallStartMs?, stallDurationMs? }`。偵測窗 = canonical movement onset(`tDetectMs`)→ canonical 首次 on-target,**與 `movementTimeMs` 是同一對邊界**;判準 = 窗內存在連續 ≥ `stallMinMs` 且 `abs(omega) < stallOmegaDegPerSec` 的區段,`omega` 走 `omegaDegPerSec()`(C-D4,不自算)。窗界任一端缺席 → `suspected: false` 且不帶欄位,語意是**無從判定**而非「沒有抬滑鼠」。**這是資料品質標註,不是構念(C-D3)**:型別命名一律 `Suspicion`,不提供 count/rate 形狀的輸出,且 `src/` 內**零 importer**(boundary 測試釘死,build 產物亦不含)。門檻**未凍結**、未寫成 production 常數(呼叫端必填)。**2026-09-08 真人校準交付 `stallMinMs = 150`／`stallOmegaDegPerSec = 2`**(四份真人 run 的 ground truth:TP 78%、偽陽 3%、baseline 0%、刻意停頓 17%)。⚠️ 該緊 ω 門檻**條件於錄製機器的取樣特性**,且真人資料上 `deriveDetectionMetrics()` 需 `sustainedTicks: 1` 才有窗([KI-031](docs/known_issue/KI-031-detection-sustained-ticks-dies-when-aim-updates-slower-than-sim.md))。⚠️ 合成期曾宣稱「分離兩者的是 ω 軸而非 duration 軸」,**已被真人資料推翻** —— 分離軸是 **duration @ 緊 ω 門檻**。定義見 [analysis-spider-shot.md](docs/operational/analysis-spider-shot.md)。 |
...
| **`peek-click-transfer-pilot-v1`／self-motion exposure／valid first shot(WP-45)** | Practice/pilot-only 的整合 transfer test：玩家依 A/D cue 從 `peek-ad-corridor-v1` 的中心掩體後移出，`visibleFraction >= 0.5` 的 128 Hz crossing 為 `tMeasurementOnsetMs`，再反向急停首發；miss 可補槍，hit 或 spawn-anchored 3000 ms timeout 推進嚴格 L/R 共 20 presentation。runtime hitscan 與離線 visibility 共用 `occlusionGeometry` segment/AABB kernel，故穿牆 shot 不會 kill。`validFirstShot` 只在首發命中、首發不早於 onset、且 residual speed 低於既有 accuracy gate 時成立；缺失 timing 不補零，並輸出 machine-readable flags。此任務**不取代**`hold-click-v1`/`counterstrafe-reversal-v1` 元件量測、不產生 composite score、不可進正式 Assessment history 或跨家族直接比較；三個角尺寸與 timeout 都是 pilot candidates。定義見 [analysis-peek-click-transfer.md](docs/operational/analysis-peek-click-transfer.md)。 |
| **`peek_click_transfer_pilot_v2`／evidence-collection revision(WP-52)** | 獨立 evidence-collection 版本，與 `peek-click-transfer-pilot-v1` 完全分離的 module/id/seed range（不原地改 v1，保留其既有語意與可追溯性，D-52.1）。WP-52 T0 拍板：spawn-anchored timeout、warmup 政策**沿用 v1 現行數值**，因為當下無真人 pilot evidence 支持變更（D-52.5/6）。角尺寸候選 T0 也先沿用 v1 的 1.5/2/3 deg（D-52.4），但 T4 真人手動走查後回報三個候選手感差異太小，故 D-52.9 把候選拉寬為 **1/2.5/5 deg**（2.5° 為新預設，min→max hitbox 寬度倍數從約 2× 拉大到約 5×）——這是 v1/v2 目前唯一有真實差異的 protocol 數值，其餘仍與 v1 逐位相同。操作端透過既有自由 checkbox Session Plan UI 把 `'peek-click-transfer'` 家族排進去可跑 session（該路徑目前解析到 **v1** 的 drill,證明 KI-016 metadata gap 已解,非啟動 v2；未重新引入 preset 下拉,見 GD-26）；手動測 v2 本身要走研究員模式「單一 Drill 調整」選單,已登記 v2 的 2.5° 預設（`main.ts` `availableDrills`，WP-52 T4）。`buildPeekClickTransferPilotEvidenceReport()`（`src/pilot/peekClickTransferPilotEvidence.ts`）聚合任意數量 session 的 presentations，輸出 completion/timeout/valid-first-shot rate、左右平衡、flag 計數，供 WP-53 go/no-go 判讀；不產生 composite score。仍是 `mode:'practice'`，不進 Assessment history（既有 drill-id-agnostic practice guard 自動涵蓋）。定義見 [analysis-peek-click-transfer.md](docs/operational/analysis-peek-click-transfer.md) §Pilot v2。 |
| **`peek_click_transfer_v1`／formal self-motion transfer Assessment(WP-53)** | WP-53 根據 WP-52 manual gate + 3 場真人 `peek_click_transfer_pilot_v2_masked` session evidence 凍結的正式 Assessment drill id；`mode:'assessment'`，`protocolVersion='peek-click-transfer-v1.0.0'`，凍結值為 `angularSizeDeg=2.5`、`distanceU=8`、`targetCount=20`、spawn-anchored 3000 ms timeout、visibility `N=9`/`0.5`（GD-29）。正式 run 會寫入 `meta.assessment` 並以 `buildPeekClickTransferV1ConditionCell()` 產生 compatibility cell；`DrillMetricRegistry` 只接受 exact `drillId='peek_click_transfer_v1'`，primary trend metrics 為 `valid-first-shot-rate` 與 median `onset-to-hit-ms`，pilot v1/v2 ids 不會併入同一 history/trend cohort。Session Plan 以獨立家族 `'peek-click-transfer-v1'` 選取，不取代 pilot 家族 `'peek-click-transfer'`，也不改 stage6 default 四家族 roster。定義見 [analysis-peek-click-transfer.md](docs/operational/analysis-peek-click-transfer.md) §Formal v1。 |
...
## M. Session 排程 / 家族輪替術語(WP-41,stage7;純函式 `src/session/sessionSchedule.ts`)

> 解決「一場測試裡四個測試家族用什麼順序出場」的排程層問題,與 WP-39 `pilotConfigs.ts`(校準候選參數值)正交,零協定依賴、零 `src/drill/*.ts` 修改(D-41.1/D-41.2)。
