...
| **ring buffer** | 固定大小、物件重用的環狀緩衝，避免配置物件造成 GC 週期性卡頓。**ring buffer 專指輸入緩衝**：真環狀、**消費後槽位重用、drill 內持續繞圈**（事件被 sim 消費即不再需要）；每個 ~1000Hz 事件壓成固定數值欄位 `type,t,a,b`、不 `push` 物件——當下擋 GC、未來 SAB-portable。⚠️ `DataRecorder` 雖也預配置+重用防 GC，但**不是環狀**（每 tick row 要留到匯出）——見 `DataRecorder` 元件列的 **preallocated arena**。 |
