...
| **教練報告 v0（coach report，`coach-report-v0`）** | 一道指令把一份匯出轉成**單檔自足靜態 HTML**（inline CSS + inline SVG，零外部資源、可直接寄送）：drill 摘要、逐 peek 時間軸、上述六個指標各帶 `n`／flags 計數／版本字串／**效度層級**、精度判定、凍結參數與已知限制。`--group-by side｜ads｜weapon_mode` 只**切分**已算好的列，不改任何參數或版本（分層前後參數區塊逐位相同），且不逐組重跑 pre-registered 判定。落實 C-D3／GD-20 紅線：未過構念驗證的量（ε(t)、REC/MR/V phase、SPARC、xcorr、Fitts）一律不進報告。 |
