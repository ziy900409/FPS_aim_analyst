...
| **`peek_click_transfer_pilot_v2`／evidence-collection revision(WP-52)** | 獨立 evidence-collection 版本，與 `peek-click-transfer-pilot-v1` 完全分離的 module/id/seed range（不原地改 v1，保留其既有語意與可追溯性，D-52.1）。WP-52 T0 拍板：角尺寸候選、spawn-anchored timeout、warmup 政策皆**沿用 v1 現行數值**，因為當下無真人 pilot evidence 支持變更（D-52.4/5/6）——v2 的「調整」目前只是換一個獨立、可稽核的 evidence 蒐集回合，尚未變更任何 protocol 數值。操作端透過既有自由 checkbox Session Plan UI 把 `'peek-click-transfer'` 家族排進去即可跑 v2 pilot（未重新引入 preset 下拉，見 GD-26）。`buildPeekClickTransferPilotEvidenceReport()`（`src/pilot/peekClickTransferPilotEvidence.ts`）聚合任意數量 session 的 presentations，輸出 completion/timeout/valid-first-shot rate、左右平衡、flag 計數，供 WP-53 go/no-go 判讀；不產生 composite score。仍是 `mode:'practice'`，不進 Assessment history（既有 drill-id-agnostic practice guard 自動涵蓋）。定義見 [analysis-peek-click-transfer.md](docs/operational/analysis-peek-click-transfer.md) §Pilot v2。 |
