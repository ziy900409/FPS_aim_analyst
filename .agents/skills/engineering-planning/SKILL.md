...
1. Read `AGENTS.md` — §3 執行協議(一 task = 一垂直切片 = 一原子 commit)與 §4 不可違反的硬約束(ADR / GD-n)。這是本 repo 的程序記憶,不是 `AGENTS.md`(那份只有 graphify / codegraph 設定)。
...
- Non-negotiable requirements — 逐條對照 `AGENTS.md §4` 硬約束
...
- Do not skip `AGENTS.md` §4 或 `docs/exec-plan/README.md`,即使需求看起來很小。
