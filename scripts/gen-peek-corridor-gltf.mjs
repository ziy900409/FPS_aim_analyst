// gen-peek-corridor-gltf.mjs - WP-34 / T2
//
// Generates the original CC0 peek-corridor scene asset from the authoritative
// propBounds JSON. The visual geometry is simple box primitives: one cover wall
// plus render-only ground/corridor cues that do not enter clearance.
...
const propsPath = resolve(here, '../src/scene/scenes/peek-corridor.props.json');
const outDir = resolve(here, '../public/assets/scenes/peek-corridor');
const outPath = resolve(outDir, 'peek-corridor.gltf');
mkdirSync(outDir, { recursive: true });

const { props } = JSON.parse(readFileSync(propsPath, 'utf8'));

const KINDS = ['wall', 'ground', 'guide'];
const BASE_COLOR = {
  wall: [0.43, 0.46, 0.5, 1],
  ground: [0.22, 0.24, 0.25, 1],
  guide: [0.68, 0.62, 0.42, 1],
};

const visuals = [
  ...props,
  { id: 'ground', kind: 'ground', min: { x: -6, y: -0.08, z: -12 }, max: { x: 6, y: 0, z: 2 } },
  { id: 'exposed-rest-lane', kind: 'guide', min: { x: 1.35, y: 0.01, z: -9 }, max: { x: 2.65, y: 0.04, z: -7 } },
];
...
const gltf = {
  asset: {
    version: '2.0',
    generator: 'FPS_aim_analyst WP-34 T2 peek-corridor generator (original geometry, CC0)',
    copyright: 'CC0 1.0 - original work, FPS_aim_analyst',
  },
  scene: 0,
  scenes: [{ name: 'peek-corridor', nodes: nodes.map((_, i) => i) }],
  nodes,
  meshes,
  materials,
